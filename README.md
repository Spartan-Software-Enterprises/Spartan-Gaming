...
- In-app stable, beta, and alpha application-update controls backed by packaged release metadata, bounded progress, and a restart confirmation instead of decorative settings.

The primary window can navigate only inside Spartan's private `spartan-app://app` protocol. Cataloged cloud-gaming and browser-game services open in isolated player views, while general external access is restricted to approved gaming-service and download/update origins. Login pages use the provider's own secure forms and profile-scoped Electron session storage; Spartan never receives account passwords or copies provider credentials into its settings. Turning off account isolation restores one shared compatibility session without moving or duplicating credentials.
...
The Settings control center covers standalone application behavior, gaming, streaming, controllers, emulation, providers, performance and power, privacy, accessibility, profiles/workspaces, updates, Android policy, SteamOS/Proton launch preferences, host transport, and device presentation. Preferences remain local unless a future sync mechanism is deliberately configured. Electron hardware acceleration is a real next-launch setting: Spartan persists the bounded choice locally, reports when a restart is required, and applies the software-rendering fallback before Electron becomes ready. GPU preference (automatic/power-saving/high-performance) and the process model (default/maximum isolation/low memory) work the same way, applying the matching Chromium switches on the next launch. Packaged desktop builds can check the selected stable, beta, or alpha GitHub release channel, download verified update metadata, report bounded progress, and ask before restarting; development builds never contact the updater. Optional Electron crash records and verbose application logs also stay on the device: retention is bounded, exported bundles redact provider URLs, addresses, paths, account identifiers, and credential-shaped text, and Settings can erase the entire log immediately. Spartan does not upload these diagnostics. Developer Mode is off by default and explicitly gates the primary application's DevTools menu, desktop shortcuts, and Settings action; provider login and player views are never selected as debugging targets.

## Device-universal direction

| Platform                    | Current project coverage                                                                                | Remaining acceptance gate                                                                     |
| --------------------------- | ------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------- |
| Windows desktop             | Electron packaging, updater runtime/release workflow, shared frontend, native package/build contracts   | Signed installer plus physical capture, input, audio, haptics, and virtual-gamepad validation |
| Linux desktop               | Electron AppImage/Debian packaging, updater metadata, and native Linux contracts                        | Wider distro/hardware coverage and physical `/dev/uinput`/FF validation                       |
| macOS desktop               | Electron packaging, updater runtime/release workflow, and native package/build contracts                | Signed/notarized release and physical platform validation                                     |
