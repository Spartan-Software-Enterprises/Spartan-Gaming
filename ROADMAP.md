...
- [ ] Native OS input injection, audio return, and end-to-end haptics (Linux uinput virtual gamepad/force feedback, Windows XInput rumble, and macOS GameController/CoreHaptics output are implemented when their host capabilities are available; Linux portal consent and audio capture device enumeration are implemented; the full remote session is verified end-to-end over a real in-process Werift loopback with H.264 video and Opus audio; Linux native input still requires verification against a real uinput device and FF-capable kernel, and Windows/macOS virtual gamepad injection remains pending).
