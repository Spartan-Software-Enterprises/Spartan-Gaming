...
- [ ] Native OS input injection, audio return, and end-to-end haptics (Linux uinput virtual gamepad/force feedback, Windows XInput rumble, and macOS GameController/CoreHaptics output are implemented when their host capabilities are available; macOS also has a fail-closed IOHIDUserDevice virtual gamepad path; Linux portal consent and audio capture device enumeration are implemented; Linux native input is verified end-to-end against a real uinput device and force feedback against FF-capable kernels; the full remote session is verified end-to-end over a real in-process Werift loopback with H.264 video and Opus audio; Windows virtual gamepad injection and privileged end-to-end validation remain pending).
