...
test('ticket CLI mints a scoped short-lived ticket without echoing the secret', () => {
  ...
  const result = spawnSync(
    ...
    { encoding: 'utf8', env: { ...process.env, SPARTAN_SIGNALING_SECRET: secret } },
  );
  ...
  assert.deepEqual(
    ...
    { version: 1, sessionId: 'ses-cli-01', role: 'client', subject: 'browser-01', ttlMs: 60000 },
  );
  const broker = createSignalingBroker({ secret });
  const participant = broker.attach({
    sessionId: output.sessionId,
    role: output.role,
...
test('ticket CLI fails without required provisioning inputs', () => {
  const result = spawnSync(
    process.execPath,
    ['scripts/issue-signaling-ticket.mjs', '--session', 'ses-cli-02'],
    { encoding: 'utf8', env: { ...process.env, SPARTAN_SIGNALING_SECRET: '' } },
  );
