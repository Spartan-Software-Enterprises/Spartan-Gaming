...
export async function createLanDemo({
  frontendPort = 0,
  signalingPort = 0,
  ttlMs = DEFAULT_TTL_MS,
  sessionId = `ses-lan-${randomUUID().slice(0, 8)}`,
  secret = randomBytes(32).toString('base64url'),
  logger = console,
} = {}) {
  if (!/^[A-Za-z0-9._:-]{8,128}$/.test(sessionId))
    throw new TypeError('sessionId has invalid characters');
  ...
  try {
    ...
    const signalingHost =
      signalingAddress.address === '::' ? '127.0.0.1' : signalingAddress.address;
    ...
    const result = Object.freeze({
      ...
      hostTicket: signaling.broker.issueTicket({
        sessionId,
        role: 'host',
        subject: 'lan-browser-host',
        ttlMs,
      }),
      clientTicket: signaling.broker.issueTicket({
        sessionId,
        role: 'client',
        subject: 'lan-browser-client',
        ttlMs,
      }),
      frontend,
      signaling,
    ...
    JSON.stringify(
      {
        ...
        instructions: [
          'Open the host URL in one browser tab, enter the signaling endpoint, session ID, and host ticket, then choose a display and start the host.',
          'Open the player URL in another tab, enter the same signaling endpoint and session ID with the client ticket, then connect securely.',
          'This demo uses localhost WebRTC and user-approved display capture; it does not launch games or inject native input.',
        ],
      },
      null,
      2,
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  createLanDemo({
    frontendPort: Number(process.env.SPARTAN_FRONTEND_PORT || 0),
    signalingPort: Number(process.env.SPARTAN_SIGNALING_PORT || 0),
  })
    .then((demo) => {
