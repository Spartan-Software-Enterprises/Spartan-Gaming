...
export function createHostDeploymentPlan({platform: targetPlatform, nodePath = process.execPath, hostRoot, hostId, hostName = 'Spartan Host', bind = '127.0.0.1', port = 8787, nativePackage, virtualGamepadPackage, virtualGamepadBackend, virtualGamepadDevice, enableInput = false, enableNativeMedia = false, enableNativeAudio = false, tlsKey, tlsCert, allowedOrigins} = {}) {
  ...
  if (nativePackage) args.push('--native-package', required(nativePackage, 'nativePackage')); if (virtualGamepadPackage) args.push('--virtual-gamepad-package', required(virtualGamepadPackage, 'virtualGamepadPackage')); if (virtualGamepadBackend) args.push('--virtual-gamepad-backend', required(virtualGamepadBackend, 'virtualGamepadBackend')); if (virtualGamepadDevice) args.push('--virtual-gamepad-device', required(virtualGamepadDevice, 'virtualGamepadDevice')); if (enableInput) args.push('--enable-input'); if (enableNativeMedia) args.push('--enable-native-media'); if (enableNativeAudio) args.push('--enable-native-audio'); if (tlsKey) args.push('--tls-key', absolute(tlsKey, 'tlsKey'), '--tls-cert', absolute(tlsCert, 'tlsCert')); if (allowedOrigins?.length) args.push('--allowed-origins', allowedOrigins.map(required).join(','));
