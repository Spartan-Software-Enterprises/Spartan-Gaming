...
test('host deployment plans are cross-platform, shell-free, and explicit about opt-in capabilities', () => {
  const plan = createHostDeploymentPlan({platform: 'windows', nodePath: 'C:/Program Files/nodejs/node.exe', hostRoot: 'C:/Spartan/host', hostId: 'living-room', hostName: 'Living Room', nativePackage: '@spartan-gaming/native-windows', virtualGamepadPackage: '@spartan-gaming/virtual-xinput', enableInput: true, port: 9000});
  assert.equal(plan.platform, 'win32'); assert.equal(plan.args.includes('--enable-input'), true); assert.equal(plan.args.includes('--enable-native-media'), false); assert.equal(plan.security.shell, false); assert.equal(plan.security.nativeMediaOptIn, true); assert.equal(plan.persistence.pairing, 'session-only'); assert.equal('signalTicket' in plan, false);
});
