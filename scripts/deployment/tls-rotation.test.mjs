    ...
    der(
      ...
      der(
        ...
        Buffer.concat([
          ...
          der(0x0c, Buffer.from('Spartan TLS test')),
        ]),
      ),
    ),
  );
...
function utc(date) {
  return der(
    0x17,
    Buffer.from(
      date.toISOString().slice(2, 19).replace('T', '').replaceAll('-', '').replaceAll(':', '') +
        'Z',
    ),
  );
}
function certificatePair(now = new Date()) {
  ...
  const tbs = der(
    ...
    Buffer.concat([
      ...
      algorithm(),
      name(),
      der(0x30, Buffer.concat([utc(from), utc(to)])),
      name(),
  ...
  const cert = der(
    0x30,
    Buffer.concat([tbs, algorithm(), der(0x03, Buffer.concat([Buffer.from([0]), signature]))]),
  );
  return {
...
test('TLS certificate inspection validates dates and key matching without exposing key material', () => {
  ...
  const pair = certificatePair(now);
  const result = inspectTlsCertificatePair({ ...pair, now });
  assert.equal(result.subject, 'CN=Spartan TLS test');
  assert.equal(result.issuer, 'CN=Spartan TLS test');
  assert.equal(result.daysRemaining, 30);
  assert.equal('keyPem' in result, false);
  const otherKey = generateKeyPairSync('rsa', { modulusLength: 2048 }).privateKey.export({
    type: 'pkcs8',
    format: 'pem',
  });
  assert.throws(
    () => inspectTlsCertificatePair({ ...pair, keyPem: otherKey, now }),
    /does not match/,
  );
});

test('TLS rotation stages, publishes, and removes old certificate material', async () => {
  const root = await mkdtemp(path.join(tmpdir(), 'spartan-tls-'));
  try {
    const now = new Date('2026-08-10T12:00:00.000Z');
    const pair = certificatePair(now);
    const keyPath = path.join(root, 'server.key');
    const certPath = path.join(root, 'server.crt');
    await writeFile(keyPath, pair.keyPem, { mode: 0o600 });
    await writeFile(certPath, pair.certPem);
    const result = await rotateTlsCertificatePair({
      keyPath,
      certPath,
      ...certificatePair(now),
      now,
    ...
    assert.equal((await readFile(certPath, 'utf8')).includes('CERTIFICATE'), true);
    const files = await (await import('node:fs/promises')).readdir(root);
    assert.deepEqual(files.sort(), ['server.crt', 'server.key']);
  } finally {
