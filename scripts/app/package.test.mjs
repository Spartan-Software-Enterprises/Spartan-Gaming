...
test('app bundle assembles runtime, frontend, emulator runtimes, seed, addon, manifest, and launcher', async () => {
  ...
  try {
    const plan = createAppBundlePlan({
      platform: 'linux',
      sourceRoot: root,
      frontendDir: join(root, 'out/spartan-frontend'),
      seedDir: join(root, 'seed'),
      emulatorDir: join(root, 'vendor/emulators'),
      nativeAddon: join(root, 'native/spartan-native-linux.node'),
      installers: ['portable'],
    });
  ...
  try {
    for (const [platform, expectedSpec] of [
      ...
      const plan = createAppBundlePlan({
        ...
        frontendDir: join(root, 'out/spartan-frontend'),
        seedDir: join(root, 'seed'),
        installers: ['portable', expectedSpec],
      });
      await buildAppBundle(plan);
      const installers = createInstallerPlan(plan);
      assert.deepEqual(
        installers.map((entry) => entry.kind),
        ['portable', expectedSpec],
      );
      const results = await executeInstallerPlan(installers, plan);
...
test('app package CLI requires a known platform and installer', () => {
  ...
  assert.throws(
    () => createAppBundlePlan({ platform: 'plan9', sourceRoot: '/tmp', frontendDir: '/tmp/f' }),
    /unsupported/,
  );
  assert.throws(
    () =>
