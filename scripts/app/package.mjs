...
const INSTALLER_KINDS = Object.freeze({
  linux: ['portable', 'deb'],
  darwin: ['portable', 'dmg-spec'],
  win32: ['portable', 'exe-spec'],
});
...
function platform(value) {
  const selected = PLATFORM_ALIASES[String(value || '').toLowerCase()];
  if (!selected) throw new TypeError(`unsupported app package platform: ${value}`);
  return selected;
}
function commandAvailable(program) {
  const result = spawnSync(program, ['--version'], { stdio: 'ignore', shell: false });
  return !result.error && result.status === 0;
}
function findExecutable(candidates) {
  for (const candidate of candidates) {
    const probes = path.isAbsolute(candidate)
      ? [candidate]
      : (process.env.PATH || '')
          .split(path.delimiter)
          .filter(Boolean)
          .map((directory) => path.join(directory, candidate));
    for (const full of probes) {
      try {
...
function hostPlatform() {
  return PLATFORM_ALIASES[process.platform] || process.platform;
}
function platformVendorPath(kind, target) {
  const root = path.join(repositoryRoot, 'vendor', kind);
  const targeted = path.join(root, PLATFORM_NAMES[target] || target);
  return existsSync(targeted) ? targeted : root;
}
function defaultPackagingInputs(target = hostPlatform()) {
  ...
  const bundledChrome =
    target === 'win32'
      ? path.join(bundledChromium, 'chrome.exe')
      : target === 'darwin'
        ? path.join(bundledChromium, 'Chromium')
        : path.join(bundledChromium, 'chrome');
  const chromiumBinary =
    process.env.SPARTAN_CHROMIUM_BINARY ||
    (existsSync(bundledChrome)
      ? bundledChromium
      : target === hostPlatform()
        ? findExecutable(chromiumNames)
        : null);
  const nodeBinary = target === hostPlatform() ? process.execPath : null;
  return {
    nodeBinary,
    chromiumBinary,
    dependenciesDir: existsSync(path.join(repositoryRoot, 'node_modules'))
...
const LINUX_RUNTIME_DEPENDENCIES = Object.freeze([
  'libnss3',
  'libatk-bridge2.0-0',
  'libdrm2',
  'libxcomposite1',
...
function installerDependencyManifest(plan) {
  const system =
    plan.platform === 'linux'
      ? [...LINUX_RUNTIME_DEPENDENCIES, ...(plan.emulators ? ['wine64', 'playonlinux'] : [])]
      : [];
  return {
    schemaVersion: 1,
    ...
    bundled: [
      'node',
      'chromium',
      ...(plan.dependencies ? ['node_modules'] : []),
      ...(plan.emulators ? ['emulator runtimes'] : []),
    ],
    system,
    updatePolicy:
      'the native package manager installs/upgrades declared dependencies; the bundled helper can refresh them manually; user credentials and game files remain outside the bundle',
  };
}
...
export function createAppBundlePlan({
  ...
  for (const installer of requested)
    if (!known.has(installer)) throw new Error(`unsupported ${target} installer: ${installer}`);
  ...
  const spec = Object.freeze({
    bundle,
    portable: path.join(out, `spartan-gaming-${version}-${target}-portable.tar.gz`),
    deb: path.join(out, `spartan-gaming_${version}_amd64.deb`),
    dmgSpec: path.join(out, 'spartan-gaming.dmg.spec.json'),
    exeSpec: path.join(out, 'spartan-gaming.windows.spec.json'),
    debDir: path.join(out, '.deb-build'),
  });
  return Object.freeze({
    ...
    bundle,
    spec,
    installers: Object.freeze(requested),
  });
}
...
async function statExists(check, kind) {
  let info;
  try {
    info = await stat(check);
  } catch (error) {
    if (error && error.code === 'ENOENT')
      throw new Error(`required input is not available: ${check}`);
    throw error;
  }
  const isDirectory = info.isDirectory();
  if (kind === 'directory' && !isDirectory)
    throw new Error(`required input must be a directory: ${check}`);
  if (kind === 'file' && isDirectory) throw new Error(`required input must be a file: ${check}`);
}

export async function buildAppBundle(plan) {
  for (const check of [
    plan.payload.frontend,
    ...plan.payload.runtime,
    ...(plan.seed ? [plan.seed] : []),
    ...(plan.emulators ? [plan.emulators] : []),
  ])
    await statExists(check, 'directory');
  ...
  for (const relative of RUNTIME_DIRECTORIES)
    await cp(path.join(plan.source, relative), path.join(plan.bundle, relative), {
      recursive: true,
    });
  ...
  if (plan.emulators)
    await cp(plan.emulators, path.join(plan.bundle, 'emulators'), { recursive: true });
  if (plan.addon) {
    await mkdir(path.join(plan.bundle, 'native'), { recursive: true });
    await cp(plan.addon, path.join(plan.bundle, 'native', path.basename(plan.addon)));
  }
  if (plan.node) {
    await mkdir(path.join(plan.bundle, 'runtime'), { recursive: true });
    await cp(
      plan.node,
      path.join(plan.bundle, 'runtime', plan.platform === 'win32' ? 'node.exe' : 'node'),
    );
  }
  if (plan.chromium) {
    await mkdir(path.join(plan.bundle, 'runtime', 'chromium'), { recursive: true });
    const chromiumInfo = await stat(plan.chromium);
    if (chromiumInfo.isDirectory())
      await cp(plan.chromium, path.join(plan.bundle, 'runtime', 'chromium'), { recursive: true });
    else
      await cp(
        plan.chromium,
        path.join(
  ...
  if (plan.dependencies)
    await cp(plan.dependencies, path.join(plan.bundle, 'node_modules'), { recursive: true });
  ...
  await writeFile(path.join(plan.bundle, 'install', 'install-linux.sh'), support.linux, {
    mode: 0o755,
  });
  await writeFile(path.join(plan.bundle, 'install', 'install-macos.sh'), support.mac, {
    mode: 0o755,
  });
  await writeFile(
    path.join(plan.bundle, 'install', 'install-windows.ps1'),
    support.windows,
    'utf8',
  );
  const chromiumEntry =
  ...
  const manifest = Object.freeze({
    ...
    compatibility:
      plan.platform === 'linux' && plan.emulators
        ? { wine: 'system dependency', playOnLinux: 'system dependency' }
        : null,
    installerRoot: 'install',
    dependencyManifest: 'install/dependencies.json',
    seedRoot: plan.seed ? 'seed' : null,
    nativeAddon: plan.addon ? `native/${path.basename(plan.addon)}` : null,
    bundledNode: plan.node ? `runtime/${plan.platform === 'win32' ? 'node.exe' : 'node'}` : null,
    bundledChromium: plan.chromium ? chromiumEntry : null,
    bundledDependencies: plan.dependencies ? 'node_modules' : null,
    defaultHost: '127.0.0.1',
    defaultPort: 4173,
    installers: Object.freeze([...plan.installers]),
  });
  await writeFile(
    path.join(plan.bundle, 'spartan-app.json'),
...
function adapterHomeDefault(platform) {
  return platform === 'win32'
    ? '%LOCALAPPDATA%\\SpartanGaming\\adapters'
    : platform === 'darwin'
      ? '$HOME/Library/Application Support/Spartan Gaming/adapters'
      : '$HOME/.config/spartan-gaming/adapters';
}

function command(command, args, cwd, label) {
  if (!commandAvailable(command))
    throw new Error(`required app installer tool is unavailable: ${command}`);
  const result = spawnSync(command, args, { cwd, stdio: 'inherit', shell: false });
  if (result.error) throw result.error;
  if (result.status !== 0)
    throw new Error(`${command} ${label} exited with status ${result.status}`);
}

export function createPortableInstallerPlan(plan) {
  return Object.freeze({ kind: 'portable', target: plan.spec.portable, cwd: plan.bundle });
}
export function createInstallerPlan(plan) {
  if (plan.platform === 'linux') {
    ...
    const portables = plan.installers.includes('deb')
      ? [
          Object.freeze({ kind: 'portable', target: plan.spec.portable, cwd: plan.bundle }),
          Object.freeze({
            kind: 'deb',
            target: plan.spec.deb,
            root: plan.spec.debDir,
            control,
            data,
            bundle: plan.bundle,
          }),
  ...
  if (plan.platform === 'darwin') {
    const portable = Object.freeze({
      kind: 'portable',
      target: plan.spec.portable,
      cwd: plan.bundle,
    });
    const spec = Object.freeze({
      kind: 'dmg-spec',
      target: plan.spec.dmgSpec,
      bundle: plan.bundle,
      platform: 'darwin',
    });
...
export async function executeInstallerPlan(installers, plan) {
  ...
  for (const installer of installers) {
    if (installer.kind === 'portable') {
      ...
      const dependencies = [
        ...(plan.node ? [] : ['nodejs (>= 20)']),
        ...installerDependencyManifest(plan).system,
      ];
      const control = [
        'Package: spartan-gaming',
        'Version: ' + plan.version,
        'Section: games',
        'Priority: optional',
        'Architecture: amd64',
        'Maintainer: Spartan Software Enterprises <support@example.invalid>',
        ...(dependencies.length ? ['Depends: ' + dependencies.join(', ')] : []),
        'Description: All-in-one cloud gaming, streaming, and emulation desktop client.',
        '',
      ].join('\n');
      ...
      const manifest = JSON.parse(
        await readFile(path.join(plan.bundle, 'spartan-app.json'), 'utf8'),
      );
      const postinst =
      ...
      await writeFile(path.join(path.dirname(installer.control), 'postinst'), postinst, {
        mode: 0o755,
      });
      ...
      command(
        'dpkg-deb',
        ['--build', '--root-owner-group', installer.root, installer.target],
        process.cwd(),
        'deb build',
      );
      ...
      const spec = Object.freeze({
        ...
        dependencyManifest: path.join(installer.bundle, 'install', 'dependencies.json'),
        installHelper:
          installer.platform === 'darwin'
            ? path.join(installer.bundle, 'install', 'install-macos.sh')
            : path.join(installer.bundle, 'install', 'install-windows.ps1'),
        note: 'Run the platform installer on its native OS; the bundled helper verifies or installs declared prerequisites before first launch.',
      });
...
function print(plan, json) {
  ...
  console.log(`frontend: ${plan.frontend}`);
  console.log(`seed: ${plan.seed || 'none'}`);
  console.log(`native addon: ${plan.addon || 'none'}`);
  console.log(`bundle: ${plan.bundle}`);
  for (const installer of plan.installers) console.log(`installer: ${installer}`);
}
...
if (path.resolve(fileURLToPath(import.meta.url)) === path.resolve(process.argv[1] || '')) {
  (async () => {
    try {
      ...
      if (options.help) {
        console.log(
          'Usage: npm run app:package -- --platform linux|windows|macos [--frontend-dir PATH] [--seed-dir PATH] [--emulator-dir PATH] [--native-addon PATH] [--node-binary PATH] [--chromium-binary PATH] [--dependencies-dir PATH] [--version VERSION] [--installer portable|deb|dmg-spec|exe-spec] [--execute]',
        );
        process.exit(0);
      }
      ...
      const emulatorDir =
        options.emulatorDir ||
        (existsSync(targetEmulators)
          ? targetEmulators
          : target === hostPlatform() && existsSync(sharedEmulators)
            ? sharedEmulators
            : null);
      const plan = createAppBundlePlan({
        platform: target,
        ...
        nodeBinary: options.nodeBinary || defaults.nodeBinary,
        chromiumBinary: options.chromiumBinary || defaults.chromiumBinary,
        dependenciesDir: options.dependenciesDir || defaults.dependenciesDir,
        version: options.version,
        installers: options.installers,
      });
      if (options.execute && (!plan.node || !plan.chromium))
        throw new Error(
          `a runnable ${target} release requires target-platform Node and Chromium binaries; build on ${target} or pass --node-binary and --chromium-binary explicitly`,
        );
      print(plan, options.json);
      const summary = await buildAppBundle(plan);
      const installers = options.execute
        ? await executeInstallerPlan(createInstallerPlan(plan), plan)
        : createInstallerPlan(plan);
      console.log(
        JSON.stringify(
          {
            service: 'spartan-app-package',
            platform: plan.platform,
            bundle: summary.bundle,
            files: summary.files,
            installers: installers.map((entry) => entry.target),
          },
          null,
          2,
        ),
