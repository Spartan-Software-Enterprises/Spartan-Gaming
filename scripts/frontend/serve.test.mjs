...
test('frontend server redirects the origin to the dashboard and serves catalogs', () =>
  withServer(async (origin) => {
    ...
    assert.deepEqual(manifestJson.display_override, [
    ...
    assert.deepEqual(
      manifestJson.shortcuts.map((shortcut) => shortcut.url),
      ['../dashboard/', '../player/', '../input/profiles.html', '../diagnostics/'],
    );
    assert.deepEqual(manifestJson.icons, [
      { src: './spartan-mark.svg', sizes: 'any', type: 'image/svg+xml', purpose: 'any maskable' },
    ]);
...
test('frontend server exposes the service worker aliases and rejects traversal', () =>
  withServer(async (origin) => {
...
test('frontend server injects an app context script into served pages', async () => {
  const frontend = createFrontendServer({
    ...
    contextScript: {
      ...
      render: () => 'window.__SPARTAN_RELEASE_FEED__ = {schemaVersion: 1};',
    },
  });
  const address = await frontend.listen();
  try {
