...
export async function buildFrontendDistribution({outputRoot = defaultOutputRoot, source = sourceRoot, publicRoot = repositoryRoot} = {}) {
  ...
  const manifest = Object.freeze({schemaVersion: 1, product: 'Spartan Gaming', artifact: 'static-frontend', entrypoints: ENTRYPOINTS, publicCatalogs: Object.freeze(['/providers/catalog.json', '/emulators/catalog.json']), serviceWorker: '/service-worker.mjs'});
  await writeFile(path.join(output, 'spartan-frontend-manifest.json'), `${JSON.stringify(manifest, null, 2)}\n`, 'utf8');
  const required = [...Object.values(ENTRYPOINTS).map(entry => entry.slice(1, -1) + '/index.html'), 'service-worker.mjs', 'providers/catalog.json', 'emulators/catalog.json', 'spartan-frontend-manifest.json'];
