...
export async function buildFrontendDistribution({outputRoot = defaultOutputRoot, source = sourceRoot, publicRoot = repositoryRoot} = {}) {
  ...
  const manifest = Object.freeze({schemaVersion: 1, product: 'Spartan Gaming', artifact: 'static-frontend', entrypoints: ENTRYPOINTS, publicCatalogs: Object.freeze(['/providers/catalog.json', '/emulators/catalog.json', '/games/catalog.json']), serviceWorker: '/service-worker.mjs'});
  await writeFile(path.join(output, 'spartan-frontend-manifest.json'), `${JSON.stringify(manifest, null, 2)}\n`, 'utf8');
  const required = [...Object.values(ENTRYPOINTS).map(entry => entry.slice(1, -1) + '/index.html'), 'service-worker.mjs', 'providers/catalog.json', 'emulators/catalog.json', 'games/catalog.json', 'spartan-frontend-manifest.json'];
  for (const relative of required) { try { await readFile(path.join(output, relative)); } catch { throw new Error(`frontend distribution is missing ${relative}`); } }
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  buildFrontendDistribution({outputRoot: argument('out', defaultOutputRoot)}).then(result => console.log(JSON.stringify({service: 'spartan-frontend-build', output: result.output, files: result.files, manifest: path.join(result.output, 'spartan-frontend-manifest.json')}))).catch(error => { console.error(error.message); process.exitCode = 1; });
}
