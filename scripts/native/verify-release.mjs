...
export async function verifyReleaseManifest({
  ...
  const key = typeof publicKeyJwk === 'string' ? JSON.parse(publicKeyJwk) : publicKeyJwk;
  if (!key || typeof key !== 'object' || Array.isArray(key))
    throw new TypeError('public key JWK must be an object');
  const manifest = JSON.parse(await fsImpl.readFile(source, 'utf8'));
  if (!(await verifyPackageManifestSignature({ manifest, publicKeyJwk: key, subtle })))
...
if (path.resolve(fileURLToPath(import.meta.url)) === path.resolve(process.argv[1] || '')) {
  try {
    ...
    const result = await verifyReleaseManifest({
      ...
      publicKeyJwk: process.env[keyEnv],
    });
