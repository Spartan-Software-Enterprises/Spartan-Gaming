...
import '../roadmap/acceptance.test.mjs';
import {
  createNativePackageMatrix,
  createNativePackagePlan,
  parseNativePackageArguments,
} from './package.mjs';

const repositoryRoot = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '../..');
const hardwareWorkflow = fs.readFileSync(
  path.join(repositoryRoot, '.github/workflows/hardware-validation.yml'),
  'utf8',
);

test('native package CLI maps all desktop targets to isolated plans', () => {
  const matrix = createNativePackageMatrix({
    sourceRoot: '/external/spartan',
    outRoot: '/external/out',
    installRoot: '/external/install',
  });
  assert.deepEqual(
  ...
  assert.deepEqual(
    ...
    ['win32', 'darwin', 'linux'].map((target) => path.resolve('/external/out', target)),
  );
...
test('native package CLI accepts platform aliases and preserves a plan-only default', () => {
  ...
  assert.equal(
    parseNativePackageArguments(['--matrix', '--json', '--configuration', 'Debug']).configuration,
    'Debug',
  );
});
...
test('desktop native package sources expose platform API implementations and package entrypoints', () => {
  ...
  for (const [target, sourceFile, marker] of targets) {
    ...
    assert.ok(
      ...
      `${target} CMakeLists.txt is required`,
    );
    assert.ok(
      fs.existsSync(path.join(directory, 'index.mjs')),
      `${target} package entrypoint is required`,
    );
    assert.ok(
      fs.existsSync(path.join(directory, 'package.json')),
      `${target} package manifest is required`,
    );
...
test('hardware validation workflow requires real platform capabilities and signed adapters', () => {
  ...
  assert.match(hardwareWorkflow, /execute_hardware:/);
  assert.match(hardwareWorkflow, /Exercise native capture audio input and haptics/);
  assert.match(hardwareWorkflow, /--execute --confirm --require-execution/);
