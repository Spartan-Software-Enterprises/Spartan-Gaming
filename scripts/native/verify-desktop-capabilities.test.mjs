...
test('desktop capability verification is observation-only and reports a ready injected package', async () => {
  ...
  assert.equal(report.kind, 'native-hardware-report'); assert.equal(report.verification, 'capability-observation'); assert.equal(report.status, 'ready'); assert.equal(report.package.state, 'ready'); assert.equal(report.virtualGamepad.state, 'external-driver-required'); assert.equal(calls.at(-1), 'closed');
...
test('desktop capability verification can explicitly exercise capture, audio, input, and haptics', async () => {
  ...
  assert.equal(report.verification, 'runtime-exercise'); assert.equal(report.status, 'ready'); assert.deepEqual(report.execution, {state: 'ready', capture: 'verified', audio: 'verified', input: 'verified', haptics: 'verified'});
