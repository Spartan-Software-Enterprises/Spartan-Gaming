...
test('release publisher verifies the complete signed manifest through WebCrypto', async () => { const calls = []; const result = await verifyReleaseManifest({manifestPath: '/external/package-manifest.signed.json', publicKeyJwk: {kty: 'EC', crv: 'P-256'}, fsImpl: {async readFile() { return JSON.stringify(manifest); }}, subtle: {async importKey(...args) { calls.push(['importKey', ...args]); return 'key'; }, async verify(...args) { calls.push(['verify', ...args]); return true; }}}); assert.equal(result.kind, 'signed-release-manifest'); assert.equal(result.verification, 'webcrypto'); assert.equal(result.status, 'verified'); assert.equal(result.platform, 'linux'); assert.equal(calls[0][0], 'importKey'); assert.equal(calls[1][0], 'verify'); });
