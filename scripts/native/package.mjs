...
import {
  ...
  loadNativePackageManifest,
  normalizeNativePackageManifest,
} from '../../host/native-package.mjs';
...
function platform(value) {
  ...
  if (!selected) throw new TypeError(`unsupported native package platform: ${value}`);
  return selected;
}
...
export function createNativePackagePlan({
  ...
  const buildDefinitionPresent =
    sourceDirectoryPresent && fs.existsSync(path.join(plan.source, 'CMakeLists.txt'));
  return Object.freeze({
    ...plan,
    sourcePresent: buildDefinitionPresent,
    sourceDirectoryPresent,
    platform: target,
  });
}

export function createNativePackageMatrix({
  manifest = loadNativePackageManifest(),
  sourceRoot = repositoryRoot,
  outRoot,
  installRoot,
  ...
  return Object.freeze(
    normalized.packages.map((entry) =>
      createNativePackagePlan({
        manifest: normalized,
        platform: entry.platform,
        sourceRoot,
        outRoot: outRoot ? path.join(outRoot, entry.platform) : undefined,
        installRoot: installRoot ? path.join(installRoot, entry.platform) : undefined,
        configuration,
      }),
    ),
  );
...
export function executeNativePackagePlan(plan) {
  if (!plan.sourcePresent)
    throw new Error(`native package CMake source is not ready: ${plan.source}`);
  for (const command of plan.commands) {
    if (!commandAvailable(command.program))
      throw new Error(`required native package tool is unavailable: ${command.program}`);
...
function print(plan, json) {
  ...
  console.log(`package: ${plan.package.packageName}`);
  console.log(`source: ${plan.source} (${plan.sourcePresent ? 'present' : 'not present'})`);
...
if (path.resolve(fileURLToPath(import.meta.url)) === path.resolve(process.argv[1] || '')) {
  try {
    ...
    if (options.help) {
      console.log(
        'Usage: npm run native:plan -- --platform windows|macos|linux [--source-root PATH] [--out-root PATH] [--install-root PATH] [--configuration Release] [--execute] [--json]',
      );
      process.exit(0);
    }
    const plans = options.matrix
      ? createNativePackageMatrix({
      ...
      : [
          createNativePackagePlan({
            platform:
              options.platform ||
              { win32: 'win32', darwin: 'darwin', linux: 'linux' }[process.platform] ||
              'linux',
            sourceRoot: options.sourceRoot,
            outRoot: options.outRoot,
            installRoot: options.installRoot,
    ...
    if (options.json) console.log(JSON.stringify(options.matrix ? plans : plans[0], null, 2));
    else plans.forEach((plan) => print(plan, false));
