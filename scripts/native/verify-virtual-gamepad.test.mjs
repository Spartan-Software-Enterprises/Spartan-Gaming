...
test('virtual-gamepad verifier loads only a verified platform-matched package and never executes input', async () => {
  ...
  assert.equal(report.status, 'ready'); assert.equal(report.kind, 'virtual-gamepad-exercise'); assert.equal(report.verification, 'signed-runtime-observation'); assert.equal(report.platform, 'win32'); assert.equal(report.adapterKind, 'virtual-gamepad'); assert.equal(report.capabilities.execute, true); assert.equal(closed, 1);
});
...
test('virtual-gamepad verifier bounds live driver identity metadata', async () => {
  const report = await verifyInstalledVirtualGamepad({platform: 'windows', installRoot: '/opt/adapters', id: 'windows-driver', publicKeyJwk: key, requireDriver: true, verifyManifest: async () => true, loadModule: async () => ({createVirtualGamepad: async () => ({platform: 'win32', verifyDriver: async () => ({state: 'ready', name: 'x'.repeat(129)}), execute() {}, close() {}})}), fsImpl: {readFile: asyncFiles()}});
  assert.equal(report.status, 'unavailable'); assert.match(report.reason, /name is too long/);
});
