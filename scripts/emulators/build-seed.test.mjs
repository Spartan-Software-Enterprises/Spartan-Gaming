...
async function makeFixture(root) {
  ...
  const unsigned = normalizeReleaseFeed({schemaVersion: 1, updatedAt: '2026-08-09', signer: 'spartan-release', records: [{id: 'dolphin', version: '1.0.0', kind: 'emulator', status: 'available', trust: 'signed', platforms: ['linux'], capabilities: ['video', 'audio'], license: 'GPL-2.0-or-later', integrity: digest(artifact), signature: {algorithm: 'ECDSA-P256-SHA256', signer: 'spartan-release', value: 'adapter-signature'}, artifact: {url: 'https://releases.example.test/dolphin-1.0.0-linux.tar', sizeBytes: artifact.byteLength, integrity: digest(artifact)}}]});
...
test('seed build fails closed on tampered, missing, or unreferenced artifacts', async () => {
  ...
  try {
    const fixture = await makeFixture(root);
    await fs.writeFile(join(fixture.artifacts, 'dolphin-1.0.0-linux.tar'), new TextEncoder().encode('tampered'));
    await assert.rejects(() => buildSeedBundle({...fixture, outPath: join(root, 'out-a')}), /size mismatch|digest mismatch/);
    await fs.unlink(join(fixture.artifacts, 'dolphin-1.0.0-linux.tar'));
    await assert.rejects(() => buildSeedBundle({...fixture, outPath: join(root, 'out-b')}), /missing bundled artifact/);
    await fs.writeFile(join(fixture.artifacts, 'dolphin-1.0.0-linux.tar'), new TextEncoder().encode('dolphin-payload'));
    await fs.writeFile(join(fixture.artifacts, 'unreferenced.bin'), new TextEncoder().encode('extra'));
    await assert.rejects(() => buildSeedBundle({...fixture, outPath: join(root, 'out-c')}), /unreferenced artifact/);
  } finally { await fs.rm(root, {recursive: true, force: true}); }
});

test('seed build parses required CLI flags', () => {
  const options = parseBuildSeedArgs(['--feed', 'a.json', '--key', 'b.json', '--artifacts', 'c', '--out', 'd']);
  ...
  assert.throws(() => parseBuildSeedArgs(['--feed', 'a.json']), /--key, --artifacts, and --out are required/);
});
