...
async function makeFeed({record, privateKey, signer = 'spartan-release'}) {
  const unsigned = normalizeReleaseFeed({schemaVersion: 1, updatedAt: '2026-08-09', signer, records: [record]});
  const signed = await signPackageManifest({manifest: unsigned, privateKey, signer});
  return normalizeReleaseFeed(signed);
}

test('emulator updater parses CLI flags and plans a verified release feed', async () => { const options = parseUpdateArgs(['--feed', 'https://releases.example.test/feed.json', '--key', '/tmp/key.json', '--install-root', '/tmp/adapters', '--platform', 'linux']); assert.equal(options.platform, 'linux'); assert.equal(options.apply, false); assert.throws(() => parseUpdateArgs(['--feed', 'x']), /--key/); });
...
test('emulator updater installs, verifies, and updates a packaged emulator release', async () => {
  ...
  const unsignedPackage = {id: 'dolphin', version: '1.0.0', kind: 'emulator', platform: 'linux', format: 'tar', entrypoint: 'bin/dolphin', files: [{path: 'bin', type: 'directory', sizeBytes: 0}, {path: 'bin/dolphin', sizeBytes: binary.length, integrity: entryIntegrity}]};
  const hostNormalized = normalizeAdapterPackageManifest({...unsignedPackage, signature: {algorithm: 'ECDSA-P256-SHA256', signer: 'spartan-release', value: 'pending'}});
  ...
  try {
    ...
    const planned = await runEmulatorUpdate({feed, publicKeyJwk, installRoot: root, platform: 'linux', apply: false});
    assert.equal(planned.installs.length, 1); assert.equal(planned.installs[0].status, 'install-available');
    const applied = await runEmulatorUpdate({feed, publicKeyJwk, installRoot: root, platform: 'linux', apply: true, download: async () => archive});
    assert.equal(applied.applied[0].status, 'installed');
    assert.deepEqual(Buffer.from(await fs.readFile(join(root, 'dolphin', '1.0.0', 'bin', 'dolphin'))), Buffer.from(binary));
    const refreshed = await runEmulatorUpdate({feed, publicKeyJwk, installRoot: root, platform: 'linux', apply: false});
