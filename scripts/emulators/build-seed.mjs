...
export function parseBuildSeedArgs(argv = process.argv.slice(2)) {
  ...
  if (!options.feed || !options.key || !options.artifacts || !options.out)
    throw new TypeError('--feed, --key, --artifacts, and --out are required');
  return options;
}

function decodeDigest(value) {
  if (!String(value).startsWith('sha256-'))
...
function assertArtifactName(name) {
  if (!name || name === '.' || name === '..' || name.includes('/') || name.includes('\\'))
    throw new TypeError(`unsafe artifact file name: ${name}`);
  return name;
}
...
export async function buildSeedBundle({ feedPath, keyPath, artifacts, outPath } = {}) {
  ...
  if (!(await verifyReleaseFeed({ feed, publicKeyJwk })))
  ...
  if (!Array.isArray(records) || records.length === 0)
    throw new Error('release feed must contain records to bundle');
  const copied = [];
  const used = new Set();
  for (const record of records) {
    if (!record.artifact?.url)
      throw new TypeError(`release record ${record.id} has no artifact descriptor`);
    ...
    if (bundled.get(name) !== record.artifact.sizeBytes)
      throw new Error(`bundled artifact size mismatch for ${record.id}: ${name}`);
    used.add(name);
    copied.push({ id: record.id, version: record.version, name });
  }
  for (const name of bundled.keys())
    if (!used.has(name)) throw new Error(`unreferenced artifact must not be bundled: ${name}`);
  ...
  for (const item of copied) {
    ...
    if (
      bytes.byteLength !== record.artifact.sizeBytes ||
      digest !== decodeDigest(record.artifact.integrity)
    )
      throw new Error(`bundled artifact digest mismatch for ${item.id}`);
...
export async function main(argv = process.argv.slice(2)) {
  ...
  if (options.help) {
    console.log(
      'Usage: node scripts/emulators/build-seed.mjs --feed <feed.json> --key <public-jwk.json> --artifacts <dir> --out <seed-dir>',
    );
    return;
  }
  const summary = await buildSeedBundle({
