...
export function parseUpdateArgs(argv = process.argv.slice(2)) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (flag === '--feed') options.feed = argv[++index];
      ...
      options.platform = PLATFORM_ALIASES[argv[++index]] || process.platform;
    else if (flag === '--apply') options.apply = true;
    else if (flag === '--help') options.help = true;
    else throw new TypeError(`unknown emulator update flag: ${flag}`);
  }
  if (!options.feed || !options.key || !options.installRoot)
    throw new TypeError('--feed, --key, and --install-root are required');
  return options;
}
...
async function readInstalledRecords(installRoot, platform) {
  ...
  for (const entry of entries) {
    ...
    let pointer;
    try {
      pointer = JSON.parse(await readFile(currentPath, 'utf8'));
    } catch {
      continue;
    }
    ...
    records.push({
      ...
      status: 'installed',
      trust: 'signed',
      platforms: [manifest.platform === 'universal' ? platform : manifest.platform],
      capabilities: ['video', 'audio'],
      license: 'project-provided',
      integrity: pointer.integrity || `sha256-${manifest.id}`,
...
export async function runEmulatorUpdate({
  feedSource,
  keyPath,
  installRoot,
  platform = process.platform,
  ...
  if (!platform || !['win32', 'darwin', 'linux'].includes(platform))
    throw new TypeError(`unsupported emulator update platform: ${platform}`);
  const trustedKey = publicKeyJwk || (await loadKey(keyPath));
  const resolvedFeed = feed || (await loadSource(feedSource));
  if (!(await verifyReleaseFeed({ feed: resolvedFeed, publicKeyJwk: trustedKey })))
    throw new Error('release feed signature verification failed');
  const installed = await readInstalledRecords(installRoot, platform);
  const planner = createReleaseFeedPlanner({
    feed: resolvedFeed,
    platform,
    kind: 'emulator',
    installed,
  });
  const installs = planner.plans.filter(
    (plan) => plan.status === 'install-available' || plan.status === 'update-available',
  );
  const results = [];
  if (apply && installs.length) {
    await mkdir(installRoot, { recursive: true });
    const verifier = async (args) => {
      const source = args.manifest || args.request?.package;
      if (!source) throw new Error('packaged emulator release is required');
      const manifest = normalizeAdapterPackageManifest(source);
      return verifyPackageManifestSignature({ manifest, publicKeyJwk: trustedKey });
    };
    const installer = createNativeAdapterInstaller({
      installRoot,
      download,
      verifySignature: verifier,
      verifyPackageManifest: verifier,
    });
...
export async function main(argv = process.argv.slice(2)) {
  ...
  if (options.help) {
    console.log(
      'Usage: node scripts/emulators/update.mjs --feed <url|path> --key <public-jwk.json> --install-root <path> [--platform linux|win32|darwin] [--apply]',
    );
    return;
  }
  const summary = await runEmulatorUpdate({
