...
import {chromiumManifestPath, loadChromiumManifest, validateChromiumManifest} from '../../chromium/config.mjs';
...
export function createChromiumBootstrapPlan({manifest = loadChromiumManifest(chromiumManifestPath), platform, checkout, root = repositoryRoot} = {}) {
  ...
  const commands = [];
  if (!fs.existsSync(source)) commands.push(Object.freeze({program: 'fetch', args: Object.freeze(['--nohooks', 'chromium']), cwd: checkoutRoot}));
  commands.push(Object.freeze({program: 'git', args: Object.freeze(['-C', source, 'fetch', 'origin', branch]), cwd: checkoutRoot}));
  commands.push(Object.freeze({program: 'git', args: Object.freeze(['-C', source, 'checkout', '--detach', 'FETCH_HEAD']), cwd: checkoutRoot}));
  commands.push(Object.freeze({program: 'gclient', args: Object.freeze(['sync', '--nohooks']), cwd: checkoutRoot}));
  commands.push(Object.freeze({program: 'gclient', args: Object.freeze(['runhooks']), cwd: checkoutRoot}));
  return Object.freeze({platform: selectedPlatform, checkout: checkoutRoot, source, branch, manifest: path.relative(root, chromiumManifestPath), commands: Object.freeze(commands)});
}

function commandAvailable(program) {
  const result = spawnSync(program, program === 'fetch' ? ['--help'] : ['--version'], {stdio: 'ignore', shell: process.platform === 'win32'});
...
export function executeChromiumBootstrap(plan) {
  ...
  for (const command of plan.commands) {
    if (!commandAvailable(command.program)) throw new Error(`required Chromium checkout tool is unavailable: ${command.program}`);
...
function printPlan(plan, json) {
  ...
  console.log(`checkout: ${plan.checkout}`); console.log(`source: ${plan.source}`); console.log(`ref: ${plan.branch}`);
...
if (path.resolve(fileURLToPath(import.meta.url)) === path.resolve(process.argv[1] || '')) {
  try {
    ...
    if (options.help) { console.log('Usage: npm run chromium:bootstrap -- --platform linux|mac|windows --checkout PATH [--execute] [--json]'); process.exit(0); }
    const platform = options.platform || ({linux: 'linux', darwin: 'mac', win32: 'windows'}[process.platform] || 'linux');
