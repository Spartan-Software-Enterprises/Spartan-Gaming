...
test('Chromium shell plans resolve platform binaries and safe app URLs', () => {
  assert.equal(
    chromiumBinaryPath({ platform: 'linux', out: '/external/out' }),
    path.join(path.resolve('/external/out'), 'chrome'),
  );
  assert.equal(
    chromiumBinaryPath({ platform: 'mac', out: '/external/out' }),
    path.join(path.resolve('/external/out'), 'Chromium.app', 'Contents', 'MacOS', 'Chromium'),
  );
  assert.equal(
    chromiumBinaryPath({ platform: 'windows', out: '/external/out' }),
    path.join(path.resolve('/external/out'), 'chrome.exe'),
  );
  const plan = createChromiumShellPlan({
    platform: 'linux',
    binary: '/usr/local/bin/chromium',
  ...
  assert.deepEqual(plan.args, [
    ...
    `--user-data-dir=${path.resolve('/external/profile')}`,
  ]);
...
test('Chromium shell plans reject unsafe URLs and repository-owned paths', () => {
  ...
  assert.throws(
    () => createChromiumShellPlan({ platform: 'linux', binary: process.cwd() }),
    /repository/,
  );
  assert.throws(
    () => createChromiumShellPlan({ platform: 'linux', out: process.cwd() }),
    /repository/,
  );
});
...
test('Chromium shell can orchestrate a local HTTP frontend without shell execution', async () => {
  ...
  const running = await launchChromiumShell({
    plan: createChromiumShellPlan({ platform: 'linux', binary: process.execPath }),
    serve: true,
