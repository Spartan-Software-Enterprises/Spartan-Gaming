...
test('Chromium build options default to a plan and preserve explicit paths', () => {
  assert.deepEqual(parseBuildArguments(['--platform', 'mac', '--source', '/tmp/chromium', '--out', 'out/custom', '--target', 'chrome', '--json']), {platform: 'mac', source: '/tmp/chromium', out: 'out/custom', target: 'chrome', execute: false, json: true, matrix: false});
});
