...
export function parseBuildArguments(argv = []) {
  const options = {
    platform: null,
    source: process.env.SPARTAN_CHROMIUM_SRC || '',
    out: '',
    target: '',
    execute: false,
    json: false,
...
function currentPlatform() {
  return { linux: 'linux', darwin: 'mac', win32: 'windows' }[process.platform] || 'linux';
}

export function createChromiumBuildPlan({
  manifest = loadChromiumManifest(manifestPath),
  ...
  const outputDirectory = path.resolve(
    sourceDirectory,
    text(out) || `out/SpartanGaming-${selectedPlatform}`,
  );
  if (outputDirectory === sourceDirectory)
    throw new Error('build output must not equal the Chromium source directory');
  if (
    outputDirectory === repositoryRoot ||
    outputDirectory.startsWith(`${repositoryRoot}${path.sep}`)
  )
    throw new Error('build output must remain outside the Spartan Gaming repository');
  ...
  return Object.freeze({
    ...
    commands: Object.freeze([
      Object.freeze({
        program: 'gn',
        args: Object.freeze(['gen', outputDirectory, `--args=${argsText}`]),
        cwd: sourceDirectory,
      }),
      Object.freeze({
        program: 'autoninja',
        args: Object.freeze(['-C', outputDirectory, buildTarget]),
        cwd: sourceDirectory,
      }),
    ]),
  });
...
export function createChromiumDevelopmentBuildMatrix({
  manifest = loadChromiumManifest(manifestPath),
  source,
  outRoot,
  root = repositoryRoot,
  ...
  validateChromiumManifest(manifest);
  const sourceDirectory = path.resolve(required(source, 'source'));
  const outputRoot = text(outRoot)
    ? path.resolve(outRoot)
    : path.join(sourceDirectory, 'out', 'SpartanGaming');
  if (outputRoot === repositoryRoot || outputRoot.startsWith(`${repositoryRoot}${path.sep}`))
    throw new Error('matrix output must remain outside the Spartan Gaming repository');
  return Object.freeze(
    ['linux', 'mac', 'windows'].map((platform) =>
      createChromiumBuildPlan({
        ...
        platform,
        source: sourceDirectory,
        out: path.join(outputRoot, platform),
        root,
      }),
    ),
  );
...
export function executeChromiumBuild(plan) {
  if (!fs.existsSync(plan.source))
    throw new Error(`Chromium source directory does not exist: ${plan.source}`);
  for (const command of plan.commands) {
    if (!commandAvailable(command.program))
      throw new Error(`required Chromium build tool is unavailable: ${command.program}`);
...
if (path.resolve(fileURLToPath(import.meta.url)) === path.resolve(process.argv[1] || '')) {
  try {
    ...
    if (options.help) {
      console.log(
        'Usage: npm run chromium:build -- [--platform linux|mac|windows] [--matrix] [--source PATH] [--out PATH] [--target chrome] [--execute] [--json]',
      );
      process.exit(0);
    }
    const platform = options.platform || currentPlatform();
    if (options.matrix) {
      const plans = createChromiumDevelopmentBuildMatrix({
        source: options.source,
        outRoot: options.out,
      });
      if (options.json) console.log(JSON.stringify(plans, null, 2));
      else plans.forEach(printPlan);
      if (options.execute) plans.forEach(executeChromiumBuild);
    } else {
