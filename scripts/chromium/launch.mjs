...
function externalPath(value, name) {
  ...
  if (result === repositoryRoot || result.startsWith(`${repositoryRoot}${path.sep}`)) throw new Error(`${name} must remain outside the Spartan Gaming repository`);
  return result;
}
function platformFor(value) { const platform = text(value) || ({linux: 'linux', darwin: 'mac', win32: 'windows'}[process.platform] || 'linux'); if (!supportedPlatforms.has(platform)) throw new Error(`unsupported Chromium platform: ${platform}`); return platform; }
function safeUrl(value) {
  const parsed = new URL(required(value, 'url'));
  if (parsed.protocol === 'https:') return parsed.href;
  if (parsed.protocol !== 'http:' || !['localhost', '127.0.0.1', '[::1]'].includes(parsed.hostname)) throw new Error('Chromium shell URLs must use HTTPS or loopback HTTP');
  return parsed.href;
}

export function parseLaunchArguments(argv = []) {
  const options = {platform: null, binary: '', out: '', url: '', frontendRoot: '', userDataDir: '', serve: false, execute: false, json: false};
...
export function createChromiumShellPlan({manifest = loadChromiumManifest(), platform, out, binary, url = 'http://127.0.0.1:4173/dashboard/', userDataDir = ''} = {}) {
  ...
  return Object.freeze({platform: selectedPlatform, binary: binaryPath, url: safeUrl(url), artifact: descriptor.artifact, args: Object.freeze(args), command: Object.freeze({program: binaryPath, args: Object.freeze(args), cwd: path.dirname(binaryPath), shell: false})});
}
...
export function executeChromiumShellPlan(plan, {spawnImpl = spawn} = {}) {
  ...
  if (!fs.existsSync(plan.command.program)) throw new Error(`Chromium binary does not exist: ${plan.command.program}`);
  return spawnImpl(plan.command.program, plan.command.args, {cwd: plan.command.cwd, shell: false, stdio: 'inherit'});
}

export async function launchChromiumShell({plan, serve = false, frontendRoot, publicRoot, spawnImpl = spawn} = {}) {
  let frontend;
  let activePlan = plan;
  if (serve) {
  ...
  try {
    ...
    return Object.freeze({plan: activePlan, child, async close() { child.kill?.(); await frontend?.close(); }});
  } catch (error) {
...
if (path.resolve(fileURLToPath(import.meta.url)) === path.resolve(process.argv[1] || '')) {
  try {
    ...
    if (options.help) { console.log('Usage: npm run chromium:launch -- --binary PATH|--out PATH [--platform linux|mac|windows] [--url URL] [--serve] [--frontend-root PATH] [--user-data-dir PATH] [--execute] [--json]'); process.exit(0); }
    const plan = createChromiumShellPlan({platform: options.platform, binary: options.binary, out: options.out, url: options.url || undefined, userDataDir: options.userDataDir});
    if (options.json || !options.execute) console.log(JSON.stringify(plan, null, 2));
    if (options.execute) {
      const running = await launchChromiumShell({plan, serve: options.serve, frontendRoot: options.frontendRoot, publicRoot: options.frontendRoot, spawnImpl: spawn});
      console.log(JSON.stringify({service: 'spartan-chromium-shell', platform: running.plan.platform, url: running.plan.url, binary: running.plan.binary}));
      running.child.once?.('exit', () => { void running.close(); });
    }
