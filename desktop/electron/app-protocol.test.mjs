...
import {APP_ORIGIN, APP_PROTOCOL_PRIVILEGES, createBundledAppProtocolHandler, resolveBundledAssetCandidates} from './app-protocol.mjs';
...
test('private app protocol exposes only bounded packaged assets', () => {
  ...
  assert.match(resolveBundledAssetCandidates(`${APP_ORIGIN}/dashboard/`, roots)[0], /src\/frontend\/dashboard$/);
  assert.match(resolveBundledAssetCandidates(`${APP_ORIGIN}/providers/catalog.json`, roots)[0], /providers\/catalog\.json$/);
...
test('private app protocol serves the offline dashboard and local catalogs', async () => {
  ...
  assert.equal((await handle(new Request(`${APP_ORIGIN}/dashboard/`, {method: 'POST'}))).status, 405);
