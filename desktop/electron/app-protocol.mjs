...
const FRONTEND_SECTIONS = new Set([
  'adapters',
  'dashboard',
  'diagnostics',
  'emulation',
...
export function resolveBundledAssetCandidates(rawUrl, { frontendRoot, publicRoot } = {}) {
  ...
  if (pathname === '/service-worker.mjs')
    return Object.freeze([path.join(frontendRoot, 'service-worker.mjs')]);
  if (pathname.startsWith('/pwa/'))
    return Object.freeze(
      [safePath(path.join(frontendRoot, 'pwa'), pathname.slice('/pwa'.length))].filter(Boolean),
    );
  for (const mount of ['providers', 'emulators', 'games']) {
    if (pathname.startsWith(`/${mount}/`) && pathname.endsWith('.json'))
      return Object.freeze(
        [safePath(path.join(publicRoot, mount), pathname.slice(mount.length + 1))].filter(Boolean),
      );
  }
  if (pathname.startsWith('/src/frontend/'))
    return Object.freeze(
      [safePath(frontendRoot, pathname.slice('/src/frontend'.length))].filter(Boolean),
    );
  if (pathname === '/favicon.ico') return Object.freeze([path.join(publicRoot, 'favicon.ico')]);
  if (/\.(?:mjs|js)$/.test(pathname))
    return Object.freeze(
      [safePath(frontendRoot, pathname), safePath(publicRoot, pathname)].filter(Boolean),
    );
  const firstSegment = pathname.split('/')[1];
  if (FRONTEND_SECTIONS.has(firstSegment))
    return Object.freeze([safePath(frontendRoot, pathname)].filter(Boolean));
  return Object.freeze([]);
}
...
export function createBundledAppProtocolHandler({
  frontendRoot,
  publicRoot,
  ...
  return async (request) => {
    ...
    const headers = {
      ...
      'content-type':
        MIME_TYPES[path.extname(resolved.file).toLowerCase()] || 'application/octet-stream',
    };
    return new Response(request.method === 'HEAD' ? null : await readFileImpl(resolved.file), {
      status: 200,
      headers,
    });
  };
