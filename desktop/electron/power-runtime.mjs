...
export function normalizePowerRuntimeState({active = false, powerMode = 'Balanced', onBattery = false, thermalState = 'unknown', speedLimit = 100} = {}) {
  ...
  return Object.freeze({active: active === true, powerMode: mode, onBattery: onBattery === true, thermalState: thermal, speedLimit: Number.isFinite(limit) ? Math.max(0, Math.min(100, Math.round(limit))) : 100});
}
...
export function normalizePowerEvent(type, details = {}) {
  ...
  if (name === 'thermal-state-change') payload.thermalState = THERMAL_STATES.has(details.state) ? details.state : 'unknown';
  if (name === 'speed-limit-change') { const limit = Number(details.limit); payload.speedLimit = Number.isFinite(limit) ? Math.max(0, Math.min(100, Math.round(limit))) : 100; }
