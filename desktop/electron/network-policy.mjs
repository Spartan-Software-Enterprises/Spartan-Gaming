export const PROVIDER_AUTH_HOSTS = Object.freeze({
  ...
  'xbox-cloud-gaming': Object.freeze(['live.com', 'microsoft.com', 'microsoftonline.com', 'xboxlive.com']),
  'xbox-remote-play': Object.freeze(['live.com', 'microsoft.com', 'microsoftonline.com', 'xboxlive.com']),
  'amazon-luna': Object.freeze(['amazon.com', 'amazon.co.uk', 'amazon.ca', 'amazon.de', 'amazon.fr', 'amazon.co.jp']),
  ...
  'playstation-plus-cloud': Object.freeze(['playstation.com', 'sony.com', 'sonyentertainmentnetwork.com']),
  'playstation-remote-play': Object.freeze(['playstation.com', 'sony.com', 'sonyentertainmentnetwork.com']),
});

function cleanHttpsUrl(value, {allowHash = false} = {}) {
  try {
    const url = new URL(value);
    if (url.protocol !== 'https:' || url.username || url.password || (!allowHash && url.hash)) return null;
    return url;
  } catch { return null; }
}
...
export function createApprovedNetworkPolicy({providers = [], games = [], downloadSources = []} = {}) {
  ...
  for (const provider of providers) {
    for (const host of [...(PROVIDER_AUTH_HOSTS[provider?.id] || []), ...(provider?.networkHosts || [])]) { const approved = cleanHost(host); if (approved) serviceHosts.add(approved); }
