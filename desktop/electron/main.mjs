...
import {APP_ORIGIN, APP_PROTOCOL_PRIVILEGES, APP_SCHEME, createBundledAppProtocolHandler} from './app-protocol.mjs';
...
async function loadApprovedNetworkPolicy() {
  ...
  return createApprovedNetworkPolicy({providers: providerCatalog.providers, games: gameCatalog.games, downloadSources: ['https://github.com/Spartan-Software-Enterprises/Spartan-Gaming/releases']});
}
...
const shortcutController = createGlobalShortcutController({registry: globalShortcut, onActivate: showMainWindow});
...
async function createMainWindow() {
  ...
  windowRef.webContents.on('will-navigate', event => { if (!isAllowedNavigation(event.url, {appOrigin: APP_ORIGIN})) event.preventDefault(); });
...
if (hasSingleInstanceLock) app.whenReady().then(async () => {
  await protocol.handle(APP_SCHEME, createBundledAppProtocolHandler({frontendRoot: path.join(repositoryRoot, 'src/frontend'), publicRoot: repositoryRoot}));
