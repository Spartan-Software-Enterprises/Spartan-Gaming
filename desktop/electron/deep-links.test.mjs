...
test('Electron deep links accept only bounded catalog launch requests', () => {
  assert.deepEqual(normalizeSpartanDeepLink('spartan://launch?backend=steam-link'), {
    version: 1,
    action: 'launch',
    backendId: 'steam-link',
  });
  assert.deepEqual(
    findSpartanDeepLink(['--hidden', 'https://example.test', 'spartan://launch?backend=game_1']),
    { version: 1, action: 'launch', backendId: 'game_1' },
  );
});
