  ...
  providers: [
    {
      ...
      networkHosts: ['login.identity.example'],
      nativeApp: { releaseUrl: 'https://downloads.example/app.exe' },
    },
  ],
...
test('standalone network policy bounds service navigation and downloads', () => {
  ...
  assert.equal(
    policy.allowsServiceUrl('https://login.identity.example/oauth/callback#token'),
    true,
  );
