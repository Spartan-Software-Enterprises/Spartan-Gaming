...
const installRoot =
  process.env.SPARTAN_NATIVE_LINUX_INSTALL || path.resolve('out/native-linux/install');

test('Linux native input composes uinput gamepad events with reference keyboard and pointer events', async () => {
  const calls = [];
  const reference = {
    input: {
      execute: async (operation) => calls.push(['reference', operation.kind]),
      close() {
        calls.push(['reference', 'close']);
      },
    },
    capabilities: { input: true, keyboard: true, pointer: true, rumble: false, technologies: {} },
  };
  const native = {
    input: {
      execute: async (operation) => calls.push(['native', operation.kind]),
      close() {
  ...
  await input.execute({ kind: 'key' });
  await input.execute({ kind: 'pointer' });
  await input.execute({ kind: 'button' });
  await input.execute({ kind: 'axis' });
  await input.execute({ kind: 'rumble' });
  input.close();
...
test('Linux native package accepts the shared frontend button and axis control vocabulary', async () => {
  const calls = [];
  const reference = {
    input: { execute: async (operation) => calls.push(['reference', operation.kind]) },
    capabilities: { input: true, keyboard: true, pointer: true, rumble: false, technologies: {} },
  };
  const native = {
    input: { execute: async (operation) => calls.push(['native', operation.control]) },
    capabilities: { gamepad: true, rumble: true },
  };
  const input = composeLinuxInput({ reference, native });
...
test('Linux native package contract covers standard and extended Gamepad button indexes', async () => {
  const source = await (
    await import('node:fs/promises')
  ).readFile(new URL('./src/bindings.cpp', import.meta.url), 'utf8');
  ...
  assert.match(source, /BTN_TRIGGER_HAPPY1/);
  assert.match(source, /index <= 31/);
...
test('Linux package exposes the uinput rumble reader through the composed input adapter', async () => {
  ...
  const native = {
    input: {
      execute: async (operation) => calls.push(['native', operation.control]),
      readRumbleEvents: () => [{ strongMagnitude: 0.75, weakMagnitude: 0.25 }],
      close() {
        calls.push(['native', 'close']);
      },
    },
...
test('Linux package reports no rumble reader without a native uinput binding', () => {
  const reference = {
    input: { execute: async (operation) => ['reference', operation.kind], close() {} },
    capabilities: { input: true, keyboard: true, pointer: true, rumble: false, technologies: {} },
  };
...
test('Linux binding injects a full button and axis sequence into the real uinput device', async (t) => {
  ...
  try {
    await access('/dev/uinput', 6);
  } catch {
    t.skip('uinput is not readable and writable in this environment');
    return;
  }
  let module;
  try {
    module = await import(pathToFileURL(path.join(installRoot, 'index.mjs')).href);
  } catch (error) {
    t.skip(`Linux package is not built in this environment: ${error.message}`);
    return;
  }
  ...
  if (!bindings.capabilities.gamepad) {
    await bindings.close();
    t.skip('the uinput virtual gamepad is unavailable');
    return;
  }
  try {
    assert.equal(
    ...
    assert.equal(
      await bindings.input.execute({ kind: 'button', control: 'button-10', pressed: true }),
      true,
    );
    assert.equal(await bindings.input.execute({ kind: 'axis', control: 'axis-0', value: 1 }), true);
    assert.equal(
      await bindings.input.execute({ kind: 'axis', control: 'axis-5', value: -0.5 }),
      true,
    );
...
test('Linux binding scales active rumble by a live force-feedback gain change', async (t) => {
  ...
  if (!bindings.capabilities.rumble) {
    await bindings.close();
    t.skip('uinput force feedback is unavailable');
    return;
  }
  try {
    try {
      ...
      t.skip(
        `this kernel cannot upload uinput force-feedback effects (${error.message}); the binding is otherwise verified by its contract and source checks`,
      );
      return;
    }
    let sawStrong = false;
    ...
    assert.ok(
      bindings.input
        .readRumbleEvents()
        .every((event) => event.strongMagnitude <= 1 && event.weakMagnitude <= 1),
      'observed magnitudes are normalized to the shared 0..1 contract',
    );
  } finally {
    await bindings.close();
  }
