...
function probe(command) {
  return { status: ['ffmpeg', 'xdotool', 'pactl'].includes(command) ? 0 : 1 };
}
function encoderOutput() {
  return ` V..... libx264              libx264 H.264 / AVC / MPEG-4 AVC / MPEG-4 part 10 encoder (codec h264)\n V....D h264_vaapi            H.264/AVC (VAAPI) (codec h264)\n V..... libvpx-vp9            libvpx VP9 (codec vp9)\n V....D vp9_vaapi             VP9 (VAAPI) (codec vp9)\n V..... libaom-av1            libaom AV1 (codec av1)\n V....D av1_vaapi             AV1 (VAAPI) (codec av1)\n`;
}
...
test('Linux reference bindings report capability readiness without hiding missing session tools', async () => {
  const bindings = await createBindings({
    ...
    spawnProbe: (command, args) =>
      command === 'ffmpeg' && args?.[1] === '-encoders'
        ? { error: null, status: 0, stdout: encoderOutput() }
        : probe(command, args),
  });
  ...
  assert.deepEqual(
    bindings.capabilities.encoders.hardware.map((entry) => entry.codec),
    ['h264', 'vp9', 'av1'],
  );
  assert.deepEqual(bindings.capabilities, {
    capture: true,
    audio: true,
    input: true,
    keyboard: true,
    ...
    encoders: {
      hardware: [
        ...
        { codec: 'vp9', encoder: 'vp9_vaapi', device: '/dev/dri/renderD128' },
    ...
    requires: ['screen-capture-permission', 'audio-session-permission', 'remote-input-permission'],
  });
});
...
test('Linux portal consent gates capture and audio plans until granted', async () => {
  ...
  const portalProbe = (command) => {
    const probeProbe = probe(command);
    return command === 'xdg-desktop-portal' ? { status: 0 } : probeProbe;
  };
  ...
  assert.deepEqual(bindings.consent.request('capture'), {
    kind: 'capture',
    granted: true,
    portal: 'org.freedesktop.portal.ScreenCast',
  });
...
test('Linux reference input maps only normalized keyboard and pointer operations to shell-free argv', () => {
  assert.deepEqual(inputCommand({ kind: 'key', control: 'KeyA', pressed: true }), {
    args: ['keydown', 'a'],
  });
  assert.deepEqual(inputCommand({ kind: 'key', control: 'ArrowLeft', pressed: false }), {
    args: ['keyup', 'Left'],
  });
  assert.deepEqual(inputCommand({ kind: 'pointer', deltaX: 12.4, deltaY: -9.6 }), {
    args: ['mousemove_relative', '--', '12', '-10'],
  });
  assert.deepEqual(inputCommand({ kind: 'pointer', action: 'pointer:down', control: 'button-0' }), {
    args: ['mousedown', '1'],
  });
  assert.deepEqual(inputCommand({ kind: 'pointer', action: 'pointer:up', control: 'button-2' }), {
    args: ['mouseup', '3'],
  });
  assert.deepEqual(inputCommand({ kind: 'pointer', action: 'pointer:wheel', deltaY: -120 }), {
    args: ['click', '4'],
  });
  assert.deepEqual(
    inputCommand({ kind: 'pointer', action: 'pointer:wheel', deltaX: 120, deltaY: 0 }),
    { args: ['click', '7'] },
  );
  ...
  assert.throws(
    () => keyName('F25'),
    (error) => error.code === 'ERR_UNSUPPORTED_INPUT' && /F25/.test(error.message),
  );
  assert.throws(
    () => inputCommand({ kind: 'button', control: 'a' }),
    (error) => error.code === 'ERR_UNSUPPORTED_INPUT',
  );
  assert.throws(
    () => inputCommand({ kind: 'pointer', action: 'pointer:down', control: 'button-9' }),
    (error) => error.code === 'ERR_UNSUPPORTED_INPUT',
  );
});
...
test('Linux reference audio bindings enumerate pactl capture devices without a shell', async () => {
  ...
  const bindings = await createBindings({
    environment: { DISPLAY: ':1' },
    spawnProbe: (command, args) =>
      command === 'pactl' && args?.join(' ') === 'list short sources'
        ? { error: null, status: 0, stdout: sources }
        : probe(command, args),
  });
  assert.deepEqual(
    bindings.audio.devices().map((device) => device.id),
  ...
  assert.equal(
    bindings.audio.plan({
      ...
      source: 'alsa_input.usb-046d_C922_Pro_Stream_Webcam',
    }).source,
    'alsa_input.usb-046d_C922_Pro_Stream_Webcam',
  );
  assert.deepEqual(
    bindings.audio.devices({ includeMonitors: true }).map((device) => device.kind),
...
test('Linux reference bindings expose a bounded display descriptor from a real xrandr query', async () => {
  ...
  const bindings = await createBindings({
    environment: { DISPLAY: ':1' },
    spawnProbe: (command, args) =>
      command === 'xrandr' && args?.join(' ') === '--query'
        ? { error: null, status: 0, stdout: xrandr }
        : probe(command, args),
  });
  assert.deepEqual(bindings.display, {
    width: 1360,
    height: 768,
    maxRefreshRate: 60,
    count: 1,
