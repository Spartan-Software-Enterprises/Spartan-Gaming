...
async function loadUinputModule() {
  try {
    return require('./spartan-native-linux.node');
  } catch {
    return null;
  }
}
...
export function composeLinuxInput({ reference, native } = {}) {
  ...
  return Object.freeze({
    async execute(operation) {
      ...
      if ((kind === 'button' || kind === 'axis') && nativeGamepad)
        return nativeInput.execute(operation);
      if (kind === 'rumble' && nativeRumble) return nativeInput.execute(operation);
      if (typeof referenceInput?.execute !== 'function')
        throw new Error(`Linux input adapter does not implement ${kind || 'unknown'} events`);
      return referenceInput.execute(operation);
    },
    readRumbleEvents() {
      return typeof nativeInput.readRumbleEvents === 'function'
        ? nativeInput.readRumbleEvents()
        : [];
    },
    close() {
      nativeInput.close?.();
...
export async function createBindings(options = {}) {
  ...
  const module =
    options.nativeModule === undefined ? await loadUinputModule() : options.nativeModule;
  ...
  return Object.freeze({
    ...
    capabilities: Object.freeze({
      ...
      input: Boolean(reference.capabilities.input || native.capabilities?.gamepad),
      keyboard: Boolean(reference.capabilities.keyboard),
      pointer: Boolean(reference.capabilities.pointer),
      gamepad: Boolean(native.capabilities?.gamepad),
      virtualGamepad: Boolean(native.capabilities?.virtualGamepad ?? native.capabilities?.gamepad),
      rumble: Boolean(native.capabilities?.rumble || reference.capabilities.rumble),
      technologies: Object.freeze({
        ...reference.capabilities.technologies,
        input: 'Linux uinput virtual gamepad + reference keyboard/pointer',
        haptics: native.capabilities?.rumble
          ? 'Linux uinput force feedback'
          : reference.capabilities.technologies?.haptics,
      }),
    }),
