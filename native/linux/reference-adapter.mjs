...
const { createAudioCapturePlan, listAudioCaptureDevices } =
  await import('../../host/audio.mjs').catch(() => import('./host/audio.mjs'));
const { createCapturePlan, createFfmpegEncoderProbe, selectHardwareEncoder } =
  await import('../../host/media.mjs').catch(() => import('./host/media.mjs'));
...
const PORTAL_KINDS = Object.freeze({
  capture: 'org.freedesktop.portal.ScreenCast',
  audio: 'org.freedesktop.portal.ScreenCast',
});
...
function keyName(control) {
  ...
  const keysym = {
    ...
    LaunchMail: 'XF86Mail',
    LaunchApp1: 'XF86Launch1',
    LaunchApp2: 'XF86Launch2',
    Sleep: 'XF86Sleep',
    WakeUp: 'XF86WakeUp',
    Print: 'Print',
    Select: 'Select',
...
function inputCommand(operation) {
  if (operation.kind === 'pointer') {
    ...
    if (
      operation.action === 'pointer:down' ||
      operation.action === 'pointer:up' ||
      operation.action === 'pointer:cancel'
    ) {
      if (!button)
        throw unsupported(`Linux X11 reference does not support mouse button ${operation.control}`);
  ...
  if (operation.kind === 'key') {
    const action =
      operation.pressed === false || operation.action === 'release' ? 'keyup' : 'keydown';
...
function portalServiceAvailable(environment, probe) {
  ...
  if (environment.SPARTAN_PORTAL_AVAILABLE !== undefined)
    return environment.SPARTAN_PORTAL_AVAILABLE === true;
...
function createPortalConsent({ environment = DEFAULT_ENVIRONMENT, probe = spawnSync } = {}) {
  ...
  return Object.freeze({
    ...
    get status() {
      return available ? (grants.size ? 'granted' : 'available') : 'unavailable';
    },
    get grantedKinds() {
    ...
    request(kind) {
      ...
      if (!available)
        throw new Error(
          'xdg-desktop-portal is not reachable; screen/audio capture consent cannot be requested',
        );
      grants.add(kind);
...
function captureController({ environment, spawnImpl, processFactory, consent }) {
  let controller = null;
  const plan = (options = {}) => {
    if (options.permissionGranted !== true && !consent.granted('capture'))
      throw new Error('Linux screen-capture permission is not granted');
    const sourceType =
      options.sourceType ||
      (environment.WAYLAND_DISPLAY || environment.XDG_RUNTIME_DIR ? 'pipewire' : 'x11');
    const source =
      options.source ||
      (sourceType === 'x11'
        ? `${required(environment.DISPLAY, 'environment.DISPLAY')}+0,0`
        : 'default');
...
function audioController({ environment, spawnImpl, processFactory, consent, spawnProbe }) {
  let controller = null;
  const backendFor = (options) =>
    options.backend ||
    (environment.WAYLAND_DISPLAY || environment.XDG_RUNTIME_DIR ? 'pipewire' : 'pulse');
  const plan = (options = {}) => {
    if (options.permissionGranted !== true && !consent.granted('audio'))
      throw new Error('Linux audio-capture permission is not granted');
  ...
  const devices = (options = {}) =>
    listAudioCaptureDevices({
      platform: PLATFORM,
      backend: backendFor(options),
      includeMonitors: options.includeMonitors === true,
      run: (command, args) => {
        const result = spawnProbe(command, args, { encoding: 'utf8' });
...
export async function createBindings({
  ...
  const encoderProbe = ffmpeg
    ? createFfmpegEncoderProbe({
        command: ffmpegCommand,
        run: (command, args, options) => {
          const result = spawnProbe(command, args, { encoding: 'utf8', ...options });
          return result?.error ? null : String(result.stdout || '');
        },
      })
    : () => false;
  const encoders = Object.freeze(
    ['h264', 'vp9', 'av1'].flatMap((codec) => {
      const selected = selectHardwareEncoder({ codec, platform: PLATFORM, probe: encoderProbe });
      return selected
        ? [Object.freeze({ codec, encoder: selected.encoder, device: selected.device })]
        : [];
    }),
  );
    ...
    async execute(operation) {
      ...
      await new Promise((resolve, reject) => {
        ...
        child.once?.('close', (code) =>
          code === 0 ? resolve() : reject(new Error(`xdotool exited with status ${code}`)),
        );
      });
    },
    close() {},
  ...
  return Object.freeze({
    ...
    capabilities: Object.freeze({
      ...
      rumble: false,
      portal: consent.available,
      encoders: Object.freeze({ hardware: encoders }),
      technologies: Object.freeze({
        capture: 'FFmpeg x11grab/PipeWire',
        audio: 'FFmpeg Pulse/PipeWire',
        input: 'X11 XTest via xdotool',
      }),
      requires: Object.freeze([
        'screen-capture-permission',
        'audio-session-permission',
        'remote-input-permission',
      ]),
    }),
    capture,
    audio,
