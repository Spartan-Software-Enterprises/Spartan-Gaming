...
function commandAvailable(command, probe = spawnSync) { try { const result = probe(command, ['-version'], {stdio: 'ignore', shell: false}); return !result?.error && result.status === 0; } catch { return false; } }

function processController({plan, spawnImpl = spawn, processFactory = createManagedProcess} = {}) {
  let managed = null;
  return Object.freeze({
    async start() { if (managed) throw new Error('desktop media process is already active'); managed = processFactory({plan, spawnImpl, stdio: ['ignore', 'pipe', 'pipe']}); try { await managed.start(); return Object.freeze({pid: managed.pid, streams: managed.streams, process: managed}); } catch (error) { managed = null; throw error; } },
...
export async function createBindings({platform, environment = process.env, spawnImpl = spawn, spawnProbe = spawnSync, processFactory = createManagedProcess} = {}) {
  ...
  return Object.freeze({platform, capabilities: Object.freeze({capture: ffmpeg, audio: ffmpeg, input: false, keyboard: false, pointer: false, gamepad: false, rumble: false, technologies: Object.freeze({capture: platform === 'darwin' ? 'FFmpeg AVFoundation' : 'FFmpeg gdigrab', audio: platform === 'darwin' ? 'FFmpeg AVFoundation/CoreAudio' : 'FFmpeg WASAPI'}), requires: Object.freeze(['screen-capture-permission', 'microphone-permission', 'native-input-adapter'])}), capture, audio, async close() { await Promise.allSettled([capture.stop(), audio.stop()]); }});
}
