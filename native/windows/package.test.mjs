...
const installRoot =
  process.env.SPARTAN_NATIVE_WINDOWS_INSTALL || path.resolve('out/native-windows/install');

test('built Windows package exposes the universal input binding shape', async (t) => {
  if (process.platform !== 'win32') {
    t.skip('Windows package contract runs on Windows');
    return;
  }
  let module;
  try {
    ...
    t.skip(`Windows package is not built in this environment: ${error.message}`);
    return;
  }
