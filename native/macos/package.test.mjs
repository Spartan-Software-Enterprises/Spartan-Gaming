...
const installRoot =
  process.env.SPARTAN_NATIVE_MACOS_INSTALL || path.resolve('out/native-macos/install');

test('built macOS package exposes the universal input binding shape', async (t) => {
  if (process.platform !== 'darwin') {
    t.skip('macOS package contract runs on macOS');
    return;
  }
  let module;
  try {
    ...
    t.skip(`macOS package is not built in this environment: ${error.message}`);
    return;
  }
...
test('macOS native input contract preserves frontend pointer button transitions', async () => {
  ...
  ).readFile(new URL('./src/bindings.mm', import.meta.url), 'utf8');
...
test('macOS native input contract maps numpad and extended function browser codes', async () => {
  ...
  assert.match(source, /kVK_JIS_Yen/);
});
