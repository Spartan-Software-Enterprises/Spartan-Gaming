...
export async function createBindings(options = {}) {
  ...
  return Object.freeze({...reference, ...bindings, platform: 'darwin', capabilities: Object.freeze({...reference.capabilities, ...bindings.capabilities, input: true, keyboard: true, pointer: true, gamepad: false, virtualGamepad: false, rumble: Boolean(bindings.capabilities?.rumble)}), capture: reference.capture, audio: reference.audio, async close() { bindings.input.close?.(); await reference.close(); }});
}
