...
if (pathEqualsMain()) {
  try {
    ...
    const secret = args.get('secret') || resolveConfiguredSecret({name: 'SPARTAN_SIGNALING_SECRET'}); const adminSecret = args.get('admin-secret') || resolveConfiguredSecret({name: 'SPARTAN_SIGNALING_ADMIN_SECRET'});
