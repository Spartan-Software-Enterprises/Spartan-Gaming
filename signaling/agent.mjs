...
export function resolveSignalingSecrets({args = new Map(), env = process.env, readFile} = {}) {
  return Object.freeze({secret: args.get('secret') || resolveConfiguredSecret({env, name: 'SPARTAN_SIGNALING_SECRET', ...(readFile ? {readFile} : {})}), adminSecret: args.get('admin-secret') || resolveConfiguredSecret({env, name: 'SPARTAN_SIGNALING_ADMIN_SECRET', ...(readFile ? {readFile} : {})})});
}
