...
function secret(value, name) {
  ...
  if (result.length < 32) throw new RangeError(`${name} must contain at least 32 characters`);
  return result;
}
function urls(value) {
  return (Array.isArray(value) ? value : text(value).split(',')).map(text).filter(Boolean);
}
...
export function normalizeProductionConfig(options = {}) {
  ...
  const turnSecret = secret(options.turnSecret, 'TURN shared secret');
  if (adminSecret === secretValue)
    throw new Error('admin secret must differ from the signaling secret');
  const tlsKey = required(options.tlsKey, 'TLS key path');
  const tlsCert = required(options.tlsCert, 'TLS certificate path');
  ...
  if (!allowedOrigins.length || allowedOrigins.some((origin) => !origin.startsWith('https://')))
    throw new TypeError('production allowed origins must contain HTTPS origins');
  const sessionStore = text(options.sessionStore).toLowerCase();
  if (!SESSION_STORES.has(sessionStore))
    throw new TypeError('production session store must be redis, database, or external');
  ...
  if (!turnUrls.length || turnUrls.some((url) => !/^turns?:/i.test(url)))
    throw new TypeError('production TURN URLs must contain turn: or turns: endpoints');
  return Object.freeze({
    environment: 'production',
    tls: Object.freeze({ keyPath: tlsKey, certPath: tlsCert }),
    allowedOrigins: Object.freeze(allowedOrigins),
    sessionStore,
    brokerPackage,
    turnUrls: Object.freeze(turnUrls),
    secrets: Object.freeze({ configured: true, adminConfigured: true, turnConfigured: true }),
  });
}

export function resolveConfiguredSecret({ env = process.env, name, readFile = readFileSync } = {}) {
  if (typeof name !== 'string' || !name.trim())
    throw new TypeError('secret environment name is required');
  ...
  try {
    return text(readFile(filePath, 'utf8'));
  } catch {
...
export function resolveProductionConfig({ env = process.env, readFile = readFileSync } = {}) {
  const source = {
    ...
    turnSecret: resolveConfiguredSecret({ env, name: 'SPARTAN_SIGNALING_TURN_SECRET', readFile }),
