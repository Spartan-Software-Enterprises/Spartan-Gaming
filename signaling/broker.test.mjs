...
test('signaling broker authenticates scoped tickets and routes messages between roles', () => {
  ...
  const client = broker.attach({
    sessionId: 'ses-signal-01',
    role: 'client',
    ticket: broker.issueTicket({ sessionId: 'ses-signal-01', role: 'client' }),
    send: (message) => received.push(['client', message.type]),
  });
  const host = broker.attach({
    sessionId: 'ses-signal-01',
    role: 'host',
    ticket: broker.issueTicket({ sessionId: 'ses-signal-01', role: 'host' }),
    send: (message) => received.push(['host', message.type]),
  });
  client.send(offer());
...
test('signaling broker rejects expired, cross-session, duplicate-role, and oversized messages', () => {
  let now = 1000;
  const broker = createSignalingBroker({ secret: 'test-only-secret', clock: () => now });
  const clientTicket = broker.issueTicket({
    sessionId: 'ses-signal-02',
    role: 'client',
    ttlMs: 1000,
  });
  now += 1001;
  ...
  assert.throws(
    () => broker.attach({ sessionId: 'ses-signal-02', role: 'client', ticket: valid, send() {} }),
    /already attached/,
  );
  assert.throws(
    () =>
      client.send(
        createSessionEnvelope({ sessionId: 'ses-other-01', type: 'session.offer', payload: {} }),
      ),
    /another signaling session/,
  );
  const huge = createSessionEnvelope({
    sessionId: 'ses-signal-02',
    type: 'session.offer',
...
test('signaling broker rejects forged ticket signatures and invalid roles', () => {
  ...
  assert.throws(
    () =>
      broker.attach({
        sessionId: 'ses-signal-03',
        role: 'host',
        ticket: 'forged.signature',
        send() {},
      }),
    /invalid signaling ticket/,
  );
  assert.throws(
    () => broker.issueTicket({ sessionId: 'ses-signal-03', role: 'relay' }),
