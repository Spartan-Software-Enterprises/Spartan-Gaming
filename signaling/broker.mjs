...
export function verifySignalingTicket(ticket, { secret, sessionId, role, now }) {
  ...
  if (actualBytes.length !== expectedBytes.length || !timingSafeEqual(actualBytes, expectedBytes))
    throw new Error('invalid signaling ticket signature');
  let claims;
  try {
    claims = JSON.parse(decode(body));
  } catch {
    throw new Error('invalid signaling ticket claims');
  }
  if (
    claims.version !== 1 ||
    ...
    throw new Error('signaling ticket scope mismatch');
  if (!Number.isFinite(claims.expiresAt) || claims.expiresAt <= now())
    throw new Error('signaling ticket expired');
...
export function createSignalingBroker({
  ...
  const sweep = () => {
    ...
    for (const [sessionId, session] of sessions)
      if (session.lastActivityAt < cutoff || session.participants.size === 0)
  ...
  const attach = ({ sessionId, role, ticket: joinTicket, send } = {}) => {
    ...
    if (!session) {
      if (sessions.size >= maxSessions)
        throw new Error('signaling broker session capacity reached');
  ...
  return Object.freeze({
    ...
    stats() {
      ...
      return Object.freeze({
        ...
        participants: [...sessions.values()].reduce(
