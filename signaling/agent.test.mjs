...
test('opt-in admin API protects health and mints scoped tickets without exposing admin secret', async () => {
  ...
  try {
    ...
    const enrollmentResponse = await fetch(`${endpoint}/admin/host-enrollment`, {method: 'POST', headers: {'content-type': 'application/json', authorization: `Bearer ${adminSecret}`}, body: JSON.stringify({endpoint: 'wss://signal.example/signal', sessionId: 'ses-enroll-01', subject: 'living-room', ttlMs: 60000})}); const enrollment = await enrollmentResponse.json(); assert.equal(enrollmentResponse.status, 201); assert.equal(enrollment.endpoint, 'wss://signal.example/signal'); assert.equal(enrollment.role, 'host'); assert.equal(enrollment.subject, 'living-room'); assert.equal(typeof enrollment.ticket, 'string'); assert.equal('adminSecret' in enrollment, false);
