...
function parseServerFrames(buffer, emit) {
  ...
  while (buffer.length - cursor >= 2) {
    ...
    const payload = buffer.subarray(cursor + header, cursor + header + length);
  ...
  return new Promise((resolve, reject) => {
    ...
    const next = () =>
      new Promise((resolveNext, rejectNext) => {
        if (messages.length) return resolveNext(messages.shift());
        const timer = setTimeout(
          () => rejectNext(new Error('signaling message timed out after 10 seconds')),
          10_000,
        );
        waiters.push((value) => {
          clearTimeout(timer);
    ...
    socket.on('data', (chunk) => {
      ...
      if (!handshake) {
        ...
        handshake = true;
        resolve({
          send: (value) => socket.write(clientFrame(value)),
          next,
...
test('reference signaling service routes authenticated client and host envelopes over real WebSockets', async () => {
  ...
  const sessionId = 'ses-signal-01';
  let client;
  let host;
  try {
    const address = await service.start();
    const clientTicket = service.broker.issueTicket({
      sessionId,
      role: 'client',
      subject: 'browser',
    });
    const hostTicket = service.broker.issueTicket({ sessionId, role: 'host', subject: 'host' });
    client = await connect(address.port);
    host = await connect(address.port);
    client.send({ type: 'signaling.join', sessionId, role: 'client', ticket: clientTicket });
    host.send({ type: 'signaling.join', sessionId, role: 'host', ticket: hostTicket });
    ...
    client.send(offer);
    assert.deepEqual(await host.next(), offer);
    const answer = createSessionEnvelope({
      ...
      payload: {
        ...
        capabilities: {
          transports: ['webrtc'],
          video: { codecs: ['h264'], maxWidth: 1280, maxHeight: 720, maxFramerate: 60 },
