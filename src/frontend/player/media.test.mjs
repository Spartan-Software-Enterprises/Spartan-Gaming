...
test('media coordinator describes and attaches audio/video tracks', () => {
  assert.deepEqual(describeMediaStream(stream), {
    videoTracks: 1,
    audioTracks: 1,
    hasVideo: true,
    hasAudio: true,
...
test('caption preferences select one matching text track and fail safely without tracks', () => {
  assert.deepEqual(normalizeCaptionPreference({ mode: 'On', language: 'Spanish' }), {
    mode: 'On',
    language: 'Spanish',
  });
  const tracks = [
    { kind: 'captions', language: 'en', default: true, mode: 'disabled' },
    { kind: 'subtitles', language: 'es', mode: 'disabled' },
  ];
  const video = { textTracks: tracks };
  assert.deepEqual(applyCaptionPreference(video, { mode: 'On', language: 'Spanish' }), {
    supported: true,
    active: 'es',
    preference: { mode: 'On', language: 'Spanish' },
  });
