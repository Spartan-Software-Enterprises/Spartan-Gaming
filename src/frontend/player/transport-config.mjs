...
export function resolveSignalingTransport({endpoint, policy, webTransportAvailable = typeof globalThis.WebTransport === 'function', webSocketAvailable = typeof globalThis.WebSocket === 'function'} = {}) {
  ...
  let parsed;
  try { parsed = new URL(endpoint); } catch { throw new TypeError('signaling endpoint is invalid'); }
  const loopback = parsed.hostname === 'localhost' || parsed.hostname === '127.0.0.1' || parsed.hostname === '[::1]';
  if (parsed.username || parsed.password || !['https:', 'wss:'].includes(parsed.protocol) && !(parsed.protocol === 'ws:' && loopback)) throw new TypeError('signaling endpoint must use a secure URL or loopback WebSocket');
  ...
  if (signalingPreference !== 'auto') {
    if (!candidates.includes(signalingPreference)) throw new Error(`Preferred signaling transport requires a compatible endpoint: ${signalingPreference}`);
    if (signalingPreference === 'webtransport' && (!normalized.allowWebTransport || !webTransportAvailable)) throw new Error('WebTransport is unavailable or disabled');
    if (signalingPreference === 'websocket' && (!normalized.allowWebSocketFallback || !webSocketAvailable)) throw new Error('WebSocket fallback is unavailable or disabled');
  }
  const selected = signalingPreference === 'auto' && protocol === 'https:' && normalized.allowWebTransport && webTransportAvailable ? 'webtransport' : candidates[0];
