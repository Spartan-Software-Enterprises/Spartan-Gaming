...
export async function preparePlayerSession({
  storage = globalThis.localStorage,
  workspaceStorage = createActiveProfileStorage({ storage }),
  controllerProfile,
  collect = collectCapabilities,
} = {}) {
  let capabilities = null;
  try {
    capabilities = await collect();
  } catch {
    capabilities = null;
  }
  ...
  return Object.freeze({
    capabilities,
    preferences,
    inputPolicy: createInputPermissionPolicy(preferences.capabilities),
  });
}
