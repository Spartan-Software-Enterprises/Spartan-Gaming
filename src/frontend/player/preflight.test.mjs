...
const storage = {getItem: () => JSON.stringify({'media.hdr': true, 'media.refreshRate': '240 Hz', 'streaming.codec': 'Automatic'})};

test('player preflight applies browser capability evidence before negotiation', async () => {
  const result = await preparePlayerSession({storage, collect: async () => ({graphics: {hdr: false}, display: {maxRefreshRate: 144}, media: {codecs: {av1: false, vp9: true, h264: true}}})});
...
test('player preflight remains usable when capability probing fails', async () => {
  const result = await preparePlayerSession({storage, collect: async () => { throw new Error('probe unavailable'); }});
