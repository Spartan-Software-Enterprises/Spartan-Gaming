...
const CAPTION_LANGUAGES = new Set(['Automatic', 'English', 'Spanish', 'French', 'German', 'Japanese']);
const CAPTION_LANGUAGE_CODES = Object.freeze({English: 'en', Spanish: 'es', French: 'fr', German: 'de', Japanese: 'ja'});

export function normalizeCaptionPreference({mode = 'Off', language = 'Automatic'} = {}) {
  return Object.freeze({mode: CAPTION_MODES.has(mode) ? mode : 'Off', language: CAPTION_LANGUAGES.has(language) ? language : 'Automatic'});
}
...
export function applyCaptionPreference(video, preference = {}) {
  ...
  if (!list || typeof list.length !== 'number') return Object.freeze({supported: false, active: null, preference: normalized});
  const available = Array.from({length: list.length}, (_, index) => list[index]).filter(Boolean);
  available.forEach(track => { track.mode = 'disabled'; });
  if (normalized.mode === 'Off') return Object.freeze({supported: true, active: null, preference: normalized});
  const language = normalized.language === 'Automatic' ? null : CAPTION_LANGUAGE_CODES[normalized.language];
  const candidates = available.filter(track => ['captions', 'subtitles'].includes(String(track.kind || '').toLowerCase()));
  const selected = candidates.find(track => language && String(track.language || track.srclang || '').toLowerCase().startsWith(language)) || candidates.find(track => track.default) || candidates[0] || null;
  if (selected) selected.mode = 'showing';
  return Object.freeze({supported: true, active: selected ? String(selected.language || selected.srclang || selected.label || selected.kind || 'default') : null, preference: normalized});
}
