...
test('immersive controller requests the selected Screen Details display and falls back when options are rejected', async () => {
  const documentRef = fakeDocument(); const calls = []; const selectedScreen = {label: 'Display 2'};
  const navigatorRef = {getScreenDetails: async () => ({screens: [{label: 'Display 1'}, selectedScreen]})};
  const target = {requestFullscreen(options) { calls.push(options); documentRef.fullscreenElement = target; if (options) throw new TypeError('screen option unsupported'); }};
  const controller = createImmersiveController({target, documentRef, navigatorRef, display: {kind: 'index', index: 1}});
  await controller.enter(); assert.equal(calls.length, 2); assert.deepEqual(calls[0], {screen: selectedScreen}); assert.equal(calls[1], undefined); controller.dispose();
});

test('immersive controller safely ignores unavailable selected displays', async () => {
  const documentRef = fakeDocument(); const calls = []; const target = {requestFullscreen(options) { calls.push(options); documentRef.fullscreenElement = target; }};
  const controller = createImmersiveController({target, documentRef, navigatorRef: {getScreenDetails: async () => ({screens: []})}, display: {kind: 'index', index: 2}});
  await controller.enter(); assert.deepEqual(calls, [undefined]);
});
