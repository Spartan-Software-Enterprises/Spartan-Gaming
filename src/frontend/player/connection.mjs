...
function endpoint(value) { const raw = text(value); if (!raw) return ''; try { const url = new URL(raw); const loopback = url.hostname === 'localhost' || url.hostname === '127.0.0.1' || url.hostname === '[::1]'; if (url.username || url.password || !['https:', 'wss:'].includes(url.protocol) && !(url.protocol === 'ws:' && loopback)) return ''; return url.href; } catch { return ''; } }
