...
const PREFERRED_OUTPUT_LABELS = Object.freeze({
  Headphones: Object.freeze(['headphone', 'headset', 'earbud']),
  Speakers: Object.freeze(['speaker']),
  'HDMI/Display': Object.freeze(['hdmi', 'display', 'monitor', 'tv']),
});
...
export function normalizeAudioOutputDevice(device, index = 0) {
  ...
  return Object.freeze({
    deviceId,
    label: text(device.label) || `Audio output ${index + 1}`,
    kind: 'audiooutput',
  });
}
...
export async function listAudioOutputDevices({
  ...
  return Object.freeze(
    devices
      .map((device, index) => normalizeAudioOutputDevice(device, index))
...
export function findPreferredAudioOutput(devices = [], preference = 'System default') {
  ...
  return (
    devices.find((device) => labels.some((label) => device.label.toLowerCase().includes(label))) ||
    null
  );
}

export async function selectAudioOutput(video, deviceId = '') {
  if (!canSelectAudioOutput(video))
