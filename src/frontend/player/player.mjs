...
import {createTouchControlEvent, createTouchControlLayout, createTouchStickEvents, normalizeTouchLayout} from '../input/touch-controls.mjs';
...
const wakeLock = createWakeLockController({enabled: sessionPreferences.preferences.mobile.keepScreenAwake});
...
function apply(event) {
  ...
  globalThis.spartanElectron?.setSessionActive?.(state.status === 'connected' || state.status === 'reconnecting');
  ...
  if (state.status === 'connected' || state.status === 'reconnecting') void wakeLock.start(); else if (state.status === 'ended' || state.status === 'error') void wakeLock.stop();
...
function installTouchControls() {
  ...
  const root = document.createElement('div'); root.className = `touch-controls touch-controls-${layout}`; root.setAttribute('aria-label', 'Touch game controls');
  root.innerHTML = createTouchControlLayout(layout).map(control => `<button type="button" class="touch-control touch-${control.group}${control.side ? ` touch-${control.side}` : ''}" data-touch-action="${control.action}" data-touch-control="${control.control || control.action}" aria-label="${control.action}">${control.label}</button>`).join('') + (layout === 'full' ? '<div class="touch-stick touch-stick-left" data-touch-stick="0" role="slider" aria-label="Left analog stick"><span></span></div><div class="touch-stick touch-stick-right" data-touch-stick="2" role="slider" aria-label="Right analog stick"><span></span></div>' : '');
  const active = new Map();
  const emitStick = (stick, event, pressed = true) => {
    const rect = stick.getBoundingClientRect(); const radius = Math.max(1, Math.min(rect.width, rect.height) / 2); let x = (event.clientX - (rect.left + rect.width / 2)) / radius; let y = (event.clientY - (rect.top + rect.height / 2)) / radius; const magnitude = Math.hypot(x, y); if (magnitude > 1) { x /= magnitude; y /= magnitude; } createTouchStickEvents({index: Number(stick.dataset.touchStick), x, y, pressed}).forEach(emitInput); stick.style.setProperty('--stick-x', `${x * 50}%`); stick.style.setProperty('--stick-y', `${y * 50}%`);
  };
  const release = (pointerId, fallback) => { const entry = active.get(pointerId); if (!entry) return; active.delete(pointerId); if (entry.kind === 'stick') { emitStick(entry.element, fallback || {clientX: 0, clientY: 0}, false); entry.element.classList.remove('is-active'); entry.element.style.setProperty('--stick-x', '0%'); entry.element.style.setProperty('--stick-y', '0%'); } else { entry.element.classList.remove('is-pressed'); emitInput(createTouchControlEvent({action: entry.action, control: entry.control, pressed: false})); } entry.element.releasePointerCapture?.(pointerId); };
  root.addEventListener('pointerdown', event => { const stick = event.target.closest('[data-touch-stick]'); const button = event.target.closest('[data-touch-action]'); if (!stick && !button) return; event.preventDefault(); event.stopPropagation(); const element = stick || button; if (stick) { active.set(event.pointerId, {kind: 'stick', element}); element.classList.add('is-active'); element.setPointerCapture?.(event.pointerId); emitStick(element, event); } else { active.set(event.pointerId, {kind: 'button', action: button.dataset.touchAction, control: button.dataset.touchControl, element: button}); button.setPointerCapture?.(event.pointerId); button.classList.add('is-pressed'); emitInput(createTouchControlEvent({action: button.dataset.touchAction, control: button.dataset.touchControl, pressed: true})); } });
  root.addEventListener('pointermove', event => { const entry = active.get(event.pointerId); if (!entry || entry.kind !== 'stick') return; event.preventDefault(); event.stopPropagation(); emitStick(entry.element, event); });
  root.addEventListener('pointerup', event => { if (!active.has(event.pointerId)) return; event.preventDefault(); event.stopPropagation(); release(event.pointerId, event); });
