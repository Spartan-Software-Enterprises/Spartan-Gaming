...
function installTouchControls() {
  ...
  root.innerHTML = createTouchControlLayout(layout).map(control => `<button type="button" class="touch-control touch-${control.group}" data-touch-action="${control.action}" aria-label="${control.action}">${control.label}</button>`).join('') + (layout === 'full' ? '<div class="touch-stick touch-stick-left" data-touch-stick="0" role="slider" aria-label="Left analog stick"><span></span></div><div class="touch-stick touch-stick-right" data-touch-stick="2" role="slider" aria-label="Right analog stick"><span></span></div>' : '');
