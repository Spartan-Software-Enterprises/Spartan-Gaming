...
test('player connections reject insecure, credentialed, and malformed signaling endpoints', () => {
  ...
  assert.equal(
    normalizePlayerConnection({ endpoint: 'wss://user:ticket@relay.example/signal' }).endpoint,
    '',
  );
  assert.equal(normalizePlayerConnection({ endpoint: 'not a URL' }).endpoint, '');
  assert.equal(
    normalizePlayerConnection({ endpoint: 'ws://localhost:8790/signal' }).endpoint,
    'ws://localhost:8790/signal',
  );
  assert.equal(
    normalizePlayerConnection({ endpoint: 'https://relay.example/signal' }).endpoint,
    'https://relay.example/signal',
  );
});
