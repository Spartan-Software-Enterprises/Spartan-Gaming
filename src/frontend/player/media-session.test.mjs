...
test('Media Session controller exposes metadata and safe transport actions', async () => {
  ...
  controller.update({ title: 'Forza Horizon 5', artist: 'Spartan Gaming' });
  ...
  actions.get('seekforward')({ seekOffset: 8 });
  assert.equal(video.currentTime, 33);
  actions.get('stop')();
...
test('Media Session gracefully degrades when the browser has no API', () => {
  const video = fakeVideo();
  assert.equal(canUseMediaSession({}), false);
  const controller = createMediaSessionController({ navigatorRef: {}, video });
