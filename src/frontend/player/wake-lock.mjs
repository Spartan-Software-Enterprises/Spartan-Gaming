...
export function createWakeLockController({
  ...
  const publish = (next) => {
    if (!STATES.includes(next)) return state;
    state = next;
    onState(state);
    return state;
  };
  const handleRelease = () => {
    sentinel = null;
    publish(desired ? 'released' : 'disabled');
  };
  const acquire = async () => {
  ...
  const acquireWhenVisible = () => {
    if (desired && documentRef?.visibilityState !== 'hidden' && !sentinel) void acquire();
  };
  const start = async () => {
    desired = true;
    return acquire();
  ...
  const stop = async () => {
    if (!desired && !sentinel) return true;
    desired = false;
    ...
    try {
      ...
      /* Browser revocation is already a safe stop. */
    }
    publish('disabled');
    return true;
  };
  ...
  return Object.freeze({
    ...
    stop,
    close() {
      documentRef?.removeEventListener?.('visibilitychange', acquireWhenVisible);
      void stop();
      return this;
    },
  });
