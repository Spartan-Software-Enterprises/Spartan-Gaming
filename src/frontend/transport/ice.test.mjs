...
test('ICE configuration rejects embedded credentials, insecure remote TURN, and excessive limits', () => {
  assert.throws(
    () => normalizeIceServer({ urls: 'turn://user:pass@relay.example.test' }),
    /embedded/,
  );
  ...
  assert.throws(
    () =>
      createIceConfiguration({
        servers: Array.from({ length: 9 }, () => ({ urls: 'stun:example.test' })),
      }),
    /8/,
  );
...
test('ICE diagnostics never expose credential values', () => {
  const redacted = redactIceConfiguration({
    iceServers: [
      { urls: ['turns:secret.example.test:443'], username: 'user', credential: 'do-not-export' },
    ],
    iceTransportPolicy: 'all',
  });
  assert.deepEqual(redacted, {
    serverCount: 1,
    urls: ['turns://secret.example.test:443'],
    iceTransportPolicy: 'all',
    bundlePolicy: 'max-bundle',
    hasEphemeralCredentials: true,
  });
