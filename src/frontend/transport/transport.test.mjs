...
test('WebTransport signaling sends datagrams, routes envelopes, and supports authenticated join', async () => { let instance; const writes = []; const reader = {read: () => new Promise(resolve => { reader.resolve = resolve; }), cancel: () => reader.resolve?.({done: true})}; const writer = {write: async value => writes.push(JSON.parse(new TextDecoder().decode(value))), releaseLock() {}}; class FakeWebTransport { constructor(url) { instance = this; this.url = url; this.ready = Promise.resolve(); this.datagrams = {writable: {getWriter: () => writer}, readable: {getReader: () => reader}}; } close() { this.closed = true; } } const transport = createWebTransportSignalTransport({endpoint: 'https://relay.example.test/signal', join: {sessionId: 'ses-wt-01', role: 'client', ticket: 'ticket'}, WebTransportImpl: FakeWebTransport}); await transport.connect(); await transport.send(message); assert.deepEqual(writes, [{type: 'signaling.join', sessionId: 'ses-wt-01', role: 'client', ticket: 'ticket'}, message]); assert.equal(instance.url, 'https://relay.example.test/signal'); transport.close(); assert.equal(transport.state, 'closed'); });
