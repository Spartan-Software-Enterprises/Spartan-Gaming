...
export function normalizeTransportPolicy(policy = {}) {
  ...
  return Object.freeze({preference, icePolicy, allowWebTransport: policy.allowWebTransport !== false, allowWebSocketFallback: policy.allowWebSocketFallback !== false, credentials: 'session-scoped'});
}

export function selectSessionTransport({clientTransports = ORDER, remoteTransports = ORDER, policy = {}} = {}) {
  const normalized = normalizeTransportPolicy(policy);
  const available = ORDER.filter(transport => clientTransports.includes(transport) && remoteTransports.includes(transport) && (transport !== 'webtransport' || normalized.allowWebTransport) && (transport !== 'websocket' || normalized.allowWebSocketFallback));
...
export function createSessionTransportOptions({policy = {}, ice} = {}) {
  const normalized = normalizeTransportPolicy(policy);
  return Object.freeze({policy: normalized, ice: ice ? createIceConfiguration({ ...ice, policy: normalized.icePolicy }) : undefined});
}
