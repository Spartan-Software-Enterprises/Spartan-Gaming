...
export function normalizeIceServer(server, { allowInsecureTurn = false } = {}) {
  if (!server || typeof server !== 'object' || Array.isArray(server))
    throw new TypeError('ICE server must be an object');
  const urls = list(server.urls).map((url) => required(url, 'ICE server URL'));
  if (!urls.length || urls.length > 8)
    throw new RangeError('ICE server must contain between 1 and 8 URLs');
  for (const value of urls) {
    let parsed;
    try {
      parsed = new URL(value);
    } catch {
      throw new TypeError('ICE server URL is invalid');
    }
    if (!URL_SCHEMES.has(parsed.protocol))
      throw new TypeError(`unsupported ICE URL scheme: ${parsed.protocol}`);
    if (parsed.username || parsed.password)
      throw new TypeError('ICE credentials must not be embedded in the URL');
    if (
      (parsed.protocol === 'turn:' || parsed.protocol === 'stun:') &&
      parsed.hostname !== 'localhost' &&
      !allowInsecureTurn &&
      parsed.protocol === 'turn:'
    )
      throw new TypeError('remote TURN servers must use turns:');
  }
  const normalized = { urls: Object.freeze([...urls]) };
  if (server.username !== undefined)
    normalized.username = required(server.username, 'ICE username');
  if (server.credential !== undefined)
    normalized.credential = required(server.credential, 'ICE credential');
  if (server.credentialType !== undefined) {
    if (!['password', 'oauth'].includes(server.credentialType))
      throw new TypeError('unsupported ICE credential type');
...
export function createIceConfiguration({
  ...
  if (!Number.isInteger(poolSize) || poolSize < 0 || poolSize > sizeLimit())
    throw new RangeError('ice candidate pool size is out of bounds');
  return Object.freeze({
    iceServers: Object.freeze(
      normalized.map((server) => normalizeIceServer(server, { allowInsecureTurn })),
    ),
...
export function redactIceConfiguration(configuration = {}) {
  ...
  return Object.freeze({
    ...
    iceTransportPolicy: POLICIES.has(configuration.iceTransportPolicy)
      ? configuration.iceTransportPolicy
      : 'all',
    bundlePolicy: BUNDLES.has(configuration.bundlePolicy)
      ? configuration.bundlePolicy
      : 'max-bundle',
    hasEphemeralCredentials: servers.some((server) =>
