...
test('PWA install controller captures and prompts for a deferred install event', async () => {
  ...
  assert.deepEqual(states, ['available', 'accepted']);
});

test('PWA install controller handles installation and removes listeners', async () => {
  const windowRef = fakeWindow();
  const controller = createPwaInstallController({ windowRef });
  windowRef.listeners.get('beforeinstallprompt')({
    preventDefault() {},
    prompt() {},
    userChoice: Promise.resolve({ outcome: 'dismissed' }),
  });
  windowRef.listeners.get('appinstalled')();
  assert.equal(controller.state, 'installed');
  assert.deepEqual(await controller.prompt(), { status: 'unavailable' });
  controller.close();
