...
function standalone(windowRef) {
  try { return Boolean(windowRef?.matchMedia?.('(display-mode: standalone)')?.matches); } catch { return false; }
}
...
export function createPwaInstallController({windowRef = globalThis, onState = () => {}} = {}) {
  ...
  const onBeforeInstallPrompt = event => { event.preventDefault?.(); deferred = event; publish('available'); };
  const onInstalled = () => { deferred = null; publish('installed'); };
  ...
  return Object.freeze({
    ...
    get canInstall() { return Boolean(deferred) && state === 'available'; },
    async prompt() {
      ...
      try {
        ...
        publish('dismissed');
        return Object.freeze({status: 'dismissed', reason: error?.message || 'install prompt failed'});
      }
    },
    close() {
