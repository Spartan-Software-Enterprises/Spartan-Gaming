...
test('display policy caps refresh, disables unsupported HDR, and filters evidenced codecs', () => {
  const policy = resolveDisplayPolicy({
    ...
    capabilities: {
      graphics: { hdr: false },
      display: { count: 1, extended: false, maxRefreshRate: 144 },
...
test('display policy fails closed for invalid display selections and refresh values', () => {
  const policy = resolveDisplayPolicy({
    settings: {
      'media.display': 'Display x',
      'media.refreshRate': 'not-a-rate',
      'streaming.resolution': 'invalid',
    },
    capabilities: { display: { count: 2, maxRefreshRate: 60 } },
  });
