const RESOLUTION_LIMITS = Object.freeze({'720p': [1280, 720], '1080p': [1920, 1080], '1440p': [2560, 1440], '4K': [3840, 2160], Source: [3840, 2160]});
const CODEC_ORDER = Object.freeze({Automatic: ['av1', 'vp9', 'h264'], AV1: ['av1', 'vp9', 'h264'], VP9: ['vp9', 'h264'], 'H.264': ['h264']});
...
export function resolveDisplayPolicy({settings = {}, capabilities} = {}) {
  const resolution = RESOLUTION_LIMITS[settings['streaming.resolution']] || RESOLUTION_LIMITS['1080p'];
  const requestedRefresh = boundedRefresh(settings['media.refreshRate'] === 'Automatic' ? null : String(settings['media.refreshRate']).replace(/\s*Hz$/i, ''));
  const observedRefresh = boundedRefresh(capabilities?.display?.maxRefreshRate);
  const maxRefreshRate = observedRefresh ? Math.min(requestedRefresh || observedRefresh, observedRefresh) : requestedRefresh;
  ...
  const warnings = [];
  if (hdrRequested && hdrAvailable === false) warnings.push('HDR is enabled in settings but unavailable in this browser/display configuration.');
  if (requestedRefresh && observedRefresh && requestedRefresh > observedRefresh) warnings.push(`Refresh rate capped at the observed ${observedRefresh} Hz browser ceiling.`);
  if (display.kind === 'index' && count > 0 && display.index >= count) warnings.push('Selected display is not currently available; the browser will use its default display.');
