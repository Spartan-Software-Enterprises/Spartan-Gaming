...
} from '../adapters/manifest-registry.mjs';
...
function moduleUrl(value) {
  ...
  if (url.username || url.password)
    throw new TypeError('browser adapter entrypoints cannot contain credentials');
  return url.href;
}

function responseBytes(response) {
  if (!response || typeof response.arrayBuffer !== 'function')
    throw new Error('browser adapter response did not provide bytes');
  const declared = Number(response.headers?.get?.('content-length') || 0);
  if (declared > MAX_MODULE_BYTES)
    throw new Error(`browser adapter exceeds the ${MAX_MODULE_BYTES} byte limit`);
  return response.arrayBuffer().then((value) => {
    const result = bytes(value);
    if (result.byteLength > MAX_MODULE_BYTES)
      throw new Error(`browser adapter exceeds the ${MAX_MODULE_BYTES} byte limit`);
    return result;
  });
}
...
async function importBrowserModule(
  moduleBytes,
  { BlobImpl = globalThis.Blob, URLImpl = globalThis.URL, importModule } = {},
) {
  if (typeof importModule === 'function') return importModule(moduleBytes);
  if (typeof BlobImpl !== 'function' || typeof URLImpl?.createObjectURL !== 'function')
    throw new Error('browser module import is unavailable');
...
function adapterFromModule(module, id) {
  ...
  if (!adapter || typeof adapter !== 'object')
    throw new Error('browser adapter module did not export an adapter');
  if (adapter.id !== id)
    throw new Error(
      `browser adapter id ${adapter.id || '<missing>'} does not match manifest ${id}`,
    );
  if (
    ...
    throw new Error(`browser adapter ${id} must provide load(), start(), and stop()`);
  return adapter;
}
 ...
 */
export async function loadVerifiedBrowserEmulatorAdapter({
  manifest,
  platform = 'browser',
  trustedSigners = {},
  consent = false,
  ...
  if (consent !== true)
    throw new Error('explicit user consent is required before loading a browser adapter');
  ...
  const readiness = evaluateAdapterManifest(normalized, {
    platform,
    kind: 'emulator',
    allowUnsigned: false,
  });
  if (readiness.status !== 'ready')
    throw new Error(readiness.reason || 'browser adapter manifest is not ready');
  ...
  if (!response?.ok)
    throw new Error(`browser adapter returned HTTP ${Number(response?.status) || 0}`);
  const moduleBytes = await responseBytes(response);
  if (!subtle || typeof subtle.digest !== 'function')
    throw new Error('WebCrypto digest verification is unavailable');
