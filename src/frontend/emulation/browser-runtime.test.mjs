...
test('browser emulator runtime validates files and manages the full lifecycle', async () => {
  ...
  await runtime.load({core, gameFile: game}); assert.equal(runtime.state, 'ready'); await runtime.start(); await runtime.input({type: 'button', action: 'confirm'}); await runtime.pause(); await runtime.resume(); await runtime.reset(); const save = await runtime.saveState(); assert.deepEqual([...save], [1, 2]); const saveSource = {name: 'slot.state', size: 2, async arrayBuffer() { return new Uint8Array([1, 2]); }}; await runtime.loadState({id: 'save:slot-1', name: 'slot.state', kind: 'save', userSelected: true, source: saveSource}); assert.equal(impl.calls.at(-1)[1].source, saveSource); await runtime.stop(); assert.equal(runtime.state, 'stopped'); assert.deepEqual(impl.calls.map(call => call[0]), ['load', 'start', 'input', 'pause', 'resume', 'reset', 'save', 'loadState', 'stop']); assert.deepEqual(states, ['loading', 'ready', 'running', 'paused', 'running', 'ready', 'stopped']);
});
