...
import {clearLaunchIntent, readLaunchIntent} from '../launch/intent.mjs';
...
async function loadCores() { try { const manifest = await fetch('../../../emulators/catalog.json').then(response => response.json()); state.cores = manifest.projects; renderCores(); const intent = readLaunchIntent(sessionStorage); const core = state.cores.find(item => item.id === intent?.backendId); if (intent?.backendType === 'emulator' && core) { toast(`${core.name}: ${intent.mode} runtime selected. Add your game files to continue.`); clearLaunchIntent(sessionStorage); } } catch { coreList.innerHTML = '<p class="empty">The emulator catalog could not be loaded.</p>'; } }
