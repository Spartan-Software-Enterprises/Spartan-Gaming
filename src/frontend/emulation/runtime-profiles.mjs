...
function list(value, name, { allowEmpty = false } = {}) {
  if (
    ...
    throw new TypeError(`${name} must contain valid strings`);
...
function optionalPath(value) {
  ...
  if (
    ...
    throw new TypeError('runtime executablePath must be a local path, not a URL or control string');
  return value.trim();
}
...
function normalizeId(value) {
  ...
  if (!ID_PATTERN.test(id))
    throw new TypeError(
      'runtime.id must use lowercase letters, numbers, dot, underscore, or hyphen',
    );
  return id;
}
...
export function normalizeRuntimeProfile(record = {}) {
  ...
  const playOnLinuxProfile =
    typeof record.playOnLinuxProfile === 'string' && record.playOnLinuxProfile.trim()
      ? record.playOnLinuxProfile.trim().slice(0, 160)
      : undefined;
  if (NATIVE_KINDS.has(kind) && !executablePath)
    throw new TypeError(
      'native runtime profiles require an executablePath or adapter package path',
    );
  if (kind === 'browser-wasm' && platform !== 'browser' && platform !== 'any')
    throw new TypeError('browser-wasm profiles must target browser or any');
...
export function normalizeRuntimeProfiles(value) {
  ...
  for (const profile of profiles) {
    if (ids.has(profile.id)) throw new Error(`duplicate runtime profile: ${profile.id}`);
    ids.add(profile.id);
  }
  return Object.freeze(profiles);
}

export function createRuntimeProfileStore({
  ...
  const persist = () => {
    try {
      storage?.setItem(RUNTIME_PROFILE_KEY, JSON.stringify(profiles));
    } catch {
      /* Storage is optional in private/restricted contexts. */
    }
  };
  return Object.freeze({
    list() {
    ...
    save(record) {
      const next = normalizeRuntimeProfile(record);
      const index = profiles.findIndex((profile) => profile.id === next.id);
      if (index >= 0) profiles = profiles.with(index, next);
      else profiles = [...profiles, next].slice(-limit);
      persist();
    ...
    export() {
      return JSON.stringify({ version: RUNTIME_PROFILE_VERSION, profiles }, null, 2);
    },
    import(value) {
      const parsed = typeof value === 'string' ? JSON.parse(value) : value;
      if (parsed?.version !== RUNTIME_PROFILE_VERSION || !Array.isArray(parsed.profiles))
        throw new TypeError('runtime profile export version is unsupported');
      profiles = normalizeRuntimeProfiles(parsed.profiles).slice(0, limit);
      persist();
      return this.list();
    },
  });
...
function platformMatches(profile, platform) {
  return profile.platform === 'any' || profile.platform === platform;
}
function coreMatches(profile, coreId) {
  return profile.coreIds.includes('*') || profile.coreIds.includes(coreId);
}
...
export function resolveRuntimeProfile({
  ...
  if (preference !== 'automatic' && !preferredKinds)
    throw new TypeError(`unsupported runtime preference: ${preference}`);
  const matches = normalized.filter(
    (profile) =>
      profile.enabled &&
      profile.trust !== 'unverified' &&
      platformMatches(profile, platform) &&
      coreMatches(profile, id) &&
      (!preferredKinds || preferredKinds.has(profile.kind)),
  );
  const browserProfile = Object.freeze({
    id: 'spartan-browser-runtime',
    name: 'Spartan browser runtime',
    kind: 'browser-wasm',
    platform: 'browser',
    version: 'built-in',
    coreIds: [id],
    capabilities: ['WebAssembly', 'WebGPU/WebGL'],
    trust: 'signed',
    enabled: true,
    notes: 'Built into the browser frontend; no native executable is launched.',
  });
  if ((preference === 'automatic' || preference === 'browser-wasm') && browserReady)
    return Object.freeze({
      status: 'ready',
      source: 'built-in',
      profile: browserProfile,
      reason: 'browser runtime capability is available',
    });
  if (matches.length)
    return Object.freeze({
      status: 'ready',
      source: 'profile',
      profile: matches[0],
      reason: 'enabled trusted runtime profile matches the core and platform',
    });
  if (preference === 'browser-wasm')
    return Object.freeze({
      status: 'browser-capability-missing',
      source: 'none',
      reason: 'browser runtime was requested but required graphics capability is unavailable',
    });
  if (
    preference === 'automatic' ||
    preference === 'native-adapter' ||
    preference === 'libretro-core'
  )
    return Object.freeze({
      status: 'configuration-required',
      source: 'none',
      reason: 'no enabled trusted runtime profile matches this core and platform',
    });
  return Object.freeze({
    status: 'unsupported',
    source: 'none',
    reason: 'no runtime profile matches this core and platform',
  });
}
