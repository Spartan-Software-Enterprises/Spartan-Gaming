...
export function resolveEmulationPreferences(settings = {}) {
  ...
  return Object.freeze({preference: frontend, renderer, allowWebGpu: settings['performance.webgpu'] !== false, allowWebAssemblyThreads: settings['performance.webAssemblyThreads'] !== false, scanLibraries: settings['emulation.scanLibraries'] !== false, autoSaveStates: settings['emulation.autoSaveStates'] !== false, cloudSaves: settings['emulation.cloudSaves'] === true, saveLocation, integerScaling: settings['emulation.integerScaling'] !== false, vsync: settings['emulation.vsync'] !== false, shaderPreset, rewind: settings['emulation.rewind'] === true, netplay: settings['emulation.netplay'] === true});
}

export function createUserFileRecord(file, {kind = 'game', userSelected = true, sha256: suppliedDigest} = {}) {
  const source = file?.source || file; const name = requiredString(file?.name || source?.name, 'file.name'); if (!['game', 'firmware', 'save', 'media'].includes(kind)) throw new TypeError('unsupported emulation file kind'); const size = Math.max(0, Number(file.size ?? source?.size) || 0); const lastModified = Math.max(0, Number(file.lastModified ?? source?.lastModified) || 0); const relativePathValue = file?.relativePath || file?.webkitRelativePath; const relativePath = typeof relativePathValue === 'string' && relativePathValue.trim() ? relativePathValue.trim().slice(0, 1024) : ''; const extension = (name.match(EXTENSION)?.[1] || '').toLowerCase(); const sha256 = normalizeDigest(suppliedDigest ?? file?.sha256);
  return freezeFileRecord({id: `${kind}:${relativePath || name}:${size}:${lastModified}`, name, ...(relativePath ? {relativePath} : {}), kind, extension, size, lastModified, userSelected: Boolean(userSelected), ...(sha256 ? {sha256} : {})}, userSelected ? source : null);
}

export async function computeSelectedFileSha256(file, {cryptoImpl = globalThis.crypto, maxBytes = MAX_HASH_BYTES} = {}) {
  if (!file || file.userSelected !== true) throw new Error('file must be explicitly selected by the user');
  const source = file.source || file; const size = Number(file.size ?? source.size) || 0;
  if (!Number.isFinite(size) || size < 0 || size > maxBytes) throw new RangeError(`file is too large to hash in the browser (maximum ${maxBytes} bytes)`);
  if (!source || typeof source.arrayBuffer !== 'function') throw new TypeError('selected file bytes are unavailable');
  if (!cryptoImpl?.subtle?.digest) throw new Error('Web Crypto SHA-256 is unavailable in this browser');
...
export function createEmulationLibraryStore({storage = globalThis.localStorage, maxEntries = 200} = {}) {
  ...
  return Object.freeze({
    ...
    add(records = []) { if (!Array.isArray(records)) throw new TypeError('library records must be an array'); const remembered = records.map(record => createUserFileRecord(record, {kind: record.kind, userSelected: false, sha256: record.sha256})); files = createEmulationLibraryIndex([...remembered, ...files]).slice(0, entryLimit).map(file => Object.freeze({...file, userSelected: false})); persist(); return this.list(); },
