...
export function createUserFileRecord(file, {kind = 'game', userSelected = true, sha256: suppliedDigest} = {}) {
  const source = file?.source || file; const name = requiredString(file?.name || source?.name, 'file.name'); if (!['game', 'firmware', 'save', 'media'].includes(kind)) throw new TypeError('unsupported emulation file kind'); const size = Math.max(0, Number(file.size ?? source?.size) || 0); const lastModified = Math.max(0, Number(file.lastModified ?? source?.lastModified) || 0); const extension = (name.match(EXTENSION)?.[1] || '').toLowerCase(); const sha256 = normalizeDigest(suppliedDigest ?? file?.sha256);
  return freezeFileRecord({id: `${kind}:${name}:${size}:${lastModified}`, name, kind, extension, size, lastModified, userSelected: Boolean(userSelected), ...(sha256 ? {sha256} : {})}, userSelected ? source : null);
}
