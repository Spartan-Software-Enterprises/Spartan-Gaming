...
export function createEmulationLaunchPlan({core, gameFile, firmwareFiles = [], policy = DEFAULT_EMULATION_POLICY} = {}) {
  ...
  const game = gameFile?.id ? gameFile : createUserFileRecord(gameFile, {kind: 'game', userSelected: gameFile?.userSelected !== false}); if (!game.userSelected) throw new Error('game files must be explicitly selected by the user'); const firmware = firmwareFiles.map(file => file.id ? file : createUserFileRecord(file, {kind: 'firmware', userSelected: file?.userSelected !== false})); if (firmware.some(file => !file.userSelected)) throw new Error('firmware files must be explicitly selected by the user');
  return Object.freeze({status: 'ready', coreId: core.id, runtime: core.mode, systems: Object.freeze([...(core.systems || [])]), license: core.license, files: Object.freeze([game, ...firmware]), policy: Object.freeze({shipRoms: false, shipBios: false, allowUserSelectedFiles: true})});
}
