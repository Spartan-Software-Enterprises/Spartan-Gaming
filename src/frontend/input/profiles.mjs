...
function normalizeCapabilities(value) { const values = Array.isArray(value) ? value : []; return Object.freeze([...new Set(values.filter(item => typeof item === 'string' && PROFILE_CAPABILITIES.has(item)))].slice(0, 16)); }
...
export function createControllerProfile({id, name, deviceMatch = 'any', bindings = DEFAULT_BINDINGS, deadzone = 0.12, rumble = true, capabilities = []} = {}) {
  ...
  return Object.freeze({id: profileId, name: normalizedName, deviceMatch: required(deviceMatch, 'deviceMatch'), bindings: validateBindings(bindings), deadzone: normalizedDeadzone, rumble: Boolean(rumble), capabilities: normalizeCapabilities(capabilities)});
}
...
export const BUILTIN_CONTROLLER_PROFILES = Object.freeze([
  ...
  createControllerProfile({id: 'mobile-controller-layout', name: 'Backbone / Razer Kishi', deviceMatch: 'Backbone|Kishi', bindings: {...DEFAULT_BINDINGS}}),
  ...
  createControllerProfile({id: 'racing-wheel-layout', name: 'Racing wheel and pedals', deviceMatch: 'wheel', bindings: {accelerate: 'axis-2-positive', brake: 'axis-3-positive', steer: 'axis-0', clutch: 'axis-4-positive', handbrake: 'button-0', pause: 'button-9'}, capabilities: ['wheel', 'pedals', 'haptics']}),
  createControllerProfile({id: 'logitech-wheel-layout', name: 'Logitech racing wheel', deviceMatch: 'Logitech.*G(29|920|923)|Logitech Racing', bindings: {accelerate: 'axis-2-positive', brake: 'axis-3-positive', steer: 'axis-0', clutch: 'axis-4-positive', handbrake: 'button-0', pause: 'button-9'}, capabilities: ['wheel', 'pedals', 'haptics']}),
  createControllerProfile({id: 'thrustmaster-wheel-layout', name: 'Thrustmaster racing wheel', deviceMatch: 'Thrustmaster.*(T150|T300|TX|TS)|Thrustmaster', bindings: {accelerate: 'axis-2-positive', brake: 'axis-3-positive', steer: 'axis-0', clutch: 'axis-4-positive', handbrake: 'button-0', pause: 'button-9'}, capabilities: ['wheel', 'pedals', 'haptics']}),
