...
import {BUILTIN_CONTROLLER_PROFILES, CONTROLLER_PROFILE_CAPABILITIES, createControllerProfile, createControllerProfileStore, resolveControllerProfile, updateControllerBinding, validateBindings} from './profiles.mjs';
...
test('built-in profiles declare bounded device capabilities alongside bindings', () => { const dualsense = BUILTIN_CONTROLLER_PROFILES.find(profile => profile.id === 'dualsense-layout'); const wheel = BUILTIN_CONTROLLER_PROFILES.find(profile => profile.id === 'logitech-wheel-layout'); assert.deepEqual(dualsense.capabilities, ['touchpad', 'gyro', 'adaptiveTriggers', 'haptics', 'battery']); assert.deepEqual(wheel.capabilities, ['wheel', 'pedals', 'haptics']); assert.equal(Object.isFrozen(dualsense.capabilities), true); });
test('profile capability editor vocabulary is bounded and immutable', () => { assert.ok(CONTROLLER_PROFILE_CAPABILITIES.includes('adaptiveTriggers')); assert.ok(CONTROLLER_PROFILE_CAPABILITIES.includes('wheel')); assert.equal(Object.isFrozen(CONTROLLER_PROFILE_CAPABILITIES), true); });

test('automatic profile selection recognizes common console controller names', () => {
  assert.equal(resolveControllerProfile({deviceName: 'Xbox Wireless Controller'}).id, 'xbox-layout');
  assert.equal(resolveControllerProfile({deviceName: 'Sony Wireless Controller'}).id, 'dualsense-layout');
  assert.equal(resolveControllerProfile({deviceName: 'Nintendo Switch Pro Controller'}).id, 'switch-pro-layout');
  assert.equal(resolveControllerProfile({deviceName: '8BitDo Ultimate 2C'}).id, 'eightbitdo-layout');
});
...
test('profile store exports capabilities and filters unknown capability labels', () => { const values = new Map(); const storage = {getItem: key => values.get(key) || null, setItem: (key, value) => values.set(key, value)}; const store = createControllerProfileStore({storage}); store.save({id: 'wheel', name: 'Wheel', capabilities: ['wheel', 'pedals', 'driver-secret']}); const exported = store.export(); assert.deepEqual(JSON.parse(exported).profiles[0].capabilities, ['wheel', 'pedals']); const restored = createControllerProfileStore({storage: {getItem: key => values.get(`restored:${key}`) || null, setItem: (key, value) => values.set(`restored:${key}`, value)}}); const profiles = restored.import(exported); assert.deepEqual(profiles[0].capabilities, ['wheel', 'pedals']); });
...
test('automatic profile selection recognizes adaptive, handheld, wheel, and HOTAS devices', () => {
  assert.equal(resolveControllerProfile({profileId: 'auto', deviceName: 'Xbox Adaptive Controller'}).id, 'xbox-adaptive-layout');
  assert.equal(resolveControllerProfile({profileId: 'auto', deviceName: 'Backbone One'}).id, 'mobile-controller-layout');
