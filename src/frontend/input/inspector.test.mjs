...
test('controller preferences normalize explicit HID and advanced controls', () => { assert.deepEqual(resolveControllerPreferences({'controllers.allowHid': true, 'controllers.adaptiveTriggers': true, 'controllers.gyro': true, 'controllers.inputLatency': 'High frequency'}), {allowHid: true, adaptiveTriggers: true, gyro: true, inputPolling: 'High frequency'}); assert.equal(resolveControllerPreferences().allowHid, false); });
