...
export function createInputMapper({bindings = DEFAULT_BINDINGS, deadzone = 0.12} = {}) {
  ...
  return Object.freeze({
    ...
    mapNativeButton(index, pressed, value = pressed ? 1 : 0) { return Number.isInteger(index) && index >= 0 && index <= 31 ? {type: 'input.event', action: `button-${index}`, kind: 'button', pressed: Boolean(pressed), value: clamp(value, 0, 1)} : null; },
