...
export const REMOTE_INPUT_KINDS = Object.freeze([
  'button',
  'axis',
  'key',
  'pointer',
...
export const REMOTE_INPUT_SOURCES = Object.freeze([
  'gamepad',
  'keyboard',
  'pointer',
  'touch',
...
export function normalizeGamepadState(gamepad, { deadzone = 0.12 } = {}) {
  if (
    ...
    !gamepad.buttons ||
    !gamepad.axes ||
    typeof gamepad.buttons.length !== 'number' ||
    typeof gamepad.axes.length !== 'number'
  )
    throw new TypeError('A Gamepad-like value is required');
  const buttons = Array.from(gamepad.buttons);
  const axes = Array.from(gamepad.axes);
  return Object.freeze({
    id: String(gamepad.id || 'unknown'),
...
export function createInputMapper({ bindings = DEFAULT_BINDINGS, deadzone = 0.12 } = {}) {
  ...
  const normalizedAxisDirection = (value) =>
    value < 0 ? 'negative' : value > 0 ? 'positive' : null;
  return Object.freeze({
    bindings: activeBindings,
    normalize: (gamepad) => normalizeGamepadState(gamepad, { deadzone }),
    actionFor(control) {
      return Object.keys(activeBindings).find((action) => activeBindings[action] === control);
    },
    controlsFor(action) {
      return activeBindings[action] ? [activeBindings[action]] : [];
    },
    mapButton(index, pressed, value = pressed ? 1 : 0) {
      const action = this.actionFor(`button-${index}`);
      return action
        ? { type: 'input.event', action, pressed: Boolean(pressed), value: clamp(value, 0, 1) }
        : null;
    },
    mapAxis(index, value) {
      const normalized = applyDeadzone(clamp(value), deadzone);
      const direction = normalized < 0 ? 'negative' : 'positive';
      const action = this.actionFor(`axis-${index}-${direction}`);
      return action && normalized !== 0
        ? { type: 'input.event', action, kind: 'axis', pressed: true, value: normalized }
        : null;
    },
    mapNativeButton(index, pressed, value = pressed ? 1 : 0) {
      return Number.isInteger(index) && index >= 0 && index <= 31
        ? {
            type: 'input.event',
            action: `button-${index}`,
            kind: 'button',
    ...
    mapNativeAxis(index, value, previousValue = 0) {
      if (!Number.isInteger(index) || index < 0 || index > 5) return [];
      const current = clamp(value);
      const previous = clamp(previousValue);
    ...
    mapAxisTransition(index, value, previousValue = 0) {
      const current = clamp(value);
      const previous = clamp(previousValue);
      const currentDirection = normalizedAxisDirection(current);
      const previousDirection = normalizedAxisDirection(previous);
      const events = [];
      if (previousDirection && previousDirection !== currentDirection) {
        const action = this.actionFor(`axis-${index}-${previousDirection}`);
        if (action)
          events.push({ type: 'input.event', action, kind: 'axis', pressed: false, value: 0 });
      }
      if (currentDirection) {
        const action = this.actionFor(`axis-${index}-${currentDirection}`);
        if (action)
          events.push({ type: 'input.event', action, kind: 'axis', pressed: true, value: current });
      }
      return events;
    },
  });
...
export function createInputEventEnvelope({
  sessionId,
  event,
  sequence = 0,
  ...
  if (typeof event.action !== 'string' || !event.action)
    throw new TypeError('input event action is required');
...
export function normalizeRemoteInputEvent(event) {
  if (!event || event.type !== 'input.event') throw new TypeError('event must be an input.event');
  if (typeof event.action !== 'string' || !event.action || event.action.length > 64)
    throw new TypeError('input event action is required');
  const source = REMOTE_INPUT_SOURCES.includes(event.source) ? event.source : 'gamepad';
  const kind = REMOTE_INPUT_KINDS.includes(event.kind)
    ? event.kind
    : source === 'keyboard'
      ? 'key'
      : source === 'pointer'
