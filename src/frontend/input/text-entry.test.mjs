...
import {normalizeTextEntryRequest, normalizeTextEntryState, TEXT_ENTRY_MAX_LENGTH} from './text-entry.mjs';

test('text entry preserves bounded selection and composing ranges', () => {
  assert.deepEqual(normalizeTextEntryState({text: 'hel', selectionStart: 3, selectionEnd: 1, composingStart: 0, composingEnd: 2}), {text: 'hel', selectionStart: 1, selectionEnd: 3, composingStart: 0, composingEnd: 2});
});
...
test('text entry clamps indexes and rejects native-unsafe payloads', () => {
  ...
  assert.throws(() => normalizeTextEntryState({text: 'x'.repeat(TEXT_ENTRY_MAX_LENGTH + 1)}), /too large/);
  assert.throws(() => normalizeTextEntryState({text: 'abc', composingStart: 2, composingEnd: 1}), /composition/);
});
...
test('showing the IME is fail-closed until a focused user gesture exists', () => {
  ...
  const request = normalizeTextEntryRequest({action: 'show', userInitiated: true, hasFocus: true, state: {text: 'hi', selectionStart: 2}});
