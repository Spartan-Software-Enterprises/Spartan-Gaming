...
export function createInputPermissionPolicy(capabilities = {}) {
  ...
  const allowed = Object.freeze({
    ...
    pointer: input.pointer !== false,
    touch: input.touch !== false,
    hid: input.hid === true,
    rumble: input.rumble !== false,
    host: false,
  });
  return Object.freeze({
    allowed,
    allows(source) {
      return allowed[SOURCES[source] || source] === true;
    },
    reason(source) {
      return this.allows(source)
        ? null
        : `${source} input is disabled by the active session policy`;
    },
  });
}
