...
export function normalizeTextEntryState({text = '', selectionStart = 0, selectionEnd = selectionStart, composingStart = -1, composingEnd = -1} = {}) {
  if (typeof text !== 'string' || text.length > MAX_TEXT_LENGTH || text.includes('\u0000')) throw new TypeError('text entry text is invalid or too large');
  ...
  if (hasComposition && (compositionStart < 0 || compositionEnd < compositionStart)) throw new TypeError('text entry composition range is invalid');
  return Object.freeze({text, selectionStart: Math.min(start, end), selectionEnd: Math.max(start, end), composingStart: compositionStart, composingEnd: compositionEnd});
}

/** Create a platform-neutral request; showing an IME always requires a fresh user gesture. */
export function normalizeTextEntryRequest({action = 'update', userInitiated = false, hasFocus = false, state} = {}) {
  ...
  return Object.freeze({version: 1, kind: 'text-entry.request', action, allowed, userInitiated: userInitiated === true, hasFocus: hasFocus === true, ime: 'android-game-text-input', state: normalizedState});
}
