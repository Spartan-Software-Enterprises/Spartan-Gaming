...
test('sticky keys releases a latched modifier after the next key chord', () => {
  ...
  assert.deepEqual(controller.process('KeyA', false), [{code: 'KeyA', pressed: false}, {code: 'ShiftLeft', pressed: false}]);
...
test('sticky keys can toggle a modifier off and flush safely', () => {
  ...
  assert.deepEqual(controller.process('ControlLeft', true), [{code: 'ControlLeft', pressed: false}]);
  assert.deepEqual(controller.flush(), []);
});
