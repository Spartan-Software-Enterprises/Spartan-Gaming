...
export function normalizePointerCoordinates({clientX = 0, clientY = 0, rect = {left: 0, top: 0, width: 1, height: 1}} = {}) {
  return Object.freeze({x: clamp((clientX - rect.left) / Math.max(1, rect.width), 0, 1), y: clamp((clientY - rect.top) / Math.max(1, rect.height), 0, 1)});
}

export function createPointerInputEvent({event, rect} = {}) {
  if (!event || !['pointerdown', 'pointermove', 'pointerup', 'pointercancel'].includes(event.type)) throw new TypeError('A supported pointer event is required');
