...
test('native passthrough forwards standard unbound gamepad buttons with their native control', () => { const mapper = createInputMapper({bindings: {confirm: 'button-0', cancel: 'button-1'}}); assert.deepEqual(mapper.mapNativeButton(12, true), {type: 'input.event', action: 'button-12', kind: 'button', pressed: true, value: 1}); assert.deepEqual(mapper.mapNativeButton(12, false), {type: 'input.event', action: 'button-12', kind: 'button', pressed: false, value: 0}); assert.equal(mapper.mapNativeButton(16, true), null); });
