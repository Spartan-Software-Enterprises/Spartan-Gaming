const MODIFIER_CODES = Object.freeze(new Set(['AltLeft', 'AltRight', 'ControlLeft', 'ControlRight', 'MetaLeft', 'MetaRight', 'ShiftLeft', 'ShiftRight']));
...
export function createStickyKeysController({enabled = false} = {}) {
  ...
  return Object.freeze({
    process(code, pressed) {
      ...
      if (!pressed) return [];
      if (latched.has(code)) { latched.delete(code); return [{code, pressed: false}]; }
      latched.add(code);
