...
function renderCapabilities() { const capabilities = detectInputCapabilities(); capabilityElement.innerHTML = Object.entries(capabilities).map(([name, supported]) => `<span class="capability"><strong>${supported ? '✓' : '—'}</strong> ${escapeHtml(name)}</span>`).join(''); }
function renderDevices() { const list = gamepads(); if (selectedIndex === null && list[0]) selectedIndex = list[0].index; if (!list.length) { devicesElement.className = 'empty'; devicesElement.textContent = 'No gamepads detected. Connect one and press a button to wake it.'; return; } devicesElement.className = ''; devicesElement.innerHTML = list.map(gamepad => `<button class="device ${gamepad.index === selectedIndex ? 'active' : ''}" data-device="${gamepad.index}"><strong>${escapeHtml(gamepad.id || 'Unknown controller')}</strong><small>Index ${gamepad.index} · ${escapeHtml(gamepad.mapping || 'unmapped')} mapping</small></button>`).join(''); }
function renderSnapshot() { const gamepad = gamepads().find(item => item.index === selectedIndex); if (!gamepad) { selectedElement.textContent = 'No controller selected'; profileElement.textContent = 'No matching profile'; liveElement.textContent = 'WAITING'; liveElement.classList.remove('active'); axesElement.innerHTML = ''; buttonsElement.innerHTML = ''; detailsElement.innerHTML = ''; return; } const snapshot = inspectGamepad(gamepad); const profile = resolveControllerProfile({profileId: controllerPreferences.defaultProfile, deviceName: snapshot.id}); selectedElement.textContent = snapshot.id; profileElement.textContent = profile ? `${profile.name} · ${profile.capabilities.length ? profile.capabilities.join(' · ') : 'Runtime-detected features'}` : 'No matching profile'; liveElement.textContent = 'LIVE'; liveElement.classList.add('active'); axesElement.innerHTML = snapshot.axes.map((value, index) => `<div class="axis"><div class="axis-head"><span>Axis ${index}</span><strong>${value.toFixed(2)}</strong></div><div class="bar"><div class="fill" style="width:${Math.abs(value) * 100}%;transform:translateX(${value < 0 ? '-100%' : '0'})"></div></div></div>`).join(''); buttonsElement.innerHTML = snapshot.buttons.map((button, index) => `<div class="button-state ${button.pressed ? 'pressed' : ''}">${index}<br>${button.value.toFixed(1)}</div>`).join(''); detailsElement.innerHTML = `<div><dt>Mapping</dt><dd>${escapeHtml(snapshot.mapping)}</dd></div><div><dt>Haptics</dt><dd>${snapshot.haptics ? 'Available' : 'Unavailable'}</dd></div><div><dt>Battery</dt><dd>${snapshot.batteryLevel === null ? 'Unknown' : `${Math.round(snapshot.batteryLevel * 100)}%`}</dd></div>`; }
async function renderHid() { if (!controllerPreferences.allowHid) { hidList.className = 'empty'; hidList.textContent = 'HID access is disabled in Controller settings.'; return; } try { const devices = await listApprovedHidDevices(); hidList.className = devices.length ? '' : 'empty'; hidList.innerHTML = devices.length ? devices.map(device => `<div class="device"><strong>${escapeHtml(device.productName)}</strong><small>Vendor ${device.vendorId} · Product ${device.productId} · ${device.opened ? 'Open' : 'Closed'}</small></div>`).join('') : 'No approved HID devices.'; } catch (error) { hidList.className = 'empty'; hidList.textContent = error.message; } }
...
window.addEventListener('gamepadconnected', event => { selectedIndex = event.gamepad.index; renderDevices(); renderSnapshot(); showNotice('Controller connected'); });
