...
test('device mode detection distinguishes television, ChromeOS, handheld, mobile, and desktop signals', () => {
  ...
  assert.equal(
    detectDeviceMode({ userAgent: 'Mozilla/5.0 Android', viewport: { width: 900 } }),
    'handheld',
  );
  assert.equal(
    detectDeviceMode({ userAgent: 'Mozilla/5.0 iPhone', viewport: { width: 390 } }),
    'mobile',
  );
  assert.equal(
    detectDeviceMode({
...
test('presentation profiles remain explicit and settings can force a target mode', () => {
  assert.equal(
    resolveDeviceMode({
      settings: { 'appearance.deviceMode': 'Television' },
  ...
  assert.equal(
    resolveDeviceMode({
      settings: { 'appearance.deviceMode': 'Automatic' },
      detectedMode: 'mobile',
    }),
    'mobile',
  );
  const profile = resolvePresentationProfile({
    settings: { 'appearance.uiScale': 100 },
    detectedMode: 'television',
    viewport: { width: 1920 },
  });
...
test('invalid device signals fail closed to desktop', () => {
  assert.equal(
    resolveDeviceMode({
      settings: { 'appearance.deviceMode': 'invalid' },
      detectedMode: 'unknown',
    }),
    'desktop',
  );
  assert.equal(
    resolvePresentationProfile({ settings: {}, detectedMode: 'unknown' }).mode,
    'desktop',
  );
});
