...
function touchPointsOf(navigatorRef) {
...
export function detectRuntimePlatform({
  ...
  const platform = String(
    navigatorRef?.userAgentData?.platform || navigatorRef?.platform || '',
  ).toLowerCase();
...
export function resolvePlatformCompatibility({
  platform = 'unknown',
  distribution = 'web',
  alternativeEngineEntitled = false,
  capabilities = {},
} = {}) {
  ...
  const gates = {
    ...
    browserEmulation:
      capabilities.graphics?.webgl === true || capabilities.graphics?.webgpu === true,
  ...
  if (tvFamily)
    limitations.push(
      'Fire TV and Roku use the shared web/TV surface; native packaging, store certification, and device-specific media APIs require separate adapters.',
    );
  if (tvFamily && capabilities.transports?.webrtc === false)
    limitations.push(
      'This TV browser does not expose WebRTC, so remote streaming is unavailable in the current session.',
    );
  return Object.freeze({
    platform: selected,
    distribution: text(distribution) || 'web',
    enginePolicy,
