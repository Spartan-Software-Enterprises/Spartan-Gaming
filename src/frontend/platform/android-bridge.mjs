const ACTIONS = Object.freeze({
  ...
  gameMode: 'android.game-mode.query',
  controllers: 'android.controllers.snapshot',
  textInput: 'android.text-input',
  gameNative: 'android.gamenative.launch',
});
...
function encode(requestId, action, payload = {}) {
  if (!Object.values(ACTIONS).includes(action))
    throw new TypeError('unsupported Android bridge action');
  ...
  if (messageSize(message) > MAX_MESSAGE_BYTES)
    throw new RangeError('Android bridge message exceeds the size limit');
  return message;
}
...
export function createAndroidBridge({
  bridge = globalThis.SpartanAndroid,
  target = globalThis,
  idFactory,
} = {}) {
  const postMessage =
    typeof bridge?.postMessage === 'function' ? bridge.postMessage.bind(bridge) : null;
  ...
  const nextRequestId = () => {
    const candidate =
      typeof idFactory === 'function'
        ? idFactory()
        : `and-${Date.now().toString(36)}-${(++sequence).toString(36)}`;
  ...
  const send = (action, payload = {}) => {
    ...
    try {
      ...
      return Object.freeze({
        ...
        action,
        requestId,
        ...(typeof result === 'boolean' ? { accepted: result } : {}),
      });
    } catch (error) {
      return Object.freeze({
        status: 'rejected',
        action,
        requestId,
        reason: boundedText(error?.message, 'bridge rejected message'),
      });
    }
  };
  const listen = (onResult) => {
    if (typeof onResult !== 'function' || typeof target?.addEventListener !== 'function')
      return () => {};
  ...
  return Object.freeze({
    ...
    listen,
    applyPolicy(policy) {
      return send(ACTIONS.policy, policy);
    },
    queryGameMode() {
      return send(ACTIONS.gameMode);
    },
    requestControllerInventory() {
      return send(ACTIONS.controllers);
    },
    requestTextInput(request) {
      return send(ACTIONS.textInput, request);
    },
    launchGameNative({ appId, store } = {}) {
      ...
      if (
        ...
        numericId <= 0 ||
        !GAME_NATIVE_STORES.has(normalizedStore)
      )
        return Object.freeze({
          status: 'rejected',
          action: ACTIONS.gameNative,
          reason: 'GameNative requires a positive library app ID and supported store',
        });
