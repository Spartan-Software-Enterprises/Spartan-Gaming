...
export function createBrowserCaptureConstraints({
  width = 1920,
  height = 1080,
  framerate = 60,
  audio = false,
  ...
  const surface = ['monitor', 'window', 'browser'].includes(displaySurface)
    ? displaySurface
    : 'monitor';
  return Object.freeze({
    video: Object.freeze({
      width: Object.freeze({ ideal: bounded(width, 1920, MAX.width) }),
      height: Object.freeze({ ideal: bounded(height, 1080, MAX.height) }),
      frameRate: Object.freeze({ ideal: bounded(framerate, 60, MAX.framerate) }),
...
export function normalizeBrowserQualityRequest({
  profile = DEFAULT_QUALITY.profile,
  maxWidth = DEFAULT_QUALITY.maxWidth,
  maxHeight = DEFAULT_QUALITY.maxHeight,
  bitrateKbps = DEFAULT_QUALITY.bitrateKbps,
  maxFramerate = DEFAULT_QUALITY.maxFramerate,
} = {}) {
  return Object.freeze({
    profile:
      typeof profile === 'string' && profile.trim() ? profile.trim() : DEFAULT_QUALITY.profile,
    maxWidth: boundedInteger(maxWidth, DEFAULT_QUALITY.maxWidth, 320, MAX.width),
    maxHeight: boundedInteger(maxHeight, DEFAULT_QUALITY.maxHeight, 180, MAX.height),
    bitrateKbps: boundedInteger(bitrateKbps, DEFAULT_QUALITY.bitrateKbps, 250, MAX.bitrateKbps),
    maxFramerate: boundedInteger(maxFramerate, DEFAULT_QUALITY.maxFramerate, 1, MAX.framerate),
  });
}
...
export function createVideoEncodingParameters(request, parameters = {}, trackSettings = {}) {
  ...
  const hasTrackDimensions =
    Number.isFinite(trackWidth) &&
    trackWidth > 0 &&
    Number.isFinite(trackHeight) &&
    trackHeight > 0;
  const scale = hasTrackDimensions
    ? Math.min(
        64,
        Math.max(1, Math.max(trackWidth / quality.maxWidth, trackHeight / quality.maxHeight)),
      )
    : null;
  return {
    ...parameters,
    encodings: parameters.encodings.map((encoding) => ({
      ...
      ...(scale ? { scaleResolutionDownBy: Number(scale.toFixed(3)) } : {}),
    })),
  };
}

export function createBrowserWebRtcPublisher({
  ...
  return Object.freeze({
    ...
    async capture(options = {}) {
      if (!mediaDevices?.getDisplayMedia)
        throw new Error('Display capture is unavailable in this browser');
      ...
      if (options.microphone === true) {
        ...
        try {
          ...
          if (!microphoneTracks.length || typeof stream.addTrack !== 'function')
            throw new Error('Microphone capture returned no usable audio track');
      ...
      for (const track of stream.getTracks()) {
        ...
        track.addEventListener?.('ended', () => bus.emit('capture.ended'));
      }
      state = 'capturing';
      bus.emit('capture', stream);
      return stream;
    },
    async acceptOffer(offer) {
      ...
      state = 'connected';
      bus.emit('state', state);
      return peer.localDescription || answer;
    },
    async applyQualityRequest(request = {}) {
      ...
      for (const sender of senders || []) {
        if (
          ...
          continue;
        try {
          const parameters = createVideoEncodingParameters(
            quality,
