...
export function normalizeAudioInputDevice(device, index = 0) {
  ...
  return Object.freeze({deviceId, label: text(device.label) || `Microphone ${index + 1}`, kind: 'audioinput'});
}
...
export async function listAudioInputDevices({mediaDevices = globalThis.navigator?.mediaDevices} = {}) {
  ...
  const seen = new Set();
  return Object.freeze(devices.map((device, index) => normalizeAudioInputDevice(device, index)).filter(device => device && !seen.has(device.deviceId) && seen.add(device.deviceId)).slice(0, 32));
}
