...
editor.addEventListener('click', event => {
  ...
  if (event.target.closest('[data-connect]')) { const code = editor.querySelector('.pairing-code')?.textContent; if (!code) return; try { const endpoint = new URL(host.endpoint); endpoint.protocol = endpoint.protocol === 'https:' ? 'wss:' : endpoint.protocol === 'http:' ? 'ws:' : endpoint.protocol; sessionStorage.setItem('spartan-gaming.pending-host-pair.v1', JSON.stringify({endpoint: endpoint.toString(), hostId: host.hostId, pairingCode: code, name: host.name})); window.location.assign('../player/index.html?backend=spartan-host'); } catch (error) { showNotice(error.message); } }
});
