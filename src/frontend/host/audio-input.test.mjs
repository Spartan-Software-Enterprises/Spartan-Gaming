...
test('microphone device normalization filters and deduplicates input devices', async () => {
  ...
  const devices = await listAudioInputDevices({mediaDevices: {enumerateDevices: async () => [
  ...
  assert.deepEqual(devices.map(device => [device.deviceId, device.label]), [['default', 'Microphone 1'], ['mic-1', 'Desk mic']]);
});
