...
import '../profiles/storage.test.mjs';

test('resume presentation defaults to the host session and describes recent backends', () => {
  assert.deepEqual(resolveResumePresentation(), {
    title: 'Desktop stream',
    copy: 'Pick up where you left off with your Spartan Host session.',
    actionLabel: '▶ Resume session',
  });
  assert.deepEqual(
    resolveResumePresentation({ name: 'PCSX2', backendType: 'emulator', action: 'choose-runtime' }),
    {
      title: 'PCSX2',
      copy: 'Continue with your last emulation connection.',
      actionLabel: 'Configure runtime',
    },
  );
  assert.equal(
    resolveResumePresentation({ name: 'GeForce NOW', backendType: 'provider', action: 'open-url' })
      .actionLabel,
    'Open service',
  );
});
