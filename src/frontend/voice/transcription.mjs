...
const TRANSCRIPTION_LANGUAGES = Object.freeze(['automatic', 'en', 'es', 'fr', 'de', 'ja', 'ko', 'zh']);
const MAX_TRANSCRIPT_SEGMENTS = 500;
const MAX_SEGMENT_LENGTH = 4096;

export function normalizeTranscriptionPreferences(raw) {
  if (!raw || typeof raw !== 'object') return { mode: 'off', language: 'automatic', showOverlay: false };
...
export function resolveTranscriptionReadiness({ preferences, sessionActive, microphonePermission, adapterAvailable }) {
  const prefs = normalizeTranscriptionPreferences(preferences);
  if (prefs.mode === 'off') return { status: 'disabled', reason: 'transcription-mode-off', ...prefs };
  if (!sessionActive) return { status: 'unavailable', reason: 'no-active-session', ...prefs };
  if (!microphonePermission) return { status: 'unavailable', reason: 'microphone-permission-missing', ...prefs };
  if (prefs.mode === 'adapter' && !adapterAvailable) return { status: 'unavailable', reason: 'official-transcription-adapter-required', ...prefs };
...
export function appendTranscriptSegment(buffer, segment) {
  ...
  return next.length > MAX_TRANSCRIPT_SEGMENTS ? next.slice(next.length - MAX_TRANSCRIPT_SEGMENTS) : next;
}

export function exportRedactedTranscript(buffer) {
  if (!Array.isArray(buffer) || buffer.length === 0) return { segments: [], count: 0, redacted: true };
