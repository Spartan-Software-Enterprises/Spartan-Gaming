...
assert.equal(
  storage.getItem('spartan-gaming.profile.family.spartan-gaming.providers.v1'),
  'family-data',
);
assert.equal(
  createProfileStorage({ storage, profileId: 'guest' }).getItem('spartan-gaming.providers.v1'),
  null,
);
...
assert.equal(
  storage.getItem('spartan-gaming.profile.gaming.spartan-gaming.launch-history.v1'),
  '["legacy"]',
);
