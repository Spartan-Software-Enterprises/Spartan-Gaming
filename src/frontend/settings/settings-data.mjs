...
export const settingsCategories = [
  ...
  },
  {
    id: 'appearance', label: 'Appearance', icon: '◐', description: 'Theme, layout, density, and fullscreen presentation.', settings: [
      ...
      toggle('appearance.consoleMode', 'Console mode', 'Use a controller-first, fullscreen-friendly dashboard presentation with larger focus targets.', false),
  ...
  },
  {
    id: 'controllers', label: 'Controllers', icon: '⌁', description: 'Gamepads, HID devices, mappings, and rumble.', settings: [
      ...
      text('controllers.virtualGamepadPackage', 'Virtual gamepad driver package', 'Optional installed host adapter package for Windows or macOS virtual-controller injection. Package names are passed to the user-owned host only; no driver is downloaded automatically.', ''),
      text('controllers.virtualGamepadDevice', 'Virtual gamepad device ID', 'Optional driver-specific device identifier for multi-device or multi-controller setups. Leave blank for automatic selection.', ''),
      text('controllers.virtualGamepadDevices', 'Virtual gamepad device IDs', 'Optional comma-separated device IDs mapped to controller slots 1–8. Leave blank to use the single device ID or automatic selection.', ''),
      select('controllers.hapticsBackend', 'Haptics backend', 'Choose browser or native rumble output for supported controllers.', ['Automatic', 'Browser vibration', 'Native rumble', 'Disabled'], 'Automatic'),
      ...
      toggle('controllers.touchpad', 'Enable touchpads', 'Forward DualSense, Steam Deck, and compatible touchpad gestures when supported.'),
      toggle('controllers.trackpads', 'Enable trackpads', 'Forward Steam Deck and Steam Controller trackpad motion/clicks when supported.'),
  ...
  },
  {
    id: 'host', label: 'Self-hosted play', icon: '⌂', description: 'Capture, encoding, pairing, wake, and input settings for user-owned hosts.', settings: [
      ...
      toggle('host.protonEnabled', 'Enable Proton launches', 'Allow the user-owned Linux/SteamOS host to launch Windows games through an explicitly selected Proton installation.', false),
      text('host.protonPath', 'Proton executable path', 'Host-local path to the selected Proton installation executable; Spartan never downloads or bundles Proton.', ''),
      text('host.protonVersion', 'Proton version label', 'Optional version label used in host readiness and launch diagnostics.', ''),
      text('host.protonCompatDataPath', 'Proton compatibility prefix', 'Host-local STEAM_COMPAT_DATA_PATH directory for the selected game; keep it outside the repository.', ''),
      text('host.protonSteamClientPath', 'Steam client path', 'Optional host-local Steam installation path used by Proton for user-owned Steam integration.', ''),
      text('host.protonEnvironment', 'Proton runtime options', 'Optional JSON object containing allow-listed Proton options such as PROTON_LOG or PROTON_USE_WINED3D; arbitrary environment variables are rejected.', ''),
      toggle('host.steamOsEnabled', 'SteamOS host profile', 'Declare that the selected Linux host is an explicitly detected SteamOS environment; generic Linux hosts remain generic Linux.', false),
      select('host.steamOsMode', 'SteamOS session mode', 'Choose the intended SteamOS session surface for diagnostics and launch planning.', ['Game Mode', 'Desktop Mode'], 'Game Mode'),
      ...
      select('host.audioBackend', 'Host audio backend', 'Audio API selected by the user-owned host.', ['Automatic', 'PipeWire', 'PulseAudio', 'WASAPI', 'CoreAudio'], 'Automatic'),
      ...
      action('host.exportConfig', 'Export host configuration', 'Create a portable host.json from host and controller settings without pairing codes or credentials.', 'Export host.json'),
