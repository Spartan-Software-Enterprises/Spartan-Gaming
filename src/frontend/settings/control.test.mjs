...
test('settings control renderer escapes imported text and option values', () => {
  ...
  const control = renderSettingControl(
    { key: 'advanced.customSignalingUrl', type: 'text' },
...
test('settings control renderer preserves safe control semantics', () => {
  const control = renderSettingControl(
    {
      key: 'controllers.inputMode',
      type: 'select',
      options: ['Auto-detect', 'XInput'],
      default: 'Auto-detect',
    },
    'XInput',
  );
...
test('settings control renderer labels stable custom select values without changing their value', () => {
  const control = renderSettingControl(
    {
      key: 'controllers.defaultProfile',
      type: 'select',
      options: ['custom-living-room'],
      optionLabels: { 'custom-living-room': 'Living room pad (custom)' },
    },
    'custom-living-room',
  );
