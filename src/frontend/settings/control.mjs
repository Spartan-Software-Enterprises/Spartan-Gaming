...
export function renderSettingControl(setting, value) {
  ...
  if (setting.type === 'toggle')
    return `<button class="toggle ${value ? 'is-on' : ''}" role="switch" aria-label="${escapeHtml(setting.label)}" aria-checked="${Boolean(value)}" data-key="${key}"><span></span><b>${value ? 'On' : 'Off'}</b></button>`;
  if (setting.type === 'select')
    return `<select aria-label="${escapeHtml(setting.label)}" data-key="${key}">${setting.options.map((option) => `<option value="${escapeHtml(option)}"${option === value ? ' selected' : ''}>${escapeHtml(setting.optionLabels?.[option] || option)}</option>`).join('')}</select>`;
