...
export function renderSettingControl(setting, value) {
  ...
  if (setting.type === 'select') return `<select data-key="${key}">${setting.options.map(option => `<option value="${escapeHtml(option)}"${option === value ? ' selected' : ''}>${escapeHtml(option)}</option>`).join('')}</select>`;
