...
test('session runtime forwards reconnect and input envelopes through signaling', async () => {
  ...
  signaling.emit('message', createSessionEnvelope({sessionId: offer.sessionId, type: 'session.answer', payload: {accepted: true}}));
  const reconnect = await runtime.requestReconnect(); runtime.send(createSessionEnvelope({sessionId: offer.sessionId, type: 'input.event', payload: {action: 'confirm', pressed: true, value: 1}}));
...
test('session runtime reconnects signaling before sending a cloud-session recovery envelope', async () => {
  const signaling = fakeTransport(); let connections = 0; signaling.connect = async function () { connections += 1; this.state = 'open'; };
  const runtime = createSessionRuntime({signaling}); const offer = await runtime.start({backend: {id: 'spartan-host'}});
  signaling.emit('message', createSessionEnvelope({sessionId: offer.sessionId, type: 'session.answer', messageId: 'msg-reconnect-answer-01', payload: {accepted: true}})); signaling.state = 'closed';
  const reconnect = await runtime.requestReconnect(); assert.equal(connections, 2); assert.equal(reconnect.type, 'session.reconnect'); assert.equal(signaling.sent.at(-1).type, 'session.reconnect');
});
...
test('session runtime accepts WebRTC answers and emits media streams', async () => {
  ...
  await new Promise(resolve => setImmediate(resolve)); media.emit('track', {streams: ['stream-01']}); assert.equal(media.answer.sdp, 'host-answer'); assert.deepEqual(streams, ['stream-01']);
});

test('session runtime accepts validated messages from a host-created data channel', async () => {
  const signaling = fakeTransport(); const listeners = new Map();
  ...
  const runtime = createSessionRuntime({signaling, media}); const offer = await runtime.start({backend: {id: 'spartan-host'}}); const channel = {};
  media.emit('datachannel', channel);
  channel.onmessage({data: JSON.stringify(createSessionEnvelope({sessionId: offer.sessionId, type: 'session.answer', messageId: 'msg-host-channel-01', payload: {accepted: true}}))});
  assert.equal(runtime.state, 'connected');
});

test('session runtime queues remote ICE candidates until the answer SDP is installed', async () => {
  const signaling = fakeTransport(); let resolveAnswer; const added = [];
  const media = {on() { return () => {}; }, async createOffer() { return {type: 'offer', sdp: 'browser-offer'}; }, acceptAnswer() { return new Promise(resolve => { resolveAnswer = resolve; }); }, addIceCandidate(candidate) { added.push(candidate); }, close() {}};
  const runtime = createSessionRuntime({signaling, media}); const offer = await runtime.start({backend: {id: 'spartan-host'}});
  runtime.receive(createSessionEnvelope({sessionId: offer.sessionId, type: 'session.ice-candidate', messageId: 'msg-early-ice-01', payload: {candidate: {candidate: 'early'}}}));
  runtime.receive(createSessionEnvelope({sessionId: offer.sessionId, type: 'session.answer', messageId: 'msg-answer-sdp-01', sequence: 1, payload: {accepted: true, sdp: {type: 'answer', sdp: 'host-answer'}}}));
  await new Promise(resolve => setImmediate(resolve)); assert.deepEqual(added, []);
  resolveAnswer(); await new Promise(resolve => setImmediate(resolve));
...
test('session runtime sends one best-effort close envelope before local teardown', async () => { const signaling = fakeTransport(); const runtime = createSessionRuntime({signaling}); const offer = await runtime.start({backend: {id: 'spartan-host'}}); signaling.emit('message', createSessionEnvelope({sessionId: offer.sessionId, type: 'session.answer', messageId: 'msg-close-answer-01', payload: {accepted: true}})); runtime.close(); runtime.close(); assert.equal(signaling.sent.filter(message => message.type === 'session.close').length, 1); assert.equal(signaling.sent.at(-1).sessionId, offer.sessionId); });
test('session runtime retries a close envelope through signaling after data channel failure', async () => { const signaling = fakeTransport(); const media = {on() { return () => {}; }, createDataChannel() { return {readyState: 'open', send() { throw new Error('data channel closed'); }, close() {}}; }, async createOffer() { return {type: 'offer', sdp: 'browser-offer'}; }, close() {}}; const runtime = createSessionRuntime({signaling, media}); const offer = await runtime.start({backend: {id: 'spartan-host'}}); runtime.close(); assert.equal(signaling.sent.at(-1).type, 'session.close'); assert.equal(signaling.sent.at(-1).sessionId, offer.sessionId); });
...
test('session runtime reports asynchronous media answer failures without unhandled rejection', async () => {
  ...
  assert.match(errors[0].message, /remote SDP could not be applied/); assert.equal(runtime.state, 'error');
});

test('session runtime reports synchronous media answer failures without leaving the session connected', async () => {
  const signaling = fakeTransport(); const listeners = new Map(); const errors = [];
  const media = {on(type, handler) { if (!listeners.has(type)) listeners.set(type, new Set()); listeners.get(type).add(handler); return () => listeners.get(type)?.delete(handler); }, async createOffer() { return {type: 'offer', sdp: 'browser-offer'}; }, acceptAnswer() { throw new Error('remote SDP failed synchronously'); }, close() {}};
  const runtime = createSessionRuntime({signaling, media}); runtime.on('error', error => errors.push(error)); const offer = await runtime.start({backend: {id: 'spartan-host'}});
  runtime.receive(createSessionEnvelope({sessionId: offer.sessionId, type: 'session.answer', messageId: 'msg-sync-answer-01', payload: {accepted: true, sdp: {type: 'answer', sdp: 'bad-answer'}}}));
  await new Promise(resolve => setImmediate(resolve));
  assert.match(errors[0].message, /failed synchronously/); assert.equal(runtime.state, 'error');
});
