...
export function createSessionPreferences(settings = {}, capabilities, workspace = null, controllerProfiles = [], {controllerProfile: controllerProfileOverride} = {}) {
  ...
  const bitrateKbps = Math.max(2000, Math.min(NETWORK_BITRATE_CAPS[networkMode], mobileDataSaver ? MOBILE_DATA_SAVER_CAP : Infinity, numberBeforeUnit(settings['streaming.bitrate'], 25) * 1000));
