...
export function createSessionPreferences(settings = {}, capabilities) {
  ...
  const preferences = Object.freeze({qualityPreset, bitrateKbps, qualityProfiles, adaptiveBitrate: settings['streaming.adaptiveBitrate'] !== false, adaptiveResolution: settings['streaming.adaptiveResolution'] !== false, lowLatencyMode: settings['streaming.lowLatencyMode'] !== false, jitterBufferMs: Math.max(0, Math.min(500, numberBeforeUnit(settings['streaming.jitterBuffer'], 60))), hardwareDecode: settings['streaming.hardwareDecode'] !== false, autoFullscreen: settings['gaming.autoFullscreen'] !== false, pauseOnBlur: settings['gaming.pauseOnBlur'] === true, pictureInPicture: settings['gaming.pictureInPicture'] !== false, hideBrowserChrome: settings['gaming.hideBrowserChrome'] !== false, showOverlay: settings['gaming.showOverlay'] !== false, instantReplay: settings['gaming.instantReplay'] === true, replayLengthSeconds, showTelemetry: settings['streaming.showTelemetry'] === true, gameVolume: volume(settings['media.gameVolume']), audioOutput, recordingCodec, recordingLocation, clearOnExit: settings['privacy.clearOnExit'] === true, touchLayout: String(settings['accessibility.touchLayout'] || 'Automatic'), controllerProfile, controllerDeadzone, controllerBindings, inputPolling, allowHid, adaptiveTriggers, gyro, display: displayPolicy.display, maxRefreshRate: displayPolicy.maxRefreshRate, displayPolicy});
  return Object.freeze({capabilities: Object.freeze({video: Object.freeze({codecs: Object.freeze(displayPolicy.codecs), maxWidth: resolution[0], maxHeight: resolution[1], maxFramerate, hdr: displayPolicy.hdr}), audio: Object.freeze({codecs: Object.freeze(['opus', 'aac']), channels: 2}), input: Object.freeze({gamepad: settings['controllers.allowGamepad'] !== false, keyboard: true, pointer: true, rumble: settings['controllers.rumble'] !== false, hid: allowHid, adaptiveTriggers, gyro})}), preferences});
}
