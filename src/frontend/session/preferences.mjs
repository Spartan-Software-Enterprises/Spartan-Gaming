...
export function readSessionPreferences(storage = globalThis.localStorage, capabilities) { const settings = createSettingsStore({storage}).read(); const workspace = createWorkspaceStore({storage}).active; return createSessionPreferences(settings, capabilities, workspace); }
