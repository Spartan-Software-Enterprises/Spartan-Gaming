...
test('session manager applies adaptive bitrate and resolution preferences to telemetry requests', () => { const manager = createSessionManager({idFactory: () => 'ses-adaptive'}); const offer = manager.start({backend: {id: 'host'}, preferences: {qualityPreset: 'balanced', adaptiveBitrate: false, adaptiveResolution: true}}); manager.receive(createSessionEnvelope({sessionId: offer.sessionId, type: 'telemetry.health', payload: {packetLossPct: 9}})); assert.equal(manager.qualityRequest.profile, 'low'); assert.equal(manager.qualityRequest.bitrateKbps, 10000); assert.equal(manager.qualityRequest.maxWidth, 1280); });
