...
test('WebRTC stats normalize into bounded health telemetry', () => {
  ...
  assert.equal(sample.packetLossPct, 2);
  assert.equal(sample.decodeFps, 0);
  assert.equal(sample.jitterMs, 4);
  assert.equal(sample.framesDropped, 1);
  assert.equal(sample.bitrateKbps, 0);
});
...
test('WebRTC telemetry derives receive bitrate and decode rate from consecutive samples', () => {
  ...
  assert.equal(sample.decodeFps, 60);
  assert.equal(sample.bitrateKbps, 800);
  assert.equal(sample.packetsReceived, 200);
});
...
test('WebRTC telemetry collector polls without exposing raw stats', async () => {
    ...
    async getStats() {
      ...
      return [
        { type: 'candidate-pair', currentRoundTripTime: 0.05 },
        {
          type: 'inbound-rtp',
          kind: 'video',
          packetsReceived: calls * 10,
          framesDecoded: calls * 60,
        },
      ];
    },
  };
  const collector = createWebRtcTelemetryCollector({
    peerConnection: peer,
    scheduler: (callback) => {
  ...
  const samples = [];
  collector.on('health', (sample) => samples.push(sample));
  collector.start();
  await new Promise((resolve) => setImmediate(resolve));
  callbacks[0]();
  await new Promise((resolve) => setImmediate(resolve));
  assert.ok(calls >= 2);
  assert.equal(Object.hasOwn(samples[0], 'ssrc'), false);
  collector.stop();
