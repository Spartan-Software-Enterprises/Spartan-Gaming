...
export function createSessionRuntime({manager = createSessionManager(), signaling, media, clock = () => new Date().toISOString(), reconnect = {}} = {}) {
  ...
  const bus = eventBus();
  let sessionId = null; let sequence = 0; let dataChannel = null; let unbind = []; let remoteDescriptionReady = false; const pendingIceCandidates = []; const telemetry = media?.peerConnection?.getStats ? createWebRtcTelemetryCollector({peerConnection: media.peerConnection}) : null;
  const send = message => {
    const validated = validateTransportMessage(message);
    if (dataChannel?.readyState === 'open') { try { dataChannel.send(JSON.stringify(validated)); return validated; } catch {} }
    signaling.send(validated); return validated;
  };
  const bindDataChannel = channel => {
    dataChannel = channel;
    dataChannel.onmessage = event => { try { receive(JSON.parse(typeof event.data === 'string' ? event.data : String(event.data))); } catch (error) { bus.emit('error', error); } };
  };
  const addIceCandidate = candidate => Promise.resolve().then(() => media.addIceCandidate(candidate)).catch(error => bus.emit('error', error));
  const drainIceCandidates = async () => { while (pendingIceCandidates.length) await addIceCandidate(pendingIceCandidates.shift()); };
  const receive = message => {
    ...
    if (validated.type === 'session.answer' && media && validated.payload.sdp) Promise.resolve().then(() => media.acceptAnswer(validated.payload.sdp)).then(() => { remoteDescriptionReady = true; return drainIceCandidates(); }).catch(error => { pendingIceCandidates.length = 0; manager.fail(); bus.emit('error', error); });
    if (validated.type === 'session.ice-candidate' && media && validated.payload.candidate) { if (remoteDescriptionReady) void addIceCandidate(validated.payload.candidate); else if (pendingIceCandidates.length < MAX_PENDING_ICE_CANDIDATES) pendingIceCandidates.push(validated.payload.candidate); else bus.emit('error', new Error('pending ICE candidate limit exceeded')); }
  ...
  const reconnectController = createReconnectController({...reconnect, attempt: async () => { if (signaling.state !== 'open') await signaling.connect(); const envelope = manager.requestReconnect(); send(envelope); bus.emit('reconnect', envelope); return envelope; }});
  ...
  const runtime = {
    ...
    async start({backend, capabilities, preferences, launch} = {}) {
      const offer = manager.start({backend, capabilities, preferences, launch}); sessionId = offer.sessionId; sequence = offer.sequence; remoteDescriptionReady = false; pendingIceCandidates.length = 0;
      bind(); await signaling.connect();
      let outbound = offer;
      if (media) { const channel = media.createDataChannel?.(); if (channel) bindDataChannel(channel); const sdp = await media.createOffer(); outbound = createSessionEnvelope({sessionId, type: offer.type, sequence: offer.sequence, sentAt: clock(), payload: {...offer.payload, sdp}}); }
    ...
    close() { reconnectController.cancel(); telemetry?.stop(); pendingIceCandidates.length = 0; if (sessionId && ['preparing', 'negotiating', 'connected', 'reconnecting'].includes(manager.state)) { try { send(createSessionEnvelope({sessionId, type: 'session.close', sequence: ++sequence, sentAt: clock(), payload: {}})); } catch (error) { bus.emit('error', error); } } manager.close(); dataChannel?.close?.(); media?.close?.(); signaling.close?.(); unbind.forEach(off => off()); unbind = []; bus.emit('closed'); },
  };
