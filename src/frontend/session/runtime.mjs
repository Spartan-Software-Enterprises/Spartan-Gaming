...
export function createSessionRuntime({manager = createSessionManager(), signaling, media, clock = () => new Date().toISOString()} = {}) {
  ...
  const bus = eventBus();
  let sessionId = null; let sequence = 0; let dataChannel = null; let unbind = [];
  const send = message => {
  ...
  const runtime = {
    ...
    async start({backend, capabilities} = {}) {
      ...
      send(outbound); bus.emit('offer', outbound); return outbound;
    },
    send,
    receive,
    requestReconnect() { const reconnect = manager.requestReconnect(); send(reconnect); bus.emit('reconnect', reconnect); return reconnect; },
    close() { manager.close(); dataChannel?.close?.(); media?.close?.(); signaling.close?.(); unbind.forEach(off => off()); unbind = []; bus.emit('closed'); },
  };
