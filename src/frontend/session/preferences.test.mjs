...
test('session preferences carry validated recording codec and location choices', () => { const result = createSessionPreferences({'media.recordingCodec': 'H.264', 'media.recordingLocation': 'Ask each time'}); assert.equal(result.preferences.recordingCodec, 'H.264'); assert.equal(result.preferences.recordingLocation, 'Ask each time'); const fallback = createSessionPreferences({'media.recordingCodec': 'unsupported', 'media.recordingLocation': 'unknown'}); assert.equal(fallback.preferences.recordingCodec, 'Automatic'); assert.equal(fallback.preferences.recordingLocation, 'Videos/Spartan Gaming'); });
