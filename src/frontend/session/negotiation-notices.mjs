...
export function describeNegotiationAdjustments({requested = {}, negotiated = {}} = {}) {
  ...
  const requestedWidth = positive(requestedVideo.maxWidth); const negotiatedWidth = positive(negotiatedVideo.maxWidth); const requestedHeight = positive(requestedVideo.maxHeight); const negotiatedHeight = positive(negotiatedVideo.maxHeight);
  if (requestedWidth && negotiatedWidth && requestedHeight && negotiatedHeight && (negotiatedWidth < requestedWidth || negotiatedHeight < requestedHeight)) items.push(`resolution ${negotiatedWidth}×${negotiatedHeight}`);
  const requestedFramerate = positive(requestedVideo.maxFramerate); const negotiatedFramerate = positive(negotiatedVideo.maxFramerate);
  if (requestedFramerate && negotiatedFramerate && negotiatedFramerate < requestedFramerate) items.push(`${negotiatedFramerate} FPS`);
...
export function formatNegotiationAdjustments(items = []) { return Array.isArray(items) && items.length ? `Session adjusted for host limits: ${items.join(' · ')}.` : ''; }
