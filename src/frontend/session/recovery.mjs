...
export function calculateReconnectDelay(attempt, {baseDelayMs = 500, maxDelayMs = 15_000, jitter = 0, random = Math.random} = {}) {
...
export function createReconnectPolicy({maxAttempts = 5, baseDelayMs = 500, maxDelayMs = 15_000, jitter = 0.2, random = Math.random} = {}) {
  const limit = Math.max(1, Math.floor(numberOr(maxAttempts, 5)));
  let attempt = 0;
  return {
    get attempt() { return attempt; },
    get exhausted() { return attempt >= limit; },
    get maxAttempts() { return limit; },
    next() {
      if (attempt >= limit) return Object.freeze({attempt, delayMs: 0, exhausted: true});
      attempt += 1;
      return Object.freeze({attempt, delayMs: calculateReconnectDelay(attempt, {baseDelayMs, maxDelayMs, jitter, random}), exhausted: false});
    },
    reset() { attempt = 0; },
    succeeded() { attempt = 0; },
  };
