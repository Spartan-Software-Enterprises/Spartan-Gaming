...
function integer(value, fallback, minimum, maximum) {
  ...
  return Number.isFinite(result) ? Math.max(minimum, Math.min(maximum, Math.floor(result))) : fallback;
}
...
export function normalizePowerState({mode = 'Balanced', batteryPercent, charging = false} = {}) {
  ...
  const percent = batteryPercent === undefined || batteryPercent === null || batteryPercent === ''
    ? null
    : integer(batteryPercent, null, 0, 100);
  return Object.freeze({mode: selectedMode, batteryPercent: percent, charging: charging === true});
}

/** Resolve a conservative quality ceiling for power-constrained handheld sessions. */
export function resolvePowerQualityPolicy({mode = 'Balanced', batteryPercent, charging = false} = {}) {
  const state = normalizePowerState({mode, batteryPercent, charging});
  const constrained = state.mode === 'Battery saver' || (!state.charging && state.batteryPercent !== null && state.batteryPercent <= 20);
  ...
  return Object.freeze({state, status: emergency ? 'emergency' : constrained ? 'constrained' : 'normal', limits: Object.freeze(limits), reason: emergency ? 'Battery is critically low; quality is capped to preserve session continuity.' : constrained ? 'Battery-saving policy caps quality to reduce power use.' : 'No additional power quality cap is active.'});
}
