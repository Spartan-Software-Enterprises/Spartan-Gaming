...
export function normalizeRecordingPreferences(raw) {
  if (!raw || typeof raw !== 'object') return { mode: 'off', retention: '7-days', includeAudio: false, includeOverlay: false };
  ...
  return { mode, retention, includeAudio: raw.includeAudio === true, includeOverlay: raw.includeOverlay === true };
}

export function resolveRecordingReadiness({ preferences, sessionActive, displayPermission, storageAvailable }) {
  ...
  if (!displayPermission) return { status: 'unavailable', reason: 'display-permission-missing', ...prefs };
...
export function computeRetentionCutoff(retention, now = Date.now()) {
  const retentionMs = { '1-day': 86400000, '7-days': 604800000, '30-days': 2592000000, manual: Infinity };
...
export function pruneExpiredSegments(segments, retention, now = Date.now()) {
  ...
  return segments.filter((seg) => seg && typeof seg === 'object' && (Number.isFinite(seg.timestamp) ? seg.timestamp : 0) >= cutoff);
}
