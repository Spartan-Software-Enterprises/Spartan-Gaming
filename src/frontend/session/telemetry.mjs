...
export function normalizeWebRtcStats(report, {previous = null, now = Date.now()} = {}) {
  ...
  const elapsedMs = Math.max(1, asNumber(now) - asNumber(previous?.timestamp, now)); const frames = asNumber(video.framesDecoded); const priorFrames = asNumber(previous?.framesDecoded, frames);
  return Object.freeze({timestamp: asNumber(now), rttMs: Math.round(asNumber(candidate.currentRoundTripTime || candidate.roundTripTime) * 1000), packetLossPct: Math.max(0, Math.min(100, packetLossPct)), decodeFps: Math.round(Math.max(0, frames - priorFrames) * 1000 / elapsedMs), framesDropped: asNumber(video.framesDropped), jitterMs: Math.round(asNumber(video.jitter) * 1000), bytesReceived: asNumber(video.bytesReceived), packetsLost: lost, packetsReceived: received});
}
