...
test('session recovery handoff is bounded, authenticated, and session scoped', () => {
  ...
  assert.equal(
    saveSessionRecoveryHandoff(
      store,
      {
        endpoint: 'wss://signal.example.test/connect',
        sessionId: 'ses-1',
        ticket: 'ticket-1',
        backendId: 'host',
        backendType: 'remote-play',
        backendName: 'Living room host',
      },
      1000,
    ),
    true,
...
test('session recovery expires and is removed', () => {
  ...
  saveSessionRecoveryHandoff(
    store,
    { endpoint: 'ws://localhost:8787', sessionId: 'ses-2', ticket: 'ticket-2' },
    2000,
  );
...
test('session recovery rejects malformed or unsafe connection data', () => {
  assert.equal(
    createSessionRecoveryHandoff(
      { endpoint: 'https://signal.example.test', sessionId: 'ses', ticket: 'ticket' },
      1000,
    ),
    null,
  );
    ...
    createSessionRecoveryHandoff(
      {
        ...
        sessionId: 'ses',
        ticket: 'ticket',
        expiresAt: 1000 + SESSION_RECOVERY_TTL_MS + 1,
      },
      1000,
    ),
    null,
