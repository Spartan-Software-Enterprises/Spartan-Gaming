...
export async function collectPermissionStates({permissionsLike = globalThis.navigator?.permissions} = {}) {
  const states = await Promise.all(PERMISSIONS.map(async permission => {
    let state = 'unavailable';
    try { state = String((await permissionsLike?.query?.({name: permission.name}))?.state || 'unavailable'); } catch { state = 'unavailable'; }
    return Object.freeze({name: permission.name, label: permission.label, state: VALID_STATES.has(state) ? state : 'unavailable'});
  }));
  return Object.freeze(states);
}

export function collectPerformanceSnapshot({navigatorLike = globalThis.navigator, performanceLike = globalThis.performance} = {}) {
  const memory = performanceLike?.memory;
  const connection = navigatorLike?.connection;
  return Object.freeze({
    logicalProcessors: Number.isInteger(navigatorLike?.hardwareConcurrency) ? navigatorLike.hardwareConcurrency : null,
    ...
    jsHeapUsedMb: Number.isFinite(memory?.usedJSHeapSize) ? Math.round(memory.usedJSHeapSize / 1024 ** 2) : null,
    jsHeapLimitMb: Number.isFinite(memory?.jsHeapSizeLimit) ? Math.round(memory.jsHeapSizeLimit / 1024 ** 2) : null,
  });
}
