...
test('provider integration selects platform preset and saved launch preference', () => {
  const integration = createProviderIntegration(xbox, {
    profile: {
      launchMode: 'official',
  ...
  assert.equal(integration.autoFullscreen, true);
  assert.match(integration.notes.at(-1), /Fullscreen/);
});
test('provider controller profile overrides the service preset', () => {
  assert.equal(
    createProviderIntegration(xbox, { profile: { controllerProfile: 'PlayStation layout' } })
      .controllerProfile,
    'PlayStation layout',
  );
});
test('provider integration exposes official surfaces without claiming API access', () => {
  const integration = createProviderIntegration(stream);
  assert.equal(integration.mode, 'browser-first');
  assert.deepEqual(integration.surfaces, ['video', 'chat']);
  assert.equal(integration.health.authenticated, false);
});
test('provider integration applies conservative presets to major services', () => {
  const integration = createProviderIntegration({
    ...xbox,
    id: 'nvidia-geforce-now',
    capabilities: ['gamepad'],
  });
  assert.equal(integration.controllerProfile, 'Auto-detect');
  assert.equal(integration.quality, 'prefer-latency');
});
test('GameNative remains an external Android companion with explicit licensing notes', () => {
  const integration = createProviderIntegration(
    {
      id: 'gamenative',
      name: 'GameNative',
      ...
      nativeApp: { platform: 'android', packageName: 'app.gamenative', license: 'GPL-3.0' },
    },
...
test('provider integration bounds unknown region hints to automatic', () => {
  const integration = createProviderIntegration(xbox, { profile: { region: 'mars' } });
  assert.equal(integration.regionHint, 'automatic');
  assert.equal(integration.regionLabel, 'Automatic');
});
test('provider integration builds and selects official target-aware embeds', () => {
  const twitch = createProviderIntegration(
    { ...stream, id: 'twitch' },
  ...
  assert.equal(
    createProviderIntegration(stream, { profile: { embedTarget: 'not valid' } }).embedUrl,
    null,
  );
});
test('provider troubleshooting remains structured and capability-aware', () => {
  const integration = createProviderIntegration(xbox, { report: { input: { gamepad: false } } });
  const issues = providerTroubleshooting(integration);
  assert.equal(issues[0].key, 'gamepad');
  assert.equal(issues[1].key, 'configuration');
});
