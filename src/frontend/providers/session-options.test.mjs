...
test('provider session options map the retained sign-in setting', () => {
  ...
  assert.equal(resolveProviderSessionOptions({ 'providers.autoLogin': false }).autoLogin, false);
});
...
test('provider session options bound the profile id and non-boolean inputs', () => {
  ...
  assert.equal(resolveProviderSessionOptions({ 'providers.autoLogin': 'false' }).autoLogin, true);
  assert.equal(
    resolveProviderSessionOptions({ 'providers.isolateAccounts': false }).isolateAccounts,
    false,
  );
  assert.equal(resolveProviderSessionOptions({ 'providers.autoDetect': false }).autoDetect, false);
});

test('provider session options include accountId for multi-account support', () => {
  assert.equal(resolveProviderSessionOptions({}, { accountId: 'personal' }).accountId, 'personal');
  assert.equal(resolveProviderSessionOptions({}, { accountId: '' }).accountId, 'default');
  assert.equal(
