...
const CONTROLLER_PROFILE_VALUES = new Set(['Auto-detect', 'Xbox layout', 'PlayStation layout', 'Nintendo layout', 'Keyboard and mouse']);
...
export function normalizeProviderProfile(profile) { required(profile?.providerId, 'profile.providerId'); const region = GLOBAL_REGION_VALUES[profile.region] || (Object.values(GLOBAL_REGION_VALUES).includes(profile.region) ? profile.region : 'automatic'); const requestedControllerProfile = typeof profile.controllerProfile === 'string' ? profile.controllerProfile.trim() : ''; const controllerProfile = CONTROLLER_PROFILE_VALUES.has(requestedControllerProfile) || CONTROLLER_PROFILE_ID.test(requestedControllerProfile) ? requestedControllerProfile : 'Auto-detect'; return Object.freeze({providerId: profile.providerId.trim(), accountLabel: String(profile.accountLabel || ''), region, quality: ['prefer-latency', 'balanced', 'prefer-quality'].includes(profile.quality) ? profile.quality : 'balanced', launchMode: ['browser', 'official', 'native'].includes(profile.launchMode) ? profile.launchMode : 'browser', controllerProfile, autoFullscreen: profile.autoFullscreen !== false, embedTarget: String(profile.embedTarget || '').trim().slice(0, 128), notes: String(profile.notes || '')}); }
