...
const cloud = {id: 'cloud', backendType: 'provider', capabilities: ['gamepad'], requirements: ['provider-account']};
const host = {id: 'host', backendType: 'provider', capabilities: ['gamepad'], requirements: ['user-owned-host'], integrationModes: ['self-hosted']};
...
test('runtime readiness adds host preflight as a separate layer', () => {
  const result = createRuntimeReadiness({entry: host, report: {input: {gamepad: true}}, hostCapabilities: {media: {capture: true, encode: true, transports: ['websocket']}, publisher: {state: 'unconfigured'}}, clientTransports: ['webrtc']});
...
test('runtime readiness fails closed for untrusted native adapters', () => {
  const entry = {id: 'emu', backendType: 'emulator', mode: 'browser-or-native', capabilities: [], requirements: []};
...
test('runtime readiness accepts a fully ready browser path', () => {
  const result = createRuntimeReadiness({entry: {...cloud, requirements: []}, report: {input: {gamepad: true}}});
