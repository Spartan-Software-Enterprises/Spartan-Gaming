...
const HOST_REQUIREMENTS = new Set([
  'host-agent',
  'user-owned-host',
  'secure-pairing',
  'explicit-pairing',
...
function needsHostPreflight(entry) {
  return (
    (entry?.requirements || []).some((requirement) => HOST_REQUIREMENTS.has(requirement)) ||
    (entry?.integrationModes || []).some(
      (mode) => mode === 'self-hosted' || mode === 'user-owned-host',
    )
  );
}
...
function chooseStatus(catalog, host, adapter) {
  if (
    catalog.status === 'browser-capability-missing' ||
    catalog.status === 'native-adapter-required'
  )
...
export function createRuntimeReadiness({
  ...
  const host =
    hostCapabilities && needsHostPreflight(entry)
      ? createHostPreflight({ capabilities: hostCapabilities, clientTransports })
      : null;
  ...
  return Object.freeze({
    ...
    reason:
      status === 'host-not-ready'
        ? 'The paired host is not ready to publish a compatible session.'
        : status === 'adapter-trust-required'
          ? 'A verified native adapter is required before launch.'
          : status === 'adapter-unavailable'
            ? 'A compatible native adapter is not installed for this platform.'
            : catalog.reason,
