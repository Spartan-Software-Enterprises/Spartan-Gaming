...
test('readiness status progresses from loading to actionable setup and ready', () => {
  ...
  assert.equal(createReadinessStatus({catalogLoaded: true, compatibility: compatibility({'configuration-required': 2}), capabilityReport: report}).id, 'attention');
  assert.equal(createReadinessStatus({catalogLoaded: true, compatibility: compatibility({'native-adapter-required': 1}), capabilityReport: report}).id, 'degraded');
  assert.equal(createReadinessStatus({catalogLoaded: true, compatibility: compatibility({}), capabilityReport: report}).id, 'ready');
});
