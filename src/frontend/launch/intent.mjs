...
export function createLaunchIntent({entry, plan, profileId, returnTo = '../dashboard/index.html', createdAt = new Date().toISOString()} = {}) {
  ...
  return Object.freeze({version: 1, backendId, backendType, mode, action, url, profileId: optional(profileId), returnTo: required(returnTo, 'returnTo'), requirements: Object.freeze([...(plan?.requirements || entry?.requirements || [])].map(value => required(value, 'requirement'))), capabilities: Object.freeze([...(plan?.capabilities || entry?.capabilities || [])].map(value => required(value, 'capability'))), createdAt: required(createdAt, 'createdAt')});
}
...
export function saveLaunchIntent(storage = globalThis.sessionStorage, intent) {
  ...
  const normalized = createLaunchIntent({entry: {id: intent?.backendId, backendType: intent?.backendType}, plan: intent, profileId: intent?.profileId, returnTo: intent?.returnTo, createdAt: intent?.createdAt});
...
export function readLaunchIntent(storage = globalThis.sessionStorage) {
  try {
    ...
    return createLaunchIntent({entry: {id: parsed.backendId, backendType: parsed.backendType}, plan: parsed, profileId: parsed.profileId, returnTo: parsed.returnTo, createdAt: parsed.createdAt});
  } catch { return null; }
}
