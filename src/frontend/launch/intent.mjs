...
function optionalBounded(value, name) {
  const result = optional(value);
  if (result && result.length > 80) throw new RangeError(`${name} is too long`);
  return result;
}
...
export function createLaunchIntent({
  ...
  return Object.freeze({
    ...
    controllerProfile: optionalBounded(
      controllerProfile || plan?.controllerProfile,
      'controllerProfile',
    ),
    returnTo: required(returnTo, 'returnTo'),
    requirements: Object.freeze(
      [...(plan?.requirements || entry?.requirements || [])].map((value) =>
        required(value, 'requirement'),
      ),
    ),
    capabilities: Object.freeze(
      [...(plan?.capabilities || entry?.capabilities || [])].map((value) =>
...
export function saveLaunchIntent(storage = globalThis.sessionStorage, intent) {
  ...
  const normalized = createLaunchIntent({
    entry: { id: intent?.backendId, backendType: intent?.backendType },
    plan: intent,
    profileId: intent?.profileId,
    controllerProfile: intent?.controllerProfile,
...
export function readLaunchIntent(storage = globalThis.sessionStorage) {
  try {
    ...
    return createLaunchIntent({
      entry: { id: parsed.backendId, backendType: parsed.backendType },
      plan: parsed,
...
export function clearLaunchIntent(storage = globalThis.sessionStorage) {
  storage?.removeItem?.(LAUNCH_INTENT_KEY);
}

export function consumeLaunchIntent(
  storage = globalThis.sessionStorage,
  { backendId, backendType, action, maxAgeMs = 10 * 60 * 1000, now = Date.now() } = {},
) {
  const intent = readLaunchIntent(storage);
  if (!intent) return null;
  const created = Date.parse(intent.createdAt);
  const age = Number(now) - created;
  if (!Number.isFinite(created) || !Number.isFinite(age) || age < 0 || age > maxAgeMs) {
