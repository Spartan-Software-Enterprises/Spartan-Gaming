...
function normalizeHandoff(value, {now = Date.now()} = {}) {
  if (!value || value.version !== LAUNCH_HANDOFF_VERSION) throw new TypeError('launch handoff version is unsupported');
  const request = normalizeHostLaunchRequest(value.request);
  const createdAt = Date.parse(value.createdAt);
  if (!Number.isFinite(createdAt) || createdAt > now || now - createdAt > MAX_AGE_MS) throw new Error('launch handoff has expired');
...
export function savePendingLaunchHandoff(storage = globalThis.sessionStorage, {request, backendId, name, now = Date.now()} = {}) {
  if (!storage || typeof storage.setItem !== 'function') throw new TypeError('session storage is required');
  const handoff = normalizeHandoff({version: LAUNCH_HANDOFF_VERSION, request, backendId, name, createdAt: new Date(now).toISOString()}, {now});
  storage.setItem(PENDING_LAUNCH_HANDOFF_KEY, JSON.stringify(handoff));
  return handoff;
}

export function readPendingLaunchHandoff(storage = globalThis.sessionStorage, {now = Date.now()} = {}) {
  try {
