...
test('launch intents carry a bounded controller profile override', () => {
  const intent = createLaunchIntent({ entry, plan, controllerProfile: 'PlayStation layout' });
  assert.equal(intent.controllerProfile, 'PlayStation layout');
  assert.throws(
    () => createLaunchIntent({ entry, plan, controllerProfile: 'x'.repeat(81) }),
    /too long/,
  );
});
...
test('launch intents reject insecure URLs and malformed stored values', () => {
  assert.throws(
    () =>
      createLaunchIntent({
        entry: { ...entry, backendType: 'provider' },
        plan: { ...plan, action: 'open-url', url: 'http://example.test' },
      }),
    /HTTPS/,
  );
...
test('launch intent consumption is target-aware, one-shot, and age-bounded', () => {
  ...
  const consumed = consumeLaunchIntent(storage, {
    backendType: 'emulator',
    action: 'choose-runtime',
    now: Date.parse(createdAt) + 1000,
  });
  assert.equal(consumed.backendId, 'dolphin');
  assert.equal(readLaunchIntent(storage), null);
  saveLaunchIntent(storage, createLaunchIntent({ entry, plan, createdAt }));
  assert.equal(
    consumeLaunchIntent(storage, {
      backendType: 'emulator',
      now: Date.parse(createdAt) + 10 * 60 * 1000 + 1,
    }),
    null,
  );
  assert.equal(readLaunchIntent(storage), null);
