...
import {
  clearPendingLaunchHandoff,
  readPendingLaunchHandoff,
  savePendingLaunchHandoff,
} from './handoff.mjs';
...
function request() {
  return createNativeHostLaunchRequest({
    plan: {
      status: 'ready',
      coreId: 'dolphin',
      files: [{ kind: 'game', name: 'game.iso', size: 42, userSelected: true }],
      integration: {
    ...
    hostContentId: 'gamecube-dolphin',
    consent: true,
  });
}
...
test('launch handoffs persist only a normalized metadata request in session storage', () => {
  ...
  const saved = savePendingLaunchHandoff(backend, {
    request: request(),
    backendId: 'dolphin',
    name: 'Dolphin',
  });
...
test('expired or malformed launch handoffs fail closed', () => {
  const backend = storage();
  savePendingLaunchHandoff(backend, {
    request: request(),
    backendId: 'dolphin',
    name: 'Dolphin',
    now: 1000,
