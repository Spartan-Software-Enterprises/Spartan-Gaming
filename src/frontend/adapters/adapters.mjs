...
export function resolveLaunchPlan(entry, {allowedModes, preferEmbedded = false, providerProfile = {}} = {}) {
  ...
  const nativeClient = selectedMode === 'native-client' ? {providerId: entry.id, platform: providerProfile.platform} : integration.nativeClient;
  const nativeLaunch = nativeClient ? createNativeClientLaunchPlan({providerId: entry.id, platform: nativeClient.platform, discovery: providerProfile.clientDiscovery}) : null;
  return Object.freeze({backendId: entry.id, status: 'ready', mode: selectedMode, ...plan, url: plan.action === 'embed-url' ? integration.embedUrl : plan.action === 'open-url' || plan.action === 'configure-api' ? entry.url : undefined, nativeLaunch, requirements: Object.freeze([...(entry.requirements || [])]), capabilities: Object.freeze([...(entry.capabilities || [])]), integration, readiness: Object.freeze({...runtimeReadiness, nextAction, issues})});
}
