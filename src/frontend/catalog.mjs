...
function assertHttpsUrl(value, id) { try { const url = new URL(value); if (url.protocol !== 'https:' || url.username || url.password) throw new Error(); } catch { throw new TypeError(`${id}: url must use HTTPS without credentials`); } }
function assertDate(value, source) { if (typeof value !== 'string' || !/^\d{4}-\d{2}-\d{2}$/u.test(value)) throw new TypeError(`${source} updatedAt must use YYYY-MM-DD`); const [year, month, day] = value.split('-').map(Number); const date = new Date(Date.UTC(year, month - 1, day)); if (date.getUTCFullYear() !== year || date.getUTCMonth() !== month - 1 || date.getUTCDate() !== day) throw new TypeError(`${source} updatedAt must be a valid date`); }
...
export function validateCatalogManifest(manifest, source) {
  ...
  const ids = new Set();
  for (const entry of entries) { if (ids.has(entry?.id)) throw new Error(`duplicate ${source} catalog id: ${entry.id}`); ids.add(entry?.id); }
