...
GameNative's upstream Android activity supports an explicit `LAUNCH_GAME` intent with a store and numeric app ID. `android/gamenative/GameNativeHandoff.kt` now implements the narrow bridge: it accepts only a positive app ID, an allow-listed store, and the official `app.gamenative` package. The Android shell still owns user consent, installation, lifecycle, and permission handling; the bridge never guesses IDs, injects configuration, or bundles GameNative. The release fallback opens the official HTTPS download URL when the intent cannot resolve.
