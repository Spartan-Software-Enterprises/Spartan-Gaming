...
Controller profiles are managed at `src/frontend/input/profiles.html`. The live controller tester is `src/frontend/input/inspector.html`; it reports normalized buttons, axes, haptics, battery metadata, and approved HID devices locally. Its inventory and refresh loop apply the normalized Controller settings: disabling gamepad access hides browser gamepads, multiplayer and player-slot choices bound the visible inventory, the configured dead zone filters live axes, and input-latency mode selects the polling interval. The page displays that effective policy beside the browser capability badges. Profiles are persisted locally, validate duplicate bindings before save, and share the same stable action vocabulary with the session player and future native input adapters. The Settings → Controllers default-profile selector includes locally saved custom profiles by stable ID with a readable custom label, so the selected mapping survives rename/export/import flows; an unknown or deleted ID safely falls back to auto-detection. Workspaces can also assign any saved custom profile, and player preflight includes its bindings, deadzone, and rumble preference in the session capabilities. On touch-capable devices, the player renders a permission-gated virtual controller from `src/frontend/input/touch-controls.mjs`; its minimal/full layout follows Settings → Accessibility → Touch-control layout and emits the same bounded `input.event` actions as a physical gamepad.

`host/bluetooth.mjs` is the platform-neutral Bluetooth boundary. It normalizes Classic/BLE discovery into opaque IDs and bounded names, kinds, battery, HID/audio/haptics capabilities, and connection state; deduplicates discovery; requires explicit permission and pairing consent; and limits reconnect attempts to previously approved devices. Windows, macOS, Linux/SteamOS, Android, ChromeOS, and Fire TV integrations must inject native adapters implementing discovery/pair/connect/forget. The boundary never retains raw addresses, executes arbitrary commands, downloads drivers, or auto-pairs unknown hardware.

`host/hardware-drivers.mjs` complements that boundary with a curated driver descriptor and install-plan contract. Descriptors are platform- and hardware-specific, signed, HTTPS-only, SHA-256 addressed, and limited to known install methods; plans require exact hardware matching plus user consent and administrator approval. The contract records a rollback version but leaves download verification, elevation, transactional installation, and rollback to platform-native adapters.
