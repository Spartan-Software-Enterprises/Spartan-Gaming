...
Steam Deck-oriented actions are defined in `src/frontend/input/steam-input.mjs` with explicit requirements for trackpads, gyro, rear buttons, touchscreen, and controller text entry. `negotiateSteamInputActions()` reports supported and unavailable actions from observed capabilities, while `negotiateCapabilities()` carries the corresponding `trackpads`, `touchscreen`, and `textEntry` fields through the session contract. Missing hardware remains unavailable and the Gamepad/HID fallback stays explicit; no physical Deck readiness is inferred.
