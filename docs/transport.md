...
When degraded telemetry changes the active quality profile, the runtime sends a validated `quality.request` envelope back through signaling. The session player also exposes an explicit Ultra/High/Balanced/Low/Emergency selector that uses the same validated request path; selecting a profile resets the adaptive controller’s recovery counter without disabling future adaptation. Host adapters may apply the request to their encoder; the reference host records it for diagnostics but cannot encode media. `input.event` envelopes are likewise delivered to the host control plane only after the session and pairing checks succeed.
