...
When degraded telemetry changes the active quality profile, the runtime sends a validated `quality.request` envelope back through signaling. The session player also exposes an explicit Ultra/High/Balanced/Low/Emergency selector that uses the same validated request path; selecting a profile resets the adaptive controller’s recovery counter without disabling future adaptation. Browser Host applies bounded bitrate and framerate requests to supported WebRTC video sender encodings and reports unsupported or failed controls in its diagnostics; native host adapters remain responsible for their encoder APIs. The reference host records requests but cannot encode media. `input.event` envelopes are likewise delivered to the host control plane only after the session and pairing checks succeed.
