...
The player also exposes an explicit Pause/Resume control in the session
overlay; it uses the same validated control envelope and remains useful as a
local media pause when no host control plane is active.
The player emits normalized controller and keyboard actions as protocol v1 `input.event` envelopes through the `spartan:input` browser event. Gamepad values include deadzone processing and signed analog ranges; analog direction changes explicitly release the prior direction before pressing the new one, including when a stick returns to neutral. Standard controls without a semantic binding fall through as native `button-N`/`axis-N` passthrough events (button indices 0-31 and axis indices 0-5), so a physical controller's D-pad, shoulder buttons, triggers, paddles, and other extended controls still reach the host for virtual-gamepad mirroring. Adapters are responsible for forwarding these envelopes over the active data channel and applying the selected controller profile on the host side. The browser does not transmit raw device identity or full Gamepad objects.
