...
The player emits normalized controller and keyboard actions as protocol v1 `input.event` envelopes through the `spartan:input` browser event. Gamepad values include deadzone processing and signed analog ranges; analog direction changes explicitly release the prior direction before pressing the new one, including when a stick returns to neutral. Standard controls without a semantic binding fall through as native `button-N`/`axis-N` passthrough events (indices 0-15 and 0-5 respectively), so a physical controller's D-pad, shoulder buttons, and triggers still reach the host for one-controller virtual-gamepad mirroring. Adapters are responsible for forwarding these envelopes over the active data channel and applying the selected controller profile on the host side. The browser does not transmit raw device identity or full Gamepad objects.
