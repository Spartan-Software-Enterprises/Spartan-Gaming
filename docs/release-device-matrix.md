...
The shared gate is: npm run format:check, npm run check, npm test, npm run test:serial, npm run electron:test, npm run test:android-shell, npm run frontend:build, npm run playwright:smoke, npm run playwright:provider-profile, npm run playwright:ui-audit, node scripts/playwright/providers-deep.mjs, and node scripts/playwright/full-verification.mjs. The UI audit covers every shipped route at desktop, landscape-phone, mobile, and television layouts, including accessible control names, visible-control bounds, horizontal overflow, runtime errors, and screenshots.
