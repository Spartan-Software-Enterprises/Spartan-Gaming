...
| Electron fullscreen, quit guard, background behavior, tray restore/quit actions, power mode, native power telemetry, privacy controls, single-instance lifecycle, and deep links | Contract-tested, including tray policy, runtime synchronization, bounded power events, session-aware power-save blocking, protocol registration, and catalog-ID deep-link filtering | Verify native window behavior, protocol registration, tray icon/menu behavior, suspend/resume, thermal/speed-limit events, GPU use, permissions, deep-link activation, and shutdown on Windows/macOS/Linux |
...
| Android touch layout, lifecycle, orientation, PiP, edge-to-edge, Game Mode, large-screen and foldable behavior | Shared Android policy and API-31+ `GameManager` query-on-resume contract tested; AWS debug APK build passed with the committed Gradle wrapper; Android wrapper build is also wired into CI | Install APKs and test phones, tablets, foldables, permissions, rotation, PiP, Game Mode changes, and resume flows |
| Android/Steam Deck controller text entry and IME composition | Bounded text, selection, composition, and user-gesture gating contracts tested; native GameTextInput boundary documented; Android WebView shell source packages the shared frontend and bridge; release signing is an environment-only Gradle boundary with a protected manual CI workflow that fails closed without all operator values | Build/sign the Android shell with SDK/Gradle, compile the Android Games SDK GameTextInput binding, then test show/hide, composing text, selection updates, keyboard dismissal, rotation, and controller-only entry on physical Android/SteamOS targets |
| SteamOS / Steam Deck handheld profile, Steam Input, Gamescope, Proton/native launch, and Game Mode behavior | Explicit SteamOS host profile, bounded 1280×800/30-40-60 policy, Gamescope plan, portable Steam Input action/glyph contract, action-level capability negotiation for trackpads/gyro/rear buttons/touchscreen/text entry/haptics/controller navigation, launch-mode readiness metadata, Proton launch boundary, battery-aware session quality policy, and shared handheld/controller contracts tested; acceptance now requires separate physical Deck and SteamOS desktop reports | Test a physical Deck and supported SteamOS host in Desktop/Game Mode, with the installed Steam Input bridge, glyphs/actions, text entry, trackpad/gyro/rear controls, haptics, controller-only navigation, Gamescope, suspend/resume, battery, external display, Steam/non-Steam handoffs, and real Proton/native games |
...
[Android GameTextInput](https://developer.android.com/games/agdk/add-support-for-text-input),
...
npm run roadmap:acceptance -- \
  ...
  --steamos-report "$EVIDENCE/steam-os/steam-deck.json" \
