...
function findOnPath(candidate) {
  for (const entry of (process.env.PATH || '').split(path.delimiter)) {
    if (!entry) continue;
    try {
      const full = path.resolve(entry, candidate);
...
export function resolveChromiumBinary(
  binary = '',
  { exists = existsSync, find = findOnPath, candidates = CHROMIUM_CANDIDATES } = {},
) {
  let found = null;
  ...
  if (given) found = path.resolve(given);
    ...
    for (const candidate of candidates) {
      const executable = find(candidate);
      if (executable) {
        found = executable;
        break;
      }
...
export function createDesktopShellPlan({
  ...
  publicRoot,
  seedRoot = null,
  adapterHome = resolveAdapterHome(),
  platform = process.platform,
  host = '127.0.0.1',
  port = 4173,
  chromiumBinary = '',
  contextScript = null,
  resolveBinary = resolveChromiumBinary,
} = {}) {
  const binary = resolveBinary(chromiumBinary);
  if (!binary)
    throw new Error(
      'no Chromium-based desktop shell found; install chromium or set SPARTAN_CHROMIUM_BINARY',
    );
  const args = [
    `--app=http://${host}:${port}/dashboard/?startup=1`,
    '--no-first-run',
    '--no-default-browser-check',
  ];
  if (typeof process.getuid === 'function' && process.getuid() === 0) args.push('--no-sandbox');
  if (process.env.SPARTAN_DISABLE_CHROMIUM_SANDBOX === '1' && !args.includes('--no-sandbox'))
  ...
  return Object.freeze({
    ...
    publicRoot: publicRoot ? path.resolve(publicRoot) : null,
    seedRoot: seedRoot ? path.resolve(seedRoot) : null,
  });
}
...
export async function launchDesktopSession({
  ...
  port = 4173,
  chromiumBinary = '',
  hostFactory = createAppHostServer,
  spawnImpl = spawn,
  contextScript = null,
  installSeed = true,
  ...
  if (seedRoot) {
    ...
    if (installSeed) {
      try {
        await runSeedInstall({ seedRoot, installRoot: adapterHome, platform, apply: true });
      } catch (error) {
        console.error(`seed install skipped: ${error.message}`);
      }
    }
    seedContext = await createSeedContextScript({ seedRoot, platform });
  }
  const appHost = hostFactory({
    host,
    port,
    root,
    publicRoot,
    contextScript: seedContext,
    agentEndpoint: null,
    adapterHome,
  });
...
function printUsage() {
  console.log(
    'Usage: node host/desktop-shell.mjs --root <frontend-dir> [--public-root PATH] [--seed-root PATH] [--adapter-home PATH] [--platform linux|win32|darwin] [--port N] [--chromium-binary PATH]',
  );
}
  ...
  (async () => {
    try {
      ...
      const session = await launchDesktopSession({
        ...
        publicRoot: path.resolve(
          readArgument('public-root', path.resolve(readArgument('root', process.cwd()), 'public')),
        ),
      ...
      console.log(
        JSON.stringify({
          service: 'spartan-desktop-shell',
          url: `http://${session.plan.host}:${session.plan.port}/dashboard/?startup=1`,
          binary: session.plan.binary,
          platform: session.plan.platform,
        }),
      );
      session.child.once('exit', () => {
        console.log('desktop shell window closed');
        process.exitCode = 0;
      });
      process.on('SIGINT', () => {
