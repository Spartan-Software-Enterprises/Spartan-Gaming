...
export function createNativeHostSession({platform, mediaPublisher, audioPublisher = null, inputExecutor = null, gameLauncher = null, requireAudio = false} = {}) {
  ...
  let state = 'idle'; let startedGame = false; let startedMedia = false; let startedAudio = false;
  ...
  const session = {
    ...
    async start() {
      ...
      state = 'preparing';
      try { if (gameLauncher) { await gameLauncher.start(); startedGame = true; } await mediaPublisher.start(); startedMedia = true; if (audioPublisher) { await audioPublisher.start(); startedAudio = true; } state = 'active'; return this; }
      catch (error) { state = 'failed'; if (startedAudio) await audioPublisher.stop().catch(() => {}); if (startedMedia) await mediaPublisher.stop().catch(() => {}); if (startedGame) await gameLauncher.stop().catch(() => {}); throw error; }
    },
    ...
    async stop() { if (state === 'stopped' || state === 'idle') { state = 'stopped'; return this; } state = 'stopping'; if (inputExecutor?.close) inputExecutor.close(); if (startedAudio) await audioPublisher.stop(); if (startedMedia) await mediaPublisher.stop(); if (startedGame) await gameLauncher.stop(); state = 'stopped'; return this; },
  };
