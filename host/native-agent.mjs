  ...
  return {
    ...
    emit(type, value) {
      for (const handler of listeners.get(type) || [])
        void Promise.resolve(handler(value)).catch((error) => {
          for (const observer of listeners.get('error') || []) observer(error);
        });
    },
  };
}
...
export function createNativeSignalingBridge({ connection } = {}) {
  if (!connection || typeof connection.send !== 'function')
    throw new TypeError('a connection with send(message) is required');
  const bus = eventBus();
  let closed = false;
  return Object.freeze({
    on: bus.on,
    emit(type, value) {
      if (!closed) bus.emit(type, value);
    },
    async connect() {
      if (closed) throw new Error('native signaling bridge is closed');
    },
    send(message) {
      if (closed) throw new Error('native signaling bridge is closed');
 ...
 */
export function createNativeWeriftConnection({
  connection,
  sessionId,
  bindings,
  module,
