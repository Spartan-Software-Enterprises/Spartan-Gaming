 ...
 */
export function createAppHostServer({
  host = '127.0.0.1',
  port = 4173,
  root,
  publicRoot,
  ...
  const resolveAsset =
    assets || createAssetResolver({ root: frontendRoot, publicRoot: publicDirectory, runtimeRoot });
  ...
  const server = createServer(async (request, response) => {
    ...
    if (url.pathname === '/health') {
      ...
      if (proxy) {
        try {
          agent = await fetchHealth(proxy.target);
        } catch {
    ...
    if (url.pathname === '/session') {
      ...
      response.end('use a WebSocket upgrade to connect to /session');
      return;
    }
  ...
  server.on('clientError', (error, socket) => {
    logger.warn?.(`app host client error: ${error.message}`);
...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  const appHost = createAppHostServer({
    host,
    port,
    root,
    publicRoot,
  ...
  appHost
    ...
    .then((address) =>
      console.log(
        JSON.stringify({
          service: 'spartan-app-host',
          url: `http://${address.address === '::' ? `[${address.address}]` : address.address}:${address.port}/dashboard/?startup=1`,
