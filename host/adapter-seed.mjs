...
export async function readSeedBundle({ seedRoot, platform = process.platform } = {}) {
  ...
  await statFile(artifacts).catch(() => {
    throw new Error(`seed artifact directory is missing: ${artifacts}`);
  });
...
export function createSeedArtifactResolver({ seedRoot, artifacts } = {}) {
  ...
  return async (url) => {
    ...
    if (!name || name === '.' || name === '..' || name.includes('/') || name.includes('\\'))
      throw new Error(`unsafe bundled artifact reference: ${url}`);
...
export async function createSeedContextScript({
  seedRoot,
  path = 'spartan-seed-context.js',
  platform = process.platform,
} = {}) {
  const bundle = await readSeedBundle({ seedRoot, platform });
  if (!(await verifyReleaseFeed({ feed: bundle.feed, publicKeyJwk: bundle.publicKeyJwk })))
    throw new Error('bundled release feed signature verification failed');
  const manifests = Object.freeze(
    bundle.feed.records.map((record) => record.package).filter(Boolean),
  );
  const render = () => {
    const body = [
 ...
 */
export async function runSeedInstall({
  seedRoot,
  installRoot,
  platform = process.platform,
  apply = false,
  ...
  if (!['win32', 'darwin', 'linux'].includes(platform))
    throw new TypeError(`unsupported seed platform: ${platform}`);
  const bundle = await readSeedBundle({ seedRoot, platform });
  if (!(await verifyReleaseFeed({ feed: bundle.feed, publicKeyJwk: bundle.publicKeyJwk })))
    throw new Error('bundled release feed signature verification failed');
  ...
  if (apply && installs.length) {
    ...
    const verifier = async (args) => {
      ...
      return verifyPackageManifestSignature({
        manifest: normalizeAdapterPackageManifest(source),
        publicKeyJwk: bundle.publicKeyJwk,
      });
    };
    const installer = createNativeAdapterInstaller({
      installRoot,
      download: artifactResolver,
