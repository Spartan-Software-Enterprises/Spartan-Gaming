...
test('platform runtime selects bounded capture and audio plans for every desktop OS', () => { for (const platform of ['win32', 'darwin', 'linux']) { const plans = createPlatformRuntimePlans({platform, environment: environments[platform], includeAudio: true, width: 1280, height: 720, framerate: 60}); assert.equal(plans.capture.platform, platform); assert.equal(plans.audio.platform, platform); assert.equal(plans.audioEncoder.codec, 'opus'); assert.equal(plans.encoder.codec, 'h264'); assert.equal(plans.capture.process.shell, false); assert.equal(plans.audioEncoder.process.shell, false); } });
