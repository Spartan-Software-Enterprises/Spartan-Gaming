...
export function createExecutablePlatformHost({platform, plans, pipeline = null, packetizer, transport, inputAdapter = null, adapterBoundary = null, permissions = {}, codec = 'h264', maxChunkBytes, spawnImpl, maxOutputBytes, stopTimeoutMs, audioPublisher = null, audioPipeline = null, audioPacketizer, audioTransport, audioCodec = 'opus', audioMaxChunkBytes, gameLauncher = null} = {}) {
  ...
  const audio = audioPublisher || (nativeAudioPipeline ? createRtpAudioPublisher({pipeline: nativeAudioPipeline, packetizer: audioPacketizer, transport: audioTransport, codec: audioCodec, container: plans.audioEncoder?.output?.container || 'raw', permissionGranted: permissions.microphone === true || permissions['microphone-capture'] === true, maxChunkBytes: audioMaxChunkBytes}) : null);
  ...
  const session = createNativeHostSession({platform, mediaPublisher: media.publisher, audioPublisher: audio?.publisher || audio, inputExecutor: input, gameLauncher});
