...
function normalizeWebRtcAdapters(value) {
  return Object.freeze(
    (Array.isArray(value) ? value : [])
      .filter(
        (adapter) =>
          adapter && typeof adapter.id === 'string' && WEBRTC_ADAPTER_STATES.has(adapter.state),
      )
      .map((adapter) => Object.freeze({ id: adapter.id, state: adapter.state })),
  );
}
 ...
 */
export function resolveHostVideoCapabilities({
  encoders = [],
  maxWidth = 3840,
  maxHeight = 2160,
  maxFramerate = 144,
  ...
  const hardware = Object.freeze(
    (Array.isArray(encoders) ? encoders : [])
      .filter((entry) => entry && VIDEO_CODECS.has(String(entry.codec)))
      .map((entry) =>
        Object.freeze({
  ...
  const codecs = Object.freeze(
    VIDEO_CODEC_ORDER.filter(
      (codec) => codec === 'h264' || codec === 'vp9' || hardwareCodecs.has(codec),
    ),
  );
  ...
  const framerate = boundedInteger(
    observedRate ? Math.min(observedRate, maxFramerate) : maxFramerate,
    144,
    24,
    240,
  );
...
export function normalizeHostCapabilities(capabilities = {}) {
  ...
  return Object.freeze({
    ...
    media: Object.freeze({
      state,
      capture: Boolean(media.capture),
      encode: Boolean(media.encode),
      audio: Boolean(media.audio),
      transports: list(media.transports, ['webrtc']),
    }),
    process: Object.freeze({
      mode: processMode,
      launch: Boolean(process.launch),
      emulator: Boolean(process.emulator),
    }),
    publisher: normalizePublisherCapabilities(capabilities.publisher),
    webrtc: Object.freeze({
      adapters: normalizeWebRtcAdapters(capabilities.webrtc?.adapters),
      ready: normalizeWebRtcAdapters(capabilities.webrtc?.adapters).some(
        (adapter) => adapter.state === 'available',
      ),
    }),
    ...
    input: Object.freeze({
      gamepad: capabilities.input?.gamepad !== false,
      virtualGamepad: Boolean(capabilities.input?.virtualGamepad ?? capabilities.input?.gamepad),
      keyboard: capabilities.input?.keyboard !== false,
      pointer: capabilities.input?.pointer !== false,
      rumble: Boolean(capabilities.input?.rumble),
      hid: Boolean(capabilities.input?.hid),
    }),
  });
}

export function hostMediaReady(capabilities) {
  const normalized = normalizeHostCapabilities(capabilities);
  return normalized.media.state === 'ready' || normalized.media.state === 'active';
}
