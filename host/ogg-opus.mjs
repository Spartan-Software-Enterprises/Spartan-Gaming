...
export function createOggOpusPacketExtractor({
  maxBufferedBytes = DEFAULT_MAX_BUFFERED_BYTES,
} = {}) {
  const limit =
    Number.isInteger(maxBufferedBytes) && maxBufferedBytes >= 65536
      ? maxBufferedBytes
  ...
  return Object.freeze({
    push(chunk) {
      ...
      while (pending.length >= 27) {
        if (pending.toString('latin1', 0, 4) !== 'OggS' || pending[4] !== 0)
          throw new Error('invalid Ogg Opus page');
        ...
        if (continued !== packetParts.length > 0)
          throw new Error('invalid Ogg Opus packet continuation');
        let offset = 27 + segmentCount;
        for (let index = 0; index < segmentCount; index += 1) {
          const size = pending[27 + index];
          packetParts.push(pending.subarray(offset, offset + size));
          packetBytes += size;
          offset += size;
          if (packetBytes > limit)
            throw new Error('Ogg Opus packet exceeds the buffered byte limit');
          if (size < 255) {
            const packet = Buffer.concat(packetParts, packetBytes);
            packetParts = [];
            packetBytes = 0;
            if (
