...
export function normalizeHostConfig(input = {}, {platform = process.platform} = {}) {
  ...
  const virtualGamepad = NATIVE_PLATFORMS.has(selectedPlatform) ? normalizeVirtualGamepadConfig({platform: selectedPlatform, backend: input.virtualGamepadBackend, packageName: input.virtualGamepadPackage, deviceId: input.virtualGamepadDevice}) : Object.freeze({version: 1, platform: selectedPlatform, backend: 'Disabled', packageName: null, deviceId: null, compatible: true, requires: Object.freeze([])});
