  ...
  return {
    on(type, handler) {
      if (typeof handler !== 'function')
        throw new TypeError('process event handler must be a function');
...
function snapshot(state, child, output, exit) {
...
export function createManagedProcess({
  ...
  if (!plan || plan.shell !== false || !Array.isArray(plan.args))
    throw new TypeError('a shell-free process plan is required');
  if (typeof spawnImpl !== 'function') throw new TypeError('spawnImpl must be a function');
  if (!Array.isArray(stdio) || stdio.length !== 3)
    throw new TypeError('stdio must contain stdin, stdout, and stderr entries');
  const outputLimit = bounded(maxOutputBytes, DEFAULT_MAX_OUTPUT_BYTES, 1024, 4 * 1024 * 1024);
  const timeout = bounded(stopTimeoutMs, DEFAULT_STOP_TIMEOUT_MS, 100, 30_000);
  const bus = events();
  let state = 'idle';
  let child = null;
  ...
  const emitState = (next) => {
    state = next;
    bus.emit('state', snapshot(state, child, output, exit));
  };
  const bindStream = (stream, name) =>
    stream?.on?.('data', (chunk) => {
      output = { ...output, [name]: append(output[name], chunk, outputLimit) };
      bus.emit(
        'output',
        Object.freeze({
          stream: name,
  ...
  const finish = (code, signal) => {
    exit = { code: code === undefined ? null : code, signal: signal || null };
    if (state !== 'stopping') emitState(code === 0 ? 'stopped' : 'failed');
    else emitState('stopped');
    bus.emit('exit', snapshot(state, child, output, exit));
  };
  const managed = {
    get state() {
      return state;
    ...
    get exit() {
      return exit ? Object.freeze({ ...exit }) : null;
    },
    on: bus.on,
    start() {
      ...
      startPromise = new Promise((resolve, reject) => {
        try {
          ...
          child.once?.('spawn', () => {
            spawned = true;
            emitState('running');
            resolve(snapshot(state, child, output, exit));
          });
    ...
    async stop({ signal = 'SIGTERM', timeoutMs = timeout } = {}) {
      if (!child || ['idle', 'stopped', 'failed'].includes(state))
        return snapshot(state, child, output, exit);
      if (state === 'starting') await startPromise.catch(() => {});
      if (!child || ['stopped', 'failed'].includes(state))
        return snapshot(state, child, output, exit);
      emitState('stopping');
      child.kill?.(signal);
      await new Promise((resolve) => {
        if (['stopped', 'failed'].includes(state)) return resolve();
        const timer = setTimeout(
          () => {
            if (!['stopped', 'failed'].includes(state)) child.kill?.('SIGKILL');
            resolve();
          },
          bounded(timeoutMs, timeout, 100, 30_000),
        );
        bus.on('exit', () => {
          clearTimeout(timer);
          resolve();
...
export function createProcessPipeline({
  plans = [],
  spawnImpl = spawn,
  maxOutputBytes,
  stopTimeoutMs,
  ...
  if (!Array.isArray(plans) || !plans.length)
    throw new TypeError('pipeline plans must contain at least one process plan');
  const processes = plans.map((plan) =>
    createManagedProcess({ plan, spawnImpl, maxOutputBytes, stopTimeoutMs }),
  );
  let state = 'idle';
  const pipeline = {
    get state() {
    ...
    async start() {
      if (state !== 'idle') throw new Error(`pipeline cannot start from ${state}`);
      state = 'starting';
      const started = [];
      try {
    ...
    async stop() {
      if (state === 'idle' || state === 'stopped') return this.processes;
      state = 'stopping';
