...
function collectedSessions() {
  const received = [];
  const first = {sessionId: 'ses-one', send: envelope => received.push({sessionId: 'ses-one', envelope})};
  const second = {sessionId: 'ses-two', send: envelope => received.push({sessionId: 'ses-two', envelope})};
...
test('rumble broadcast forwards bounded host rumble to every subscribed session', () => {
  ...
  const controller = createRumbleBroadcastController({adapter: {readRumbleEvents: () => [{strongMagnitude: 2, weakMagnitude: -1}]}});
  controller.add(first); controller.add(second);
  controller.poll();
...
test('rumble broadcast leaves sessions without a reader idle', () => {
  const {received, first} = collectedSessions();
  let intervalCount = 0;
  const controller = createRumbleBroadcastController({adapter: {}, setIntervalImpl: () => { intervalCount += 1; return 1; }, clearIntervalImpl: () => { intervalCount -= 1; }});
  controller.attach();
...
test('rumble broadcast polls the adapter reader while attached', () => {
  ...
  const ticks = [];
  let onTick = null;
  const controller = createRumbleBroadcastController({adapter: reader, intervalMs: 10, setIntervalImpl: fn => { ticks.push(10); onTick = fn; return {fn}; }, clearIntervalImpl: () => { ticks.push('clear'); }});
  controller.add(first);
  controller.attach();
...
test('rumble broadcast tolerates a failing reader', () => {
  const {received, first} = collectedSessions();
  const controller = createRumbleBroadcastController({adapter: {readRumbleEvents: () => { throw new Error('reader unavailable'); }}});
  controller.add(first);
  controller.poll();
  assert.equal(received.length, 0);
});
...
test('rumble broadcast ignores malformed rumble payloads', () => {
  const {received, first} = collectedSessions();
  const controller = createRumbleBroadcastController({adapter: {readRumbleEvents: () => [null, {strongMagnitude: 'x'}]}});
  controller.add(first);
  controller.poll();
  assert.equal(received.length, 2);
  for (const {envelope} of received) {
