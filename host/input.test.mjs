...
import {createInputInjectionPlan, inputAdapterIsReady, normalizeInputAdapterCapabilities} from './input.mjs';
...
test('input injection plans cover keyboard, pointer, gamepad, and rumble without shell execution', () => {
  const keyboard = createInputInjectionPlan({platform: 'win32', event: {type: 'input.event', kind: 'key', source: 'keyboard', action: 'confirm', control: 'Enter', pressed: true}, permissions: {'remote-input': true}});
  ...
  const pointer = createInputInjectionPlan({platform: 'linux', event: {type: 'input.event', kind: 'pointer', source: 'pointer', action: 'look', x: 2, y: -1, deltaX: 9000}, permissions: {'remote-input': true}});
