...
} from '../src/frontend/adapters/manifest-registry.mjs';
...
export function normalizeReleaseFeed(value) {
  if (!value || typeof value !== 'object' || Array.isArray(value))
    throw new TypeError('release feed must be an object');
  ...
  if (!Array.isArray(value.records) || value.records.length > 500)
    throw new TypeError('release feed.records must contain up to 500 entries');
  const records = value.records.map(normalizeAdapterManifest);
  const seen = new Set();
  for (const record of records) {
    if (seen.has(record.id)) throw new Error(`duplicate release feed entry: ${record.id}`);
    seen.add(record.id);
  }
  return Object.freeze({
    ...
    signature: value.signature
      ? Object.freeze({
          algorithm: required(value.signature.algorithm, 'release feed.signature.algorithm'),
          signer: required(value.signature.signer, 'release feed.signature.signer'),
          value: required(value.signature.value, 'release feed.signature.value'),
        })
      : null,
  });
}
...
export async function verifyReleaseFeed({
  feed,
  publicKeyJwk,
...
export function createReleaseFeedPlanner({
  feed,
  platform,
  kind,
  installed = [],
  ...
  const normalized = normalizeReleaseFeed(feed);
  if (platform && !['win32', 'darwin', 'linux', 'browser'].includes(platform))
    throw new TypeError(`unsupported release feed platform: ${platform}`);
    ...
    .map((record) => {
      ...
      if (!installedRecord)
        return Object.freeze({
          ...
          candidate: record,
          readiness: registry.resolve(record.id, { kind: record.kind, allowUnsigned }),
        });
...
export function createReleaseInstallRequest({ plan, platform, consent = false } = {}) {
  ...
  const updatePlan =
    plan.status === 'install-available'
      ? Object.freeze({
          status: 'update-available',
          id: plan.candidate.id,
          from: plan.candidate.version,
          to: plan.candidate.version,
