...
export function createPairingCode(length = 8) {
  if (!Number.isInteger(length) || length < 6 || length > 12) throw new TypeError('pairing code length must be between 6 and 12');
...
export function normalizePairingCode(value) {
  ...
  if (!CODE_PATTERN.test(code)) throw new TypeError('pairing code must use the safe 6–12 character alphabet');
  return code;
}
...
export function createPairingAuthority({code = createPairingCode(), expiresAt = Date.now() + 5 * 60 * 1000} = {}) {
  const expected = normalizePairingCode(code); let used = false;
