...
test('Werift host runtime forwards session control requests', async () => { const signaling = signal(); const session = {async acceptOffer() { return {type: 'answer', sdp: 'answer'}; }, async addIceCandidate() {}, async close() {}}; const controls = []; const runtime = createWeriftHostRuntime({signaling, sessionId: 'ses-control', onControl: value => controls.push(value), sessionFactory: async () => session}); await runtime.start(); signaling.emit('message', createSessionEnvelope({sessionId: 'ses-control', type: 'session.offer', payload: {sdp: {type: 'offer', sdp: 'offer'}, transports: ['webrtc'], video: {codecs: ['h264']}, audio: {codecs: ['opus']}}})); await new Promise(resolve => setTimeout(resolve, 0)); signaling.emit('message', createSessionEnvelope({sessionId: 'ses-control', type: 'session.control', sequence: 2, payload: {action: 'pause'}})); await new Promise(resolve => setTimeout(resolve, 0)); assert.deepEqual(controls, [{action: 'pause'}]); runtime.close(); });
