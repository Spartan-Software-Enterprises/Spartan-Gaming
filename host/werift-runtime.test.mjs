...
test('Werift host runtime negotiates SDP and routes session messages', async () => {
  ...
  const session = {
    ...
    async applyQualityRequest(value) {
      this.quality = value;
  ...
  const runtime = createWeriftHostRuntime({
    ...
    sessionFactory: async (options) => {
      options.onIceCandidate({ candidate: 'local' });
      return session;
    },
    onInput: (value) => inputs.push(value),
    onQuality: (value, result) => {
  ...
  assert.deepEqual(session.quality, { profile: 'low' });
  assert.deepEqual(qualityResults, [{ status: 'applied', applied: true }]);
  runtime.close();
...
test('Werift host runtime rejects offers without SDP and reports errors', async () => {
  ...
  signaling.emit(
    ...
    createSessionEnvelope({
      sessionId: 'ses-error',
      type: 'session.offer',
      payload: { transports: ['webrtc'], video: { codecs: ['h264'] }, audio: { codecs: ['opus'] } },
    }),
  );
  await new Promise((resolve) => setTimeout(resolve, 0));
...
test('Werift host runtime rolls back a failed session factory so the next offer can retry', async () => {
  ...
  const session = {
    async acceptOffer() {
      return { type: 'answer', sdp: 'retry-answer' };
    },
  ...
  const runtime = createWeriftHostRuntime({
    ...
    sessionFactory: async () => {
      attempts += 1;
      if (attempts === 1) throw new Error('media setup failed');
      return session;
    },
  });
  await runtime.start();
  ...
  assert.equal(signaling.sent.at(-1).payload.sdp.sdp, 'retry-answer');
  runtime.close();
});
test('Werift host runtime advertises active media audio only when enabled', async () => {
  const signaling = signal();
  const session = {
    async acceptOffer() {
      return { type: 'answer', sdp: 'answer' };
  ...
  await new Promise((resolve) => setTimeout(resolve, 0));
  assert.equal(signaling.sent.at(-1).payload.hostCapabilities.media.audio, true);
  runtime.close();
});
test('Werift host runtime forwards session control requests', async () => {
  const signaling = signal();
  const session = {
    async acceptOffer() {
      return { type: 'answer', sdp: 'answer' };
...
test('Werift host runtime validates and forwards metadata-only launch requests', async () => {
  const signaling = signal();
  const session = {
    async acceptOffer() {
      return { type: 'answer', sdp: 'answer' };
    ...
    createSessionEnvelope({
      ...
      payload: {
        ...
        launch: {
          ...
          content: { game: { name: 'game.iso', size: 10 }, firmware: [] },
  ...
  await new Promise((resolve) => setTimeout(resolve, 0));
  assert.equal(received.hostContentId, 'entry-1');
  runtime.close();
});
