...
test('VP9 packetizer uses B and E bits for fragmented frame boundaries', () => {
  const packets = createRtpPacketizer({codec: 'vp9', mtu: 256}).push(Buffer.alloc(500), {timestamp: 42});
  assert.equal(packets[0].payload[0] & 0x0c, 0x08); assert.equal(packets.at(-1).payload[0] & 0x0c, 0x04);
...
test('AV1 packetizer signals only coded sequence starts', () => {
  ...
  const first = packetizer.push(Buffer.from('first'))[0]; const next = packetizer.push(Buffer.from('next'))[0]; const restarted = packetizer.push(Buffer.from('restart'), {newCodedVideoSequence: true})[0];
  assert.equal(first.payload[0] & 0x18, 0x18); assert.equal(next.payload[0] & 0x18, 0x10); assert.equal(restarted.payload[0] & 0x18, 0x18);
});
...
test('Opus packetizer preserves an encoded frame without a video payload descriptor', () => {
  ...
  assert.equal(packet.header.marker, true); assert.equal(packet.header.timestamp, 42); assert.deepEqual(packet.payload, frame);
...
test('VP9 packetizer extracts On2 IVF frames from ffmpeg -f ivf output', () => {
  const frameA = Buffer.from('vp9-frame-a'); const frameB = Buffer.from('vp9-frame-b');
  const header = Buffer.alloc(32); header.write('DKIF', 0, 'latin1'); header.writeUInt32LE(90000, 16); header.writeUInt32LE(1, 20);
  const sizeA = Buffer.alloc(4); sizeA.writeUInt32LE(frameA.length, 0);
  const sizeB = Buffer.alloc(4); sizeB.writeUInt32LE(frameB.length, 0);
  const timestampA = Buffer.alloc(8); timestampA.writeBigUInt64LE(42n);
  const timestampB = Buffer.alloc(8); timestampB.writeBigUInt64LE(84n);
  ...
  assert.equal(packets.length, 2); assert.equal(packets[0].payload.subarray(1).toString(), 'vp9-frame-a'); assert.equal(packets[1].payload.subarray(1).toString(), 'vp9-frame-b'); assert.equal(packets[0].payload[0] & 0x0c, 0x0c); assert.equal(packets[0].payload[0] & 0x82, 0); assert.equal(packets[0].header.timestamp, 42); assert.equal(packets[1].header.timestamp, 84); assert.equal(packets[0].header.marker, true); assert.equal(packets[1].header.marker, true);
});
