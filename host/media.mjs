...
const HARDWARE_ENCODERS = Object.freeze({
  linux: Object.freeze({
    h264: ['h264_vaapi', 'h264_nvenc', 'h264_qsv', 'h264_v4l2m2m'],
    vp9: ['vp9_vaapi', 'vp9_qsv'],
    av1: ['av1_vaapi', 'av1_nvenc', 'av1_qsv'],
  }),
  win32: Object.freeze({
    h264: ['h264_nvenc', 'h264_qsv', 'h264_amf', 'h264_mf'],
    vp9: ['vp9_qsv', 'vp9_nvenc'],
...
export function resolveFfmpegCommand(environment = {}) {
  const configured =
    typeof environment.SPARTAN_FFMPEG_COMMAND === 'string'
      ? environment.SPARTAN_FFMPEG_COMMAND.trim()
      : '';
  if (!configured) return 'ffmpeg';
  if (configured.length > 256 || /[\u0000-\u001f\u007f\s]/.test(configured))
    throw new TypeError('SPARTAN_FFMPEG_COMMAND must be a shell-free executable path or name');
  return configured;
}
...
export function createFfmpegEncoderProbe({ command = 'ffmpeg', run = commandOutputProbe } = {}) {
  ...
  return (encoder) => {
    if (cached === null)
      cached = listFfmpegEncoders(run(command, ['-hide_banner', '-encoders'], { stdio: 'pipe' }));
      ...
      Object.freeze({
        ...
        requires:
          platform === 'darwin'
            ...
            : platform === 'linux' && sourceType === 'pipewire'
...
export function validateCapturePermission({ platform, sourceType, environment = {} } = {}) {
  if (!PLATFORMS.has(platform) || !SOURCE_TYPES.has(sourceType))
    return Object.freeze({ allowed: false, reason: 'Unsupported capture platform or source' });
  if (platform === 'linux' && sourceType === 'x11' && !environment.DISPLAY)
    return Object.freeze({ allowed: false, reason: 'DISPLAY is not set' });
  if (
    platform === 'linux' &&
    sourceType === 'pipewire' &&
  ...
  if (platform === 'darwin' && environment.screenRecordingGranted !== true)
    return Object.freeze({ allowed: false, reason: 'Screen Recording permission is not granted' });
...
export function createCapturePlan({
  ...
  if (!PLATFORMS.has(platform) || !SOURCE_TYPES.has(sourceType))
    throw new TypeError('unsupported capture platform or source');
  const permission = validateCapturePermission({ platform, sourceType, environment });
  if (!permission.allowed) throw new Error(permission.reason);
  const size = `${positiveInteger(width, 'width', 1920, 7680)}x${positiveInteger(height, 'height', 1080, 4320)}`;
  const fps = positiveInteger(framerate, 'framerate', 60, 240);
  const args = ['-hide_banner', '-loglevel', 'warning'];
  if (platform === 'linux' && sourceType === 'x11')
    args.push('-f', 'x11grab', '-video_size', size, '-framerate', String(fps), '-i', source);
  else if (platform === 'linux' && sourceType === 'pipewire')
    args.push('-f', 'pipewire', '-framerate', String(fps), '-i', source);
    ...
    args.push(
      ...
      size,
      '-i',
      audio ? `${source}:default` : `${source}:none`,
    );
...
function probeEncoders(probe = null, platform = null, codec = null) {
  ...
  const candidates =
    platform && HARDWARE_ENCODERS[platform]?.[codec] ? HARDWARE_ENCODERS[platform][codec] : [];
...
export function selectHardwareEncoder({
  ...
  const devicePath =
    device ||
    (encoder === 'h264_vaapi' || encoder === 'vp9_vaapi' || encoder === 'av1_vaapi'
      ? VAAPI_DEVICE.find((candidate) => probe?.(candidate) !== false) || VAAPI_DEVICE[0]
      : null);
  return Object.freeze({
    encoder,
    platform,
...
export function createEncoderPlan({
  codec = 'h264',
  width = 1920,
  height = 1080,
  framerate = 60,
  ...
  probe = null,
  device = null,
  outputFormat = ELEMENTARY_FORMATS[codec],
  ffmpegCommand = 'ffmpeg',
} = {}) {
  if (!CODECS[codec]) throw new TypeError(`unsupported encoder codec: ${codec}`);
  const fps = positiveInteger(framerate, 'framerate', 60, 240);
  const boundedBitrate = positiveInteger(bitrateKbps, 'bitrateKbps', 10000, 1000000);
  const selected = preferHardware
    ? selectHardwareEncoder({ codec, platform, probe, device })
    : null;
  ...
  return Object.freeze({
    ...
    codec,
    outputFormat,
    width: positiveInteger(width, 'width', 1920, 7680),
    height: positiveInteger(height, 'height', 1080, 4320),
    framerate: fps,
    bitrateKbps: boundedBitrate,
    preference: hardware
      ? 'hardware'
      : preferHardware
        ? 'hardware-when-platform-adapter-provides-it'
        : 'software',
    hardware,
    encoder,
    device: selected?.device || null,
    availableEncoders: selected?.available || Object.freeze([]),
    ...
    requires: Object.freeze(
      hardware
        ? ['hardware-encoder-device', 'webrtc-publisher']
        : preferHardware
          ? ['platform-encoder-selection', 'webrtc-publisher']
          : ['webrtc-publisher'],
    ),
  });
}
