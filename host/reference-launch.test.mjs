...
function request() {
  return createNativeHostLaunchRequest({
    plan: {
      status: 'ready',
      coreId: 'dolphin',
      files: [{ kind: 'game', name: 'game.iso', size: 1, userSelected: true }],
      integration: {
    ...
    hostContentId: 'dolphin-main',
    consent: true,
  });
}

test('reference game launch config matches the host-local game before starting', async () => {
  const configured = createReferenceGameLaunch({
    platform: 'linux',
    runtimeId: 'dolphin',
    runtimePath: '/usr/bin/dolphin',
    gamePath: '/games/game.iso',
    hostContentId: 'dolphin-main',
    spawnImpl,
  });
...
test('reference Proton launch config keeps the runtime path host-local and matches consent metadata', async () => {
  const configured = createReferenceProtonLaunch({
    runtimeId: 'proton',
    protonPath: '/opt/Proton/proton',
    runtimeVersion: '9.0',
    gamePath: '/games/game.exe',
    hostContentId: 'proton-main',
    args: ['-novid'],
    spawnImpl,
  });
  assert.equal(configured.descriptor.runtimeKind, 'proton');
  assert.equal(configured.runtimeProfile.kind, 'proton');
  assert.deepEqual(configured.launcher.plan.process.args, ['run', '/games/game.exe', '-novid']);
  const launch = request();
  const protonRequest = {
    ...launch,
    runtime: { ...launch.runtime, id: 'proton', kind: 'proton', version: '9.0' },
    hostContentId: 'proton-main',
    content: { ...launch.content, game: { ...launch.content.game, name: 'game.exe' } },
  };
  assert.equal(configured.matches(protonRequest), true);
  await configured.launcher.start();
  await configured.launcher.stop();
});
