...
import {resolveChromiumBinary, createDesktopShellPlan, launchDesktopSession} from './desktop-shell.mjs';

test('desktop shell resolves a Chromium binary from an override or PATH candidates', () => {
  assert.equal(resolveChromiumBinary('/opt/fake/chromium', {exists: () => true}), '/opt/fake/chromium');
  ...
  assert.equal(resolveChromiumBinary('', {exists: () => true, find: candidate => `/usr/bin/${candidate}`}), '/usr/bin/chromium');
});

test('desktop shell plan targets the loopback app URL in app mode', () => {
  const plan = createDesktopShellPlan({host: '127.0.0.1', port: 4411, platform: 'linux', resolveBinary: () => '/opt/fake/chromium'});
...
test('desktop shell exposes an explicit sandbox compatibility switch', () => {
  ...
  try { assert.ok(createDesktopShellPlan({platform: 'linux', resolveBinary: () => '/opt/fake/chromium'}).args.includes('--no-sandbox')); }
  finally { if (previous === undefined) delete process.env.SPARTAN_DISABLE_CHROMIUM_SANDBOX; else process.env.SPARTAN_DISABLE_CHROMIUM_SANDBOX = previous; }
});

test('desktop shell fails closed without a usable Chromium binary', () => {
  assert.throws(() => createDesktopShellPlan({resolveBinary: () => null}), /no Chromium-based desktop shell/);
});
...
test('desktop session starts the host, opens the window, and closes the host with it', async () => {
  ...
  const events = [];
  const fakeHost = {
    listen: async () => { events.push('listen'); return {address: '127.0.0.1', port: 4412}; },
    close: async () => { events.push('close'); },
  };
  const fakeChild = new EventEmitter();
  fakeChild.killed = false;
  fakeChild.kill = () => { fakeChild.killed = true; events.push('kill'); };
  const session = await launchDesktopSession({
    root: '/tmp/root', publicRoot: '/tmp/public', host: '127.0.0.1', port: 4412, platform: 'linux',
    chromiumBinary: fakeBinary, hostFactory: () => fakeHost, spawnImpl: () => fakeChild, healthCheck: async () => true,
  });
...
test('desktop session aborts and closes the host when health never reports ready', async () => {
  ...
  await assert.rejects(() => launchDesktopSession({
    root: '/tmp/root', publicRoot: '/tmp/public', platform: 'linux',
    chromiumBinary: '/opt/fake/chromium', hostFactory: () => fakeHost, healthCheck: async () => false, resolveBinary: () => '/opt/fake/chromium',
