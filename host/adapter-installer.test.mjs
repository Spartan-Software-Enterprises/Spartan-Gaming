...
import {
  createAdapterInstallTransactionPlan,
  createNativeAdapterInstaller,
} from './adapter-installer.mjs';
...
const request = {
  ...
  artifact: {
    url: 'https://downloads.example.test/dolphin',
...
test('adapter install plans reject unsafe paths and mismatched verification metadata', () => {
  assert.throws(
    () =>
      createAdapterInstallTransactionPlan({
        request: { ...request, id: '../escape' },
  ...
  assert.throws(
    () =>
      createAdapterInstallTransactionPlan({
        request: {
          ...request,
          verification: { ...request.verification, integrity: 'sha256-other' },
        },
        installRoot: '/tmp/adapters',
      }),
    /match/,
...
test('native adapter installer verifies, stages, publishes, and writes an atomic pointer', async () => {
  ...
  try {
    const installer = createNativeAdapterInstaller({
      ...
      verifySignature: async ({ data, signature }) =>
        data.byteLength === payload.byteLength && signature.signer === 'release',
    });
    const result = await installer.install(request);
    assert.equal(result.status, 'installed');
    assert.equal(
      await fs.readFile(join(root, 'dolphin-native', 'current.json'), 'utf8'),
      JSON.stringify({ id: 'dolphin-native', version: '1.1.0', integrity }),
    );
    assert.equal(
      (await fs.readFile(join(root, 'dolphin-native', '1.1.0', 'adapter.artifact'))).toString(),
      'signed-adapter-payload',
    );
  } finally {
...
test('native adapter installer rolls back staging and publication on verification failure', async () => {
  const root = await fs.mkdtemp(join(tmpdir(), 'spartan-adapter-'));
  try {
    const installer = createNativeAdapterInstaller({
      installRoot: root,
    ...
    await assert.rejects(() => installer.install(request), /signature/);
    await assert.rejects(() => fs.access(join(root, 'dolphin-native', '1.1.0')));
  } finally {
    await fs.rm(root, { recursive: true, force: true });
  }
});
test('native adapter installer fails closed when no signature verifier is supplied', async () => {
  const root = await fs.mkdtemp(join(tmpdir(), 'spartan-adapter-'));
  try {
    const installer = createNativeAdapterInstaller({
      installRoot: root,
    ...
    await assert.rejects(() => installer.install(request), /verifier/);
  } finally {
    await fs.rm(root, { recursive: true, force: true });
  }
});
