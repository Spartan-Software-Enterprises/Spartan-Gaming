...
import {detectWebRtcAdapters, listWebRtcAdapters, selectWebRtcAdapter, webRtcReadiness} from './webrtc.mjs';

test('WebRTC adapter detection stays optional and reports available implementations', async () => {
  const adapters = await detectWebRtcAdapters({loader: async packageName => packageName === 'werift' ? {name: packageName} : Promise.reject(new Error('not installed'))});
  assert.deepEqual(listWebRtcAdapters().map(adapter => adapter.id), ['node-datachannel', 'werift']);
...
test('WebRTC readiness reports the install requirement when no adapter is present', async () => {
  const adapters = await detectWebRtcAdapters({loader: async () => { throw new Error('missing'); }});
  assert.equal(selectWebRtcAdapter(adapters), null);
  assert.deepEqual(webRtcReadiness(adapters), {ready: false, available: [], requires: ['optional-webrtc-adapter']});
});
