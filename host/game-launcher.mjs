...
function requiredText(value, name, maximum = 1024) {
  if (
    ...
    throw new TypeError(`${name} must be a bounded local string`);
  return value.trim();
}
...
function localPath(value, name) {
  ...
  if (/^[a-z]+:\/\//i.test(path) || path.startsWith('\\\\'))
    throw new TypeError(`${name} must be a local path, not a URL or network path`);
  return path;
}
...
function stringList(value, name) {
  if (
    ...
    throw new TypeError(`${name} must contain bounded argument strings`);
...
export function createGameLaunchPlan({
  platform,
  runtimeProfile,
  executablePath = runtimeProfile?.executablePath,
  gamePath,
  args = [],
  cwd,
  env = {},
  ...
  if (!runtimeProfile || !RUNTIME_KINDS.has(runtimeProfile.kind))
    throw new TypeError('runtimeProfile must be a trusted native emulator profile');
  if (runtimeProfile.enabled === false || runtimeProfile.trust === 'unverified')
    throw new Error('runtime profile is not enabled and trusted');
  ...
  return Object.freeze({
    ...
    runtime: Object.freeze({
      ...
      version: requiredText(runtimeProfile.version || 'unversioned', 'runtimeProfile.version', 80),
    }),
    gamePath: content,
    process,
...
export function createGameLauncher({ plan, spawnImpl, maxOutputBytes, stopTimeoutMs } = {}) {
  if (plan?.kind !== 'game-launch' || plan.process?.shell !== false)
    throw new TypeError('a game-launch plan is required');
