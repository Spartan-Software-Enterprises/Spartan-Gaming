...
test('reference host completes a paired direct session and records control traffic', async () => {
  ...
  try {
    ...
    await new Promise(resolve => setTimeout(resolve, 60)); const health = await fetch(info.health).then(response => response.json()); assert.equal(health.activeSessions, 1); assert.equal(health.inputEvents, 1); assert.equal(health.lastQuality.profile, 'low'); assert.equal(health.lastInputExecution.state, 'disabled'); assert.equal(health.pairingUsed, true);
