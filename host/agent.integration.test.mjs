...
function frame(value) {
  ...
  const mask = Buffer.from([0x52, 0x70, 0x61, 0x72]);
...
function socketClient(port) {
  ...
  const pump = () => {
    ...
    while (buffer.length >= 2) {
      ...
      if (length === 127 || buffer.length < header + length) return;
      const payload = buffer.subarray(header, header + length).toString('utf8');
  ...
  const ready = new Promise((resolve, reject) => {
    socket.once('connect', () => {
      ...
      const check = () => (handshake ? resolve() : setTimeout(check, 5));
      check();
    });
...
async function startAgent(extra = []) {
  ...
  const info = await new Promise((resolve, reject) => {
    let pending = '';
    const timer = setTimeout(() => reject(new Error('reference host did not start')), 5000);
...
test('reference host completes a paired direct session and records control traffic', async () => {
  const { child, info } = await startAgent();
  const client = socketClient(new URL(info.endpoint).port);
  const manager = createSessionManager({ idFactory: () => 'ses-host-integration' });
  try {
    await client.ready;
    ...
    assert.equal(answer.type, 'session.answer');
    assert.equal(answer.payload.accepted, true);
    assert.equal(answer.payload.hostId, 'host-integration');
    manager.receive(answer);
    ...
    const health = await fetch(info.health).then((response) => response.json());
    assert.equal(health.secure, false);
    assert.deepEqual(health.limits, { maxConnections: 8, maxMessagesPerSecond: 120 });
    assert.equal(health.runtimePolicy.videoCodec, 'Automatic');
    assert.equal(health.runtimePolicy.maxResolution, '1080p');
    assert.equal(health.runtimePolicy.enableInput, false);
    assert.equal(health.controllerPolicy.version, 1);
    assert.equal(health.controllerPolicy.playerSlots, 4);
    assert.equal(health.controllerPolicy.allowGamepad, true);
    assert.equal(health.connections, 1);
    assert.equal(health.rejectedConnections, 0);
    assert.equal(health.activeSessions, 1);
    assert.equal(health.inputEvents, 1);
    assert.equal(health.lastQuality.profile, 'low');
    assert.equal(health.lastInputExecution.state, 'disabled');
    assert.equal(health.pairingUsed, true);
  } finally {
    client.close();
    child.kill();
...
test('reference host launches a matched native game request and stops it on session close', async () => {
  ...
  const client = socketClient(new URL(info.endpoint).port);
  const manager = createSessionManager({ idFactory: () => 'ses-game-launch' });
  try {
    await client.ready;
    const launch = createNativeHostLaunchRequest({
      plan: {
        status: 'ready',
        coreId: 'node-runtime',
        files: [{ kind: 'game', name: 'spartan-test.iso', size: 1, userSelected: true }],
        integration: {
          runtime: 'native-emulator',
          runtimeProfile: {
            id: 'node-runtime',
    ...
    const answer = JSON.parse(await client.next());
    assert.equal(answer.payload.accepted, true);
    const running = await fetch(info.health).then((response) => response.json());
    assert.equal(running.gameLaunch.state, 'running');
    assert.ok(running.gameLaunch.pid);
    client.send(manager.close());
    await new Promise((resolve) => setTimeout(resolve, 80));
    const stopped = await fetch(info.health).then((response) => response.json());
    assert.equal(stopped.gameLaunch.state, 'stopped');
  } finally {
    client.close();
    child.kill();
    await new Promise((resolve) => child.once('exit', resolve));
...
test('reference host shuts down gracefully and stops a managed game launch on SIGTERM', async () => {
  ...
  const client = socketClient(new URL(info.endpoint).port);
  const manager = createSessionManager({ idFactory: () => 'ses-shutdown' });
  try {
    await client.ready;
    const launch = createNativeHostLaunchRequest({
      plan: {
