...
function h264Nalus(value) {
  ...
  while (cursor < value.length) {
    ...
    const next = (() => {
      for (let index = cursor; index < value.length; index += 1)
        if (startCodeLength(value, index)) return index;
      return value.length;
    })();
  ...
  if (value.length >= 4) {
    ...
    while (offset + 4 <= value.length) {
      ...
      avcc.push(value.subarray(offset, offset + length));
      offset += length;
    }
...
function packetizeH264(value, { maxPayload, make, timestamp }) {
  ...
  nalus.forEach((nalu, naluIndex) => {
    ...
    output.push(
      ...fragment(nalu.subarray(1), maxPayload - 2, (part, first, last) => {
...
function ivfFrames(value) {
  ...
  while (offset + 12 <= value.length) {
    ...
    const rtpTimestamp =
      rate && scale
        ? Number(((timestamp * BigInt(scale) * 90000n) / BigInt(rate)) & 0xffffffffn)
        : Number(timestamp & 0xffffffffn);
    frames.push(
      Object.freeze({ payload: value.subarray(offset, offset + size), timestamp: rtpTimestamp }),
    );
    offset += size;
  }
...
function packetizeDescriptor(value, { maxPayload, make, descriptor }) {
  ...
  if (value.length <= maxPayload - descriptorSize)
    return [make(Buffer.concat([descriptor(true, true), value]), true)];
  return fragment(value, maxPayload - descriptorSize, (part, first, last) =>
    make(Buffer.concat([descriptor(first, last), part]), last),
  );
}
 ...
 */
export function createRtpPacketizer({
  codec = 'h264',
  ssrc = 1,
  payloadType = 96,
  mtu = DEFAULT_MTU,
  ...
  const make = (payload, marker, timestamp) =>
    packetFactory({
      RtpPacket,
      RtpHeader,
      payload,
      marker,
      payloadType,
      sequenceNumber: source.sequence++ & 0xffff,
      timestamp,
      ssrc,
    });
  return Object.freeze({
    ...
    push(chunk, { timestamp = 0, newCodedVideoSequence = false } = {}) {
      ...
      if (codec === 'h264')
        return packetizeH264(value, {
          maxPayload,
          timestamp: time,
          make: (payload, marker) => make(payload, marker, time),
        });
      if (codec === 'opus') {
        if (value.length > maxPayload)
          throw new Error('Opus frame exceeds RTP packetizer payload limit');
        return [make(value, true, time)];
      }
      const frames =
        codec === 'vp9'
          ? ivfFrames(value) || [Object.freeze({ payload: value, timestamp: time })]
          : [Object.freeze({ payload: value, timestamp: time })];
      const output = [];
      for (const frame of frames) {
        if (!frame.payload.length) continue;
        if (codec === 'vp9')
          output.push(
            ...packetizeDescriptor(frame.payload, {
              maxPayload,
              make: (payload, marker) => make(payload, marker, frame.timestamp),
              descriptor: (first, last) => Buffer.from([(first ? 0x08 : 0) | (last ? 0x04 : 0)]),
            }),
          );
        else {
