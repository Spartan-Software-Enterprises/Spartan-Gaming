 ...
 */
export function createNativeWeriftHost({
  signaling,
  module,
  pipeline,
  capturePlan,
  ...
  peerConfig,
  codec = 'h264',
  framerate = encoderPlan?.framerate ?? 60,
  ssrc,
  streamId,
  label,
  maxChunkBytes,
  ...
  audioContainer = 'raw',
  audioSsrc = 2,
  audioStreamId = 'spartan-stream',
  audioLabel = 'Spartan Gaming Audio',
  ...
  required(packetizer, 'packetizer');
  const nativePipeline =
    pipeline ||
    createNativeMediaPipeline({
      capturePlan,
      encoderPlan,
      spawnImpl,
      maxOutputBytes,
  ...
  const nativeInput =
    ...
    (inputAdapter
      ? createNativeInputExecutor({
          platform: inputAdapter.platform,
  ...
  const runtime = createWeriftHostRuntime({
    ...
    sessionFactory: async ({ onIceCandidate, onStateChange, launch }) => {
      if (gameLauncher && (!launchValidator || !launch || !launchValidator(launch)))
        throw new Error('native launch request does not match the configured host content');
      mediaPublisher = createWeriftRtpPublisher({
        module,
      ...
      if (audioPipeline || audioPacketizer) {
        if (!audioPipeline || !audioPacketizer)
          throw new TypeError('audioPipeline and audioPacketizer are required together');
        audioPublisher = createWeriftAudioRtpPublisher({
          module,
          pipeline: audioPipeline,
          packetizer: audioPacketizer,
          peer: mediaPublisher.adapter.peer,
          codec: audioCodec,
          ...
          maxChunkBytes: audioMaxChunkBytes,
        });
      }
      const session = createWeriftSession({
        adapter: mediaPublisher.adapter,
        onIceCandidate,
        onStateChange,
      });
      let started = false;
      ...
      return {
        async acceptOffer(offer) {
          try {
            ...
            await mediaPublisher.publisher.start();
            started = true;
            if (audioPublisher) {
              await audioPublisher.publisher.start();
              audioStarted = true;
            }
            return await session.acceptOffer(offer);
          } catch (error) {
            await session.close();
            if (audioStarted) await audioPublisher.publisher.stop();
            if (started) await mediaPublisher.publisher.stop();
            if (gameStarted) {
              await gameLauncher.stop();
              gameStarted = false;
            }
            throw error;
          }
        ...
        async close() {
          await session.close();
          if (audioPublisher) await audioPublisher.publisher.stop();
          await mediaPublisher.publisher.stop();
          if (gameStarted) {
            await gameLauncher.stop();
            gameStarted = false;
          }
...
export function createNativeWeriftHostFromPlatformBindings({
  ...
  if (bindings.platform !== platform)
    throw new TypeError('platform bindings must match the requested platform');
  if (typeof bindings.capture?.plan !== 'function')
    throw new TypeError('platform bindings must provide capture.plan(options)');
  if (runtimeProfile && gamePath && !hostContentId)
    throw new TypeError('hostContentId is required when native game launch is configured');
  const capturePermissionGranted =
    captureOptions.permissionGranted === true ||
    permissions['screen-capture'] === true ||
    permissions['screen-recording'] === true;
  const capturePlan = bindings.capture.plan({
    ...captureOptions,
    width,
    height,
    framerate,
    permissionGranted: capturePermissionGranted,
  });
  ...
  const videoPacketizer =
    options.packetizer ||
    createRtpPacketizer({
      codec,
      ssrc: options.ssrc ?? 1,
      RtpPacket: options.module?.RtpPacket,
      RtpHeader: options.module?.RtpHeader,
    });
  const gameLauncher =
    runtimeProfile && gamePath
      ? createGameLauncher({
          plan: createGameLaunchPlan({
            platform,
            runtimeProfile,
            gamePath,
            args: gameArgs,
  ...
  if (includeAudio) {
    if (typeof bindings.audio?.plan !== 'function')
      throw new TypeError(
        'platform bindings must provide audio.plan(options) when audio is enabled',
      );
    audioPacketizer ||= createRtpPacketizer({
      codec: audioCodec,
      ssrc: options.audioSsrc ?? 2,
      RtpPacket: options.module?.RtpPacket,
      RtpHeader: options.module?.RtpHeader,
    });
    audioPermissionGranted =
      audioOptions.permissionGranted === true ||
      permissions.microphone === true ||
      permissions['microphone-capture'] === true;
    const audioCapturePlan = bindings.audio.plan({
      ...audioOptions,
      permissionGranted: audioPermissionGranted,
    });
    audioEncoderPlan = createAudioEncoderPlan({
      codec: audioCodec,
      bitrateKbps: audioBitrateKbps,
      channels: audioCapturePlan.channels,
      sampleRate: audioCapturePlan.sampleRate,
    });
    audioPipeline = createNativeMediaPipeline({
      capturePlan: audioCapturePlan,
      encoderPlan: audioEncoderPlan,
      spawnImpl: options.spawnImpl,
      maxOutputBytes: options.maxOutputBytes,
      stopTimeoutMs: options.stopTimeoutMs,
    });
  ...
  return createNativeWeriftHost({
    ...
    gameLauncher,
    launchValidator:
      gameLauncher && hostContentId
        ? (request) =>
            matchesHostLaunchRequest(request, {
