...
test('keyboard-only adapters never negotiate virtual-gamepad permission', () => {
  assert.equal(
    virtualGamepadPermissionGranted({
      inputEnabled: true,
      inputAdapter: { gamepad: false, keyboard: true, pointer: true },
    }),
    false,
  );
  assert.equal(
    virtualGamepadPermissionGranted({
      inputEnabled: true,
      inputAdapter: { gamepad: false, keyboard: true, pointer: true, rumble: true },
    }),
    false,
  );
...
test('virtual-gamepad permission requires both input enablement and a gamepad adapter', () => {
  assert.equal(
    virtualGamepadPermissionGranted({ inputEnabled: true, inputAdapter: { gamepad: true } }),
    false,
  );
  assert.equal(
    virtualGamepadPermissionGranted({
      inputEnabled: true,
      inputAdapter: { gamepad: true, virtualGamepad: true },
    }),
    true,
  );
  assert.equal(
    virtualGamepadPermissionGranted({ inputEnabled: false, inputAdapter: { gamepad: true } }),
    false,
  );
...
test('native input executor dispatches only permissioned normalized operations', async () => {
  const operations = [];
  const executor = createNativeInputExecutor({
    platform: 'linux',
    permissions: { 'remote-input': true },
    adapter: { platform: 'linux', execute: async (operation) => operations.push(operation) },
  });
  const plan = await executor.dispatch({
    type: 'input.event',
    action: 'look',
    kind: 'pointer',
    x: 1.4,
  ...
  assert.equal(plan.permission.granted, true);
...
test('native input executor fails closed without permission and does not call the adapter', async () => {
  ...
  assert.equal(executor.state, 'ready');
});
...
test('native input executor drops unsupported operations without poisoning later dispatch', async () => {
  ...
  const dropped = await executor.dispatch({
    type: 'input.event',
    action: 'key:IntlRo',
    kind: 'key',
    control: 'IntlRo',
  ...
  assert.equal(dropped.unsupported, true);
  assert.equal(dropped.reason, 'unsupported key');
  assert.equal(dropped.permission.granted, true);
  assert.equal(executor.state, 'active');
  const plan = await executor.dispatch({
    type: 'input.event',
    action: 'key:KeyA',
...
test('native input executor bounds dispatch rate and closes the adapter', async () => {
  ...
  assert.equal(executor.state, 'closed');
});
