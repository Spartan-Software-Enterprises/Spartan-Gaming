...
export function createPlatformNativeAdapter({platform, kind, bindings} = {}) {
  if (!PLATFORMS.has(platform)) throw new TypeError(`unsupported native adapter platform: ${platform}`);
  if (!KINDS.has(kind)) throw new TypeError(`unsupported native adapter kind: ${kind}`);
  if (!bindings || typeof bindings !== 'object') throw new TypeError('native bindings are required');
  for (const operation of OPERATIONS[kind]) if (typeof bindings[operation] !== 'function') throw new TypeError(`${platform} ${kind} adapter requires bindings.${operation}()`);
  const spec = PLATFORM_SPECS[platform];
  const adapter = {platform, kind, id: `${spec.id}-${kind}`, technology: spec.technologies[kind], permission: spec.permission[kind], capabilities: Object.freeze({platform, kind, technology: spec.technologies[kind], permission: spec.permission[kind]}), close: () => bindings.close?.()};
  for (const operation of OPERATIONS[kind]) adapter[operation] = (...args) => bindings[operation](...args);
...
export function getPlatformNativeAdapterSpec(platform, kind) {
  ...
  if (!KINDS.has(kind)) throw new TypeError(`unsupported native adapter kind: ${kind}`);
  const spec = PLATFORM_SPECS[platform];
  return Object.freeze({platform, kind, id: `${spec.id}-${kind}`, technology: spec.technologies[kind], permission: spec.permission[kind], operations: OPERATIONS[kind]});
}

export const PLATFORM_NATIVE_ADAPTER_SPECS = Object.freeze(Object.fromEntries([...PLATFORMS].flatMap(platform => [...KINDS].map(kind => [`${platform}-${kind}`, getPlatformNativeAdapterSpec(platform, kind)]))));
