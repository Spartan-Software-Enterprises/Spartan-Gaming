...
export function createReferenceGameLaunch({
  platform,
  runtimeId,
  runtimeKind = 'native-emulator',
  runtimeVersion = 'unversioned',
  runtimePath,
  gamePath,
  hostContentId,
  args = [],
  ...
  const runtimeProfile = Object.freeze({
    id,
    kind: runtimeKind,
    version: text(runtimeVersion, 'runtimeVersion', 80),
    trust: 'user-approved',
    enabled: true,
    executablePath: text(runtimePath, 'runtimePath', 1024),
  });
  const plan = createGameLaunchPlan({
    platform,
    runtimeProfile,
    gamePath: text(gamePath, 'gamePath', 1024),
    args,
    cwd,
    env,
  });
  ...
  return Object.freeze({
    ...
    matches(request) {
      try {
        return matchesHostLaunchRequest(request, {
          runtimeId: id,
...
export function createReferenceProtonLaunch({
  platform = 'linux',
  runtimeId = 'proton-local',
  runtimeVersion = 'unversioned',
  protonPath,
  gamePath,
  hostContentId,
  ...
  const contentId = text(hostContentId, 'hostContentId');
  if (!ID.test(contentId)) throw new TypeError('hostContentId contains unsupported characters');
  const runtimeProfile = createProtonRuntimeProfile({
    id,
    protonPath: text(protonPath, 'protonPath', 2048),
    version: text(runtimeVersion, 'runtimeVersion', 80),
    compatDataPath,
    steamClientPath,
    options,
  ...
  return Object.freeze({
    ...
    descriptor: Object.freeze({
      ...
      hostContentId: contentId,
      gameName,
      protonVersion: runtimeProfile.version,
    }),
    matches(request) {
      try {
        return matchesHostLaunchRequest(request, {
