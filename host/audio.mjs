...
export function validateAudioPermission({ platform, backend, environment = {} } = {}) {
  if (!PLATFORMS.has(platform) || !BACKENDS.has(backend))
    return Object.freeze({ allowed: false, reason: 'Unsupported audio platform or backend' });
  if (['win32', 'darwin'].includes(platform) && environment.microphoneGranted !== true)
    return Object.freeze({
      allowed: false,
      reason: 'Microphone/audio capture permission is not granted',
    });
  if (
    platform === 'linux' &&
    backend === 'pipewire' &&
    ...
    return Object.freeze({
      allowed: false,
      reason: 'PipeWire session environment is not available',
    });
...
function frozenDevices(devices) {
...
export function parsePactlSources(output = '') {
  ...
  for (const line of String(output).split('\n')) {
    ...
    devices.push({
      ...
      kind: name.endsWith('.monitor') ? 'monitor' : 'capture',
      state: fields[3],
    });
  }
...
export function parseAvfoundationDevices(output = '') {
  ...
  for (const line of String(output).split('\n')) {
    ...
    if (index !== null && line.trim()) {
      devices.push({ id: String(index), label: line.trim(), kind: 'microphone' });
      index = null;
    }
  }
  return frozenDevices(devices);
...
export function parseDshowAudioDevices(output = '') {
  ...
  for (const line of String(output).split('\n')) {
    ...
    if (inAudio && match)
      devices.push({ id: `audio=${match[1]}`, label: match[1], kind: 'microphone' });
  }
  return frozenDevices(devices);
}
...
export function listAudioCaptureDevices({ platform, backend, run, includeMonitors = false } = {}) {
  ...
  const format =
    backend === 'coreaudio' ? 'avfoundation' : backend === 'wasapi' ? 'dshow' : 'pulse';
  const output =
    backend === 'coreaudio'
  ...
  return includeMonitors
    ? devices
    : Object.freeze(devices.filter((device) => device.kind !== 'monitor'));
}

export function createAudioCapturePlan({
  platform,
  backend,
  source,
  channels = 2,
  ...
  if (!PLATFORMS.has(platform) || !BACKENDS.has(backend))
    throw new TypeError('unsupported audio platform or backend');
  const permission = validateAudioPermission({ platform, backend, environment });
  if (!permission.allowed) throw new Error(permission.reason);
  const boundedChannels = bounded(channels, 2, 1, 8);
  const boundedRate = bounded(sampleRate, 48000, 8000, 192000);
  const args = [
    '-hide_banner',
    '-loglevel',
    'warning',
  ...
  return Object.freeze({
    ...
    permission,
    process: createProcessLaunchPlan({ executable: resolveFfmpegCommand(environment), args }),
    output: Object.freeze({ format: 'f32le', target: 'stdout', requiresPublisher: true }),
  });
}

export function createAudioPublisherPlan({ capturePlan, codec = 'opus', bitrateKbps = 128 } = {}) {
  if (!capturePlan?.process?.args || capturePlan.output?.requiresPublisher !== true)
    throw new TypeError('capturePlan must require an audio publisher');
  if (!CODECS.has(codec)) throw new TypeError(`unsupported audio codec: ${codec}`);
  return Object.freeze({
    kind: 'audio-publisher',
    state: 'plan-only',
    ready: false,
    codec,
    bitrateKbps: bounded(bitrateKbps, 128, 16, 512),
    capture: Object.freeze({
      platform: capturePlan.platform,
      backend: capturePlan.backend,
      channels: capturePlan.channels,
      sampleRate: capturePlan.sampleRate,
      process: capturePlan.process,
    }),
    requires: Object.freeze(['native-audio-capture', 'webrtc-audio-publisher']),
  });
}

/** Create a shell-free encoder plan for raw float PCM captured by the audio plan. */
export function createAudioEncoderPlan({
  codec = 'opus',
  bitrateKbps = 128,
  channels = 2,
  sampleRate = 48000,
  ...
  const boundedChannels = bounded(channels, 2, 1, 8);
  const boundedRate = bounded(sampleRate, 48000, 8000, 192000);
  const boundedBitrate = bounded(bitrateKbps, 128, 16, 512);
  const encoder = codec === 'opus' ? 'libopus' : 'aac';
  const container = codec === 'opus' ? 'ogg-opus' : 'adts';
  const outputArgs =
    codec === 'opus'
      ? ['-frame_duration', '20', '-f', 'opus', '-page_duration', '20000', 'pipe:1']
      : ['-f', 'adts', 'pipe:1'];
  return Object.freeze({
    kind: 'audio-encoder',
    codec,
    ...
    process: createProcessLaunchPlan({
      executable: resolveFfmpegCommand({ SPARTAN_FFMPEG_COMMAND: ffmpegCommand }),
      args: [
        '-hide_banner',
        '-loglevel',
        'warning',
    ...
    output: Object.freeze({ container, target: 'stdout' }),
    requires: Object.freeze(['native-audio-capture', 'webrtc-audio-publisher']),
  });
}
...
export function normalizeAudioCapabilities(audio = {}) {
  return Object.freeze({
    version: 1,
    state: ['unconfigured', 'plan-only', 'ready', 'active', 'failed'].includes(audio.state)
      ? audio.state
      : 'unconfigured',
    codecs: Object.freeze(
      Array.isArray(audio.codecs) && audio.codecs.length
        ? [...new Set(audio.codecs.filter((codec) => CODECS.has(codec)))]
        : ['opus'],
    ),
    channels: bounded(audio.channels, 2, 1, 8),
    sampleRate: bounded(audio.sampleRate, 48000, 8000, 192000),
    requires: Object.freeze(
      Array.isArray(audio.requires) && audio.requires.length
        ? [...new Set(audio.requires.map(String))]
        : ['native-audio-capture', 'webrtc-audio-publisher'],
    ),
  });
}
 ...
 */
export function createAudioPublisher({
  pipeline,
  sink,
  codec = 'opus',
  ...
  if (!pipeline || typeof pipeline.start !== 'function' || typeof pipeline.stop !== 'function')
    throw new TypeError('an audio pipeline is required');
  if (!sink || typeof sink.write !== 'function')
    throw new TypeError('audio publisher sink must implement write(chunk)');
  if (!CODECS.has(codec)) throw new TypeError(`unsupported audio codec: ${codec}`);
  if (permissionGranted !== true) throw new Error('audio capture permission is not granted');
  const limit = Math.max(
    1024,
    Math.min(DEFAULT_MAX_AUDIO_CHUNK_BYTES, Number(maxChunkBytes) || DEFAULT_MAX_AUDIO_CHUNK_BYTES),
  );
  ...
  const publisher = {
    ...
    get capabilities() {
      return normalizeAudioCapabilities({
        state: state === 'active' ? 'active' : state === 'failed' ? 'failed' : 'ready',
        codecs: [codec],
      });
    },
    async start() {
      if (state !== 'unconfigured' && state !== 'ready')
        throw new Error(`audio publisher cannot start from ${state}`);
      state = 'ready';
      await pipeline.start();
      const output = pipeline.audioOutput;
      if (!output?.on) {
        state = 'failed';
        await pipeline.stop();
        throw new Error('audio pipeline did not expose an encoded audio stream');
      }
      sink.open?.({ codec });
      handler = (chunk) => {
        try {
          const value = Buffer.isBuffer(chunk) ? chunk : Buffer.from(chunk);
          if (value.length > limit) throw new Error('encoded audio chunk exceeds publisher limit');
    ...
    async stop() {
      if (state === 'unconfigured' || state === 'stopped') return this;
      if (handler && pipeline.audioOutput?.off) pipeline.audioOutput.off('data', handler);
      handler = null;
      await pipeline.stop();
      sink.close?.();
      state = 'stopped';
...
export function createRtpAudioPublisher({
  pipeline,
  packetizer,
  transport,
  codec = 'opus',
  ...
  if (!packetizer || typeof packetizer.push !== 'function')
    throw new TypeError('audio packetizer must implement push(chunk, metadata)');
  if (!transport || typeof transport.send !== 'function')
    throw new TypeError('audio transport must implement send(packet)');
  if (!['raw', 'ogg-opus'].includes(container) || (container === 'ogg-opus' && codec !== 'opus'))
    throw new TypeError('unsupported audio RTP container');
  let packetsSent = 0;
  let timestamp = 0;
  const extractor =
    container === 'ogg-opus'
      ? createOggOpusPacketExtractor({ maxBufferedBytes: maxChunkBytes })
      : null;
  const publisher = createAudioPublisher({
    ...
    sink: {
      ...
      write: (chunk) => {
        for (const frame of extractor ? extractor.push(chunk) : [chunk]) {
          const packets = packetizer.push(frame, { codec, timestamp });
          timestamp = (timestamp + 960) >>> 0;
          if (!packets || typeof packets[Symbol.iterator] !== 'function')
            throw new TypeError('audio packetizer.push must return an iterable of RTP packets');
