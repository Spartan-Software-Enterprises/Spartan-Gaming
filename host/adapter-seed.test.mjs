...
import {
  readSeedBundle,
  createSeedArtifactResolver,
  runSeedInstall,
  createSeedContextScript,
} from './adapter-seed.mjs';
...
async function buildSeedFixture(root) {
  ...
  const signedFeed = await signPackageManifest({
...
test('seed bundle exposes a verified feed and a local artifact resolver', async () => {
  ...
  try {
    ...
    assert.equal(
      await verifyReleaseFeed({ feed: bundle.feed, publicKeyJwk: bundle.publicKeyJwk }),
      true,
    );
...
test('seed install applies bundled adapters and reports up-to-date on refresh', async () => {
  ...
  try {
    const { seedRoot, archive } = await buildSeedFixture(root);
    const planned = await runSeedInstall({
      seedRoot,
      installRoot,
      platform: 'linux',
      apply: false,
    ...
    const applied = await runSeedInstall({
      seedRoot,
      installRoot,
      platform: 'linux',
      apply: true,
    ...
    assert.deepEqual(
      Buffer.from(await fs.readFile(join(installRoot, 'dolphin', '1.0.0', 'bin', 'dolphin'))),
      Buffer.from(new TextEncoder().encode('dolphin-binary')),
    );
    const refreshed = await runSeedInstall({
      seedRoot,
      installRoot,
      platform: 'linux',
      apply: false,
