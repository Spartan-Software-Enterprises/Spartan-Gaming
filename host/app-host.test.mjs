...
function unmaskPayload(chunk) {
  if (chunk.length < 6) return chunk.toString('utf8');
  const mask = chunk.subarray(2, 6);
  const payload = Buffer.from(chunk.subarray(6));
...
function startEchoAgent() {
  const server = createServer((request, response) => {
    if (request.url === '/health') {
      ...
      response.end(
        JSON.stringify({ service: 'spartan-host-reference', version: 1, hostName: 'Echo Agent' }),
      );
      return;
    }
  ...
  server.on('upgrade', (request, socket) => {
    ...
    socket.on('data', (chunk) => {
      socket.write(frame(`echo:${unmaskPayload(chunk)}`));
    });
...
test('app host serves the bundled frontend with an injected context script and health', async () => {
  const appHost = createAppHostServer({
    ...
    logger: { warn() {} },
    contextScript: {
      path: 'spartan-context.js',
      render: () => 'window.__SPARTAN_RELEASE_FEED__ = {schemaVersion: 1, records: []};',
    },
  });
...
test('app host proxies /session upgrades to an embedded agent and reports its health', async () => {
  ...
  try {
    const appHost = createAppHostServer({
      ...
      root: frontendRoot,
      logger: { warn() {} },
      agentEndpoint: `ws://127.0.0.1:${agentPort}/session`,
    });
    const address = await appHost.listen();
    try {
      const origin = `http://127.0.0.1:${address.port}`;
