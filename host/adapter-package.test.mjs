...
test('package extraction plans keep every target inside the destination', () => { const plan = createPackageExtractionPlan({manifest, destination: '/tmp/spartan-package'}); assert.equal(plan.entries.find(entry => entry.path === 'bin/adapter').target, '/tmp/spartan-package/bin/adapter'); assert.throws(() => createPackageExtractionPlan({manifest: {...manifest, files: [{...manifest.files[0], path: '/absolute'}]}, destination: '/tmp/spartan-package'}), /relative path/); });
