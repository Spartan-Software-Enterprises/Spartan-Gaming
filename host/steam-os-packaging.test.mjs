...
test('SteamOS packaging profile is user-scoped and immutable-host safe', () => {
  ...
  assert.throws(() => normalizeSteamOsPackagingProfile({installScope: 'system'}), /user installation/);
});

test('SteamOS install plans require consent and a verified digest', () => {
  const profile = normalizeSteamOsPackagingProfile();
  assert.throws(() => createSteamOsInstallPlan({profile, artifactPath: '/tmp/spartan.flatpak', artifactSha256: 'a'.repeat(64)}), /consent/);
  const plan = createSteamOsInstallPlan({profile, artifactPath: '/tmp/spartan.flatpak', artifactSha256: 'A'.repeat(64), consentGiven: true});
