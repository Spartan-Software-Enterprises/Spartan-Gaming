...
test('publisher plan composes capture and encoder plans without executing processes', () => {
  const capture = createCapturePlan({
    platform: 'linux',
    sourceType: 'x11',
    source: ':0.0',
    environment: { DISPLAY: ':0' },
  });
  const encoder = createEncoderPlan({
    codec: 'h264',
    width: 1280,
    height: 720,
    framerate: 60,
...
test('publisher capabilities retain bounded negotiated limits', () => {
  const publisher = normalizePublisherCapabilities({
    state: 'ready',
    transports: ['invalid', 'webrtc'],
    video: { codecs: ['invalid', 'vp9'], maxWidth: 99999, maxFramerate: 999 },
    audio: { channels: 99 },
  });
