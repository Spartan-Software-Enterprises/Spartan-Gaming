...
function localPath(value, name) {
  ...
  if (/^[a-z]+:\/\//i.test(result) || result.startsWith('\\\\'))
...
export function normalizeSteamOsProfile({
  platform = 'linux',
  detected = false,
  mode = 'game',
  ...
  framerate = 60,
  steamInput = 'fallback',
  launchMode = 'native-linux',
  steamAppId,
  protonConfigured = false,
  gamePathSelected = false,
} = {}) {
  ...
  if (!FRAMERATES.has(Number(framerate)))
    throw new RangeError('SteamOS framerate must be 30, 40, or 60');
  if (Number(width) !== 1280 || Number(height) !== 800)
    throw new RangeError('SteamOS handheld profile is capped at 1280x800');
  if (!['official-actions', 'fallback'].includes(steamInput))
    throw new TypeError('Steam Input mode is unsupported');
  return Object.freeze({
    platform,
    detected,
    mode,
    gamescopeEnabled: flag(gamescopeEnabled, 'gamescopeEnabled'),
    width: 1280,
    height: 800,
    ...
    overlaySafeInputFocus: true,
    sleepWakeRecovery: 'session-reconnect',
    power: Object.freeze({
      maxWidth: 1280,
      maxHeight: 800,
      supportedFramerates: Object.freeze([...FRAMERATES]),
    }),
    launch: createSteamOsLaunchReadiness({
      mode: launchMode,
      steamAppId,
      protonConfigured,
      gamePathSelected,
...
export function detectSteamOs({
  ...
  let content = '';
  try {
    content = String(readFile(osReleasePath, 'utf8'));
  } catch {
    return normalizeSteamOsProfile({ platform, detected: false });
  }
...
export function createGamescopeLaunchPlan({
  profile,
  gamescopePath,
  gamePath,
  gameArgs = [],
  ...
  if (!profile?.detected)
    throw new Error('Gamescope planning requires an explicitly detected SteamOS host');
  ...
  if (
    ...
    throw new TypeError('gameArgs must contain bounded argument strings');
  return Object.freeze({
    kind: 'gamescope-launch',
    platform: 'linux',
    profile,
    process: createProcessLaunchPlan({
      ...
      cwd: cwd === undefined ? undefined : localPath(cwd, 'gameCwd'),
    }),
    requires: Object.freeze([
      'detected-steamos-host',
      'user-selected-gamescope',
...
export function createSteamOsLaunchReadiness({
  mode = 'native-linux',
  steamAppId,
  protonConfigured = false,
  gamePathSelected = false,
  ...
  if (appId !== null && !/^\d{1,9}$/.test(appId))
    throw new TypeError('Steam app ID must be a bounded numeric identifier');
  const requires = [];
  let status = 'configuration-required';
  let reason = 'Select a user-owned game path before launch.';
  if (mode === 'native-linux' && gamePathSelected) {
    status = 'ready';
    reason = 'Native Linux launch is configured for a user-owned game path.';
  }
  if (mode === 'proton') {
    if (!protonConfigured) requires.push('proton-installation');
    if (!gamePathSelected) requires.push('user-selected-game-file');
    if (!requires.length) {
      status = 'ready';
      reason = 'User-owned Proton launch is configured.';
    } else reason = 'A Proton installation and user-selected game path are required.';
  }
  if (mode === 'steam') {
    if (!appId) requires.push('steam-app-id');
    status = 'official-handoff-required';
    reason =
      'Steam-owned launch requires an installed, user-approved Steam bridge; Spartan does not control the Steam client.';
  }
  if (mode === 'non-steam') {
    if (!gamePathSelected) requires.push('user-selected-game-file');
    status = 'bridge-required';
    reason = 'Non-Steam launch requires an operator-approved native or Proton handoff.';
  }
  return Object.freeze({
    mode,
    status,
    ...
    userOwned: mode !== 'steam',
    physicalValidationRequired: true,
  });
}

export {
  FRAMERATES as STEAMOS_FRAMERATES,
  MODES as STEAMOS_MODES,
  LAUNCH_MODES as STEAMOS_LAUNCH_MODES,
};
